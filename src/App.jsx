...
function MerchTracker({ me, onSignOut }){
  ...
  const findNext=()=>{ const f=frCase?frFind:(frFind||"").toLowerCase(); if(!f) return; let cells=[]; if(frScope==="selected"){ const R=rect(); if(R){ for(let r=R.r1;r<=R.r2;r++) for(let cc=R.c1;cc<=R.c2;cc++){ const row=rows[r], col=navCols[cc]; if(row&&col&&FR_COLS.includes(col)&&col!=="styleNo") cells.push([row.s,col]); } } } else { for(const row of rows) for(const col of FR_COLS){ if(col==="styleNo") continue; cells.push([row.s,col]); } } const matches=[]; const seen=new Set(); for(const [s,col] of cells){ const cur=frGet(s,col); if(!cur) continue; const hay=frCase?cur:cur.toLowerCase(); if(hay.indexOf(f)===-1) continue; const key=s.id+":"+col; if(seen.has(key)) continue; seen.add(key); matches.push({ id:s.id, col }); } if(!matches.length){ window.alert('No cells contain "'+frFind+'".'); return; } const ni=(findIdx+1)%matches.length; setFindIdx(ni); const t=matches[ni]; setSel({ id:t.id, col:t.col }); setFocus(null); scrollToCell(t.id, t.col); };
...
{history && (<div onClick={()=>setHistory(false)} style={{ position:"fixed", inset:0, background:"rgba(26,26,26,0.55)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}><div onClick={e=>e.stopPropagation()} style={{ background:"var(--surface)", border:"2px solid var(--ink)", boxShadow:"8px 8px 0 var(--ink)", width:640, maxWidth:"100%", maxHeight:"80vh", display:"flex", flexDirection:"column", fontFamily:"'JetBrains Mono',monospace" }}><div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 14px", borderBottom:"1px solid var(--line-3)" }}><div style={{ fontFamily:"'Archivo',sans-serif", fontWeight:800, fontSize:17 }}>Change history</div><button onClick={()=>setHistory(false)} style={{ border:"1px solid var(--ink)", background:"var(--bg)", cursor:"pointer", fontFamily:"inherit", fontSize:11, padding:"4px 9px" }}>Close</button></div><div style={{ padding:"8px 14px", borderBottom:"1px solid #f2f2f2" }}><input value={histFilter} onChange={e=>setHistFilter(e.target.value)} placeholder="filter by style no, field, stage, or person…" style={{ width:"100%", boxSizing:"border-box", fontFamily:"inherit", fontSize:11, padding:6, border:"1px solid var(--ink)" }}/></div><div style={{ overflowY:"auto", padding:"4px 0" }}>{auditBusy ? <div style={{ fontSize:11, color:"var(--muted-1)", padding:"24px", textAlign:"center" }}>Loading…</div> : (()=>{ const q=histFilter.trim().toLowerCase(); const fl=auditRows.filter(a=> !q || (a.style_no||"").toLowerCase().includes(q) || (a.field||"").toLowerCase().includes(q) || (a.actor_name||"").toLowerCase().includes(q) || (colLabelOf(a.col)||"").toLowerCase().includes(q)); if(fl.length===0) return <div style={{ fontSize:11, color:"var(--muted-1)", padding:"24px", textAlign:"center" }}>{auditRows.length===0?"No changes recorded yet.":"No matches."}</div>; return fl.map(a=>(<div key={a.id} style={{ padding:"7px 14px", borderBottom:"1px solid #f5f5f5", fontSize:11, display:"flex", gap:10, alignItems:"baseline" }}><span style={{ color:"var(--muted-1)", fontSize:9, whiteSpace:"nowrap", minWidth:80 }}>{tsShort(a.created_at)}</span><span style={{ flex:1 }}><b>{a.style_no||a.style_id}</b> · {colLabelOf(a.col)} <span style={{ color:"var(--muted-2)" }}>({a.field})</span><br/><span style={{ color:"var(--danger)" }}>{a.old_val||"—"}</span> <span style={{ color:"var(--muted-2)" }}>→</span> <span style={{ color:"var(--success)" }}>{a.new_val||"—"}</span> <span style={{ color:"var(--muted-1)" }}>· {a.actor_name}</span></span></div>)); })()}</div></div></div>)}
      ...
      {layoutMode==="workspace" && <div style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 22px 2px", fontSize:9.5, color:"var(--muted-2)", textTransform:"uppercase", letterSpacing:0.45 }}><span style={{ fontWeight:700, color:"var(--accent)" }}>Workspace</span><span>Search</span><span style={{ color:"var(--line-2)" }}>·</span><span>Actions</span><span style={{ color:"var(--line-2)" }}>·</span><span>View</span><span style={{ color:"var(--line-2)" }}>·</span><span>Collab</span><span style={{ marginLeft:"auto", textTransform:"none", letterSpacing:0, fontSize:9, color:"var(--muted-1)" }}>Compact mode keeps the original lighter control strip.</span></div>}
      <div style={layoutMode==="workspace"?{ display:"flex", gap:10, alignItems:"center", padding:"12px 14px 10px", margin:"8px 22px 8px", flexWrap:"wrap", background:"var(--surface)", border:"1px solid #d9d1c2", boxShadow:"3px 3px 0 rgba(26,26,26,0.08)" }:{ display:"flex", gap:10, alignItems:"center", padding:"12px 22px 6px", flexWrap:"wrap" }}>{layoutMode==="workspace" && <span style={{ fontSize:9, fontWeight:700, letterSpacing:0.5, textTransform:"uppercase", color:"var(--accent)", marginRight:4 }}>Search & filters</span>}
        ...
        <span style={{ position:"relative" }}><button onClick={(e)=>{ e.stopPropagation(); setFrOpen(o=>!o); }} title="Find & replace text in cells" style={{ fontFamily:"inherit", fontSize:11, padding:"6px 10px", cursor:"pointer", border:"1px solid var(--ink)", background:frOpen?"var(--ink)":"var(--surface)", color:frOpen?"var(--bg)":"var(--ink)", fontWeight:700 }}>⌕ Find/Replace</button>{frOpen && (<div onClick={e=>e.stopPropagation()} style={{ position:"absolute", top:"100%", left:0, marginTop:4, zIndex:75, background:"var(--surface)", border:"1px solid var(--ink)", boxShadow:"4px 4px 0 var(--ink)", padding:12, width:260 }}><div style={{ fontSize:11, fontWeight:700, marginBottom:8 }}>Find &amp; replace</div><label style={{ fontSize:10, color:"var(--muted-2)" }}>Find</label><input autoFocus value={frFind} onChange={e=>{ setFrFind(e.target.value); setFindIdx(-1); }} placeholder="text to find" style={{ width:"100%", boxSizing:"border-box", fontFamily:"inherit", fontSize:11, padding:5, marginBottom:6, border:"1px solid var(--ink)" }}/><label style={{ fontSize:10, color:"var(--muted-2)" }}>Replace with</label><input value={frRepl} onChange={e=>setFrRepl(e.target.value)} placeholder="(leave blank to delete)" style={{ width:"100%", boxSizing:"border-box", fontFamily:"inherit", fontSize:11, padding:5, marginBottom:8, border:"1px solid var(--ink)" }}/><div style={{ display:"flex", gap:12, fontSize:10, marginBottom:6 }}><label style={{ display:"flex", gap:4, cursor:"pointer" }}><input type="radio" checked={frScope==="filtered"} onChange={()=>setFrScope("filtered")}/>All filtered</label><label style={{ display:"flex", gap:4, cursor:"pointer" }}><input type="radio" checked={frScope==="selected"} onChange={()=>setFrScope("selected")}/>Selected cells</label></div><label style={{ fontSize:10, display:"flex", gap:5, marginBottom:8, cursor:"pointer" }}><input type="checkbox" checked={frCase} onChange={e=>setFrCase(e.target.checked)}/>Match case</label><div style={{ fontSize:10, color:"var(--muted-3)", marginBottom:8, minHeight:13 }}>{frScope==="selected" && !rect() ? "Select cells in the grid first." : (frFind ? findReplace(true)+" cell(s) will change" : "Applies to text fields (style, colour, brand, owner, remarks…)")}</div><div style={{ display:"flex", gap:8 }}><button disabled={!frFind} onClick={(e)=>{ e.stopPropagation(); findNext(); }} title="jump to the next cell containing this text" style={{ flex:1, fontFamily:"inherit", fontSize:11, fontWeight:700, padding:6, cursor:frFind?"pointer":"not-allowed", border:"1px solid var(--ink)", background:"var(--surface)", opacity:frFind?1:0.6 }}>Find next</button><button disabled={!frFind} onClick={(e)=>{ e.stopPropagation(); const n=findReplace(true); if(!n){ window.alert('No cells match "'+frFind+'".'); return; } const bw=frRepl.trim()===""?"\n\n\u26a0\ufe0f This will BLANK those cells.":""; if(!window.confirm('Replace "'+frFind+'" \u2192 "'+(frRepl||"(blank)")+'" in '+n+' cell(s)?'+bw)) return; findReplace(false); flash(); }} style={{ flex:1, fontFamily:"inherit", fontSize:11, fontWeight:700, padding:6, cursor:frFind?"pointer":"not-allowed", border:"1px solid var(--ink)", background:frFind?"var(--accent)":"var(--line-2)", opacity:frFind?1:0.6 }}>Replace all</button><button onClick={()=>setFrOpen(false)} style={{ fontFamily:"inherit", fontSize:11, padding:"6px 10px", cursor:"pointer", border:"1px solid var(--ink)", background:"var(--bg)" }}><X size={12}/></button></div></div>)}</span>
      ...
      <div style={layoutMode==="workspace"?{ display:"flex", gap:10, alignItems:"center", padding:"12px 14px 10px", margin:"0 22px 8px", flexWrap:"wrap", fontSize:10, color:"#777", background:"var(--surface)", border:"1px solid #d9d1c2", boxShadow:"3px 3px 0 rgba(26,26,26,0.08)" }:{ display:"flex", gap:10, alignItems:"center", padding:"0 22px 10px", flexWrap:"wrap", fontSize:10, color:"#777" }}>{layoutMode==="workspace" && <span style={{ fontSize:9, fontWeight:700, letterSpacing:0.5, textTransform:"uppercase", color:"var(--accent)", marginRight:4 }}>Cell tools & notes</span>}
