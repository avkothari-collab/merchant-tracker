...
function PeerTag({ who }){ if(!who) return null; return (<span style={{ position:"absolute", inset:0, border:"2px solid "+who.color, pointerEvents:"none", zIndex:4, boxSizing:"border-box" }}><span style={{ position:"absolute", top:-11, left:-1, background:who.color, color:"#fff", fontSize:8, fontWeight:700, padding:"0 3px", whiteSpace:"nowrap", lineHeight:"11px", maxWidth:90, overflow:"hidden", textOverflow:"ellipsis" }}>{who.name}</span></span>); }
function MerchTracker({ me, onSignOut }){
  ...
  const exportExcel=()=>{ try{ const data=rows.map(({s,c})=>{ const o={ "Order No":s.orderNo, "Style No":s.styleNo, "Sample Fit":s.sampleFit, "Family":s.family, "Colour":s.colour, "Brand":s.brand, "Fabric Type":s.fabricType, "Junior":s.owner, "Qty":s.qty, "Order Date":s.ordRec, "Delivery":s.delivery, "Status":c.status }; STAGES.forEach(st=>{ const r=(c.stages||[]).find(x=>x.key===st.key); o[st.label]= (s.actuals&&s.actuals[st.key])? fmt(parse(s.actuals[st.key])) : (r&&r.skipped?"WAIVED":(r&&r.rejected?"REJECTED":"")); }); return o; }); const ws=XLSX.utils.json_to_sheet(data); const wb=XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb,ws,"Tracker"); XLSX.writeFile(wb,"merch_tracker_"+iso(TODAY)+".xlsx"); }catch(e){ alert("Export failed: "+(e.message||e)); } };
  ...
  return (
    <div ref={gridRef} tabIndex={0} onKeyDown={onKeyDown} onCopy={handleCopy} onPaste={handlePaste} onMouseDown={(e)=>{ if(editing) return; if(e.target.closest && (e.target.closest("input")||e.target.closest("button")||e.target.closest("th"))) return; const td=e.target.closest && e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m||e.shiftKey) return; e.preventDefault(); setSel({ id:Number(m[1]), col:m[2] }); setFocus(null); selectingRef.current=true; setDragSel(true); }} onMouseUp={()=>{ if(filling){ applyFillHandle(); setFilling(false); setFillFrom(null); setFillTo(null); } if(selectingRef.current){ selectingRef.current=false; setDragSel(false); } }} onMouseOver={(e)=>{ const td=e.target.closest && e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m) return; if(filling){ setFillTo({ id:Number(m[1]), col:m[2] }); return; } if(selectingRef.current){ setFocus({ id:Number(m[1]), col:m[2] }); } }} onClick={()=>{ finishEditing(); setFillOpen(false); setColsOpen(false); setFilterCol(null); setFrOpen(false); }}
      ...
      <div style={{ display:"flex", gap:10, alignItems:"center", padding:"12px 22px 6px", flexWrap:"wrap" }}>
        ...
        <button onClick={(e)=>{ e.stopPropagation(); exportExcel(); }} title="export current (filtered) view to Excel" style={{ fontFamily:"inherit", fontSize:11, padding:"6px 11px", cursor:"pointer", border:"1px solid #1a1a1a", background:"#fff", fontWeight:700, display:"flex", alignItems:"center", gap:6 }}>⬇ Export</button>
      ...
      <div ref={scrollWrapRef} style={{ overflow:"auto", padding:"0 22px", maxHeight:"calc(100vh - 210px)" }}>
        <table role="grid" aria-label="Pre-production tracker grid. Arrow keys to move, Escape to exit, Tab to leave the grid." style={{ borderCollapse:"separate", borderSpacing:0, zoom:textScale, fontSize:11, tableLayout:"fixed", userSelect:dragSel?"none":"auto" }}>
          ...
          <thead><tr role="row">
            <Th col="__style" label="Style No" sort={sort} onSort={clickHeader} width={widthOf("__style")} onResize={onResize} onAutoFit={autoFit} scale={textScale} {...filterProps("__style")} sticky left={0} z={6}/>
            {visInfo.map(c=><Th key={c.key} col={c.key} label={c.label} sort={sort} onSort={clickHeader} width={widthOf(c.key)} onResize={onResize} onAutoFit={autoFit} scale={textScale} {...filterProps(c.key)} sticky={isFrozen(c.key)} left={isFrozen(c.key)?leftOf(c.key):undefined} z={5}/>)}
            {visStages.map(st=><Th key={st.key} col={st.key} label={st.label} sort={sort} onSort={clickHeader} width={widthOf(st.key)} onResize={onResize} onAutoFit={autoFit} scale={textScale} {...filterProps(st.key)}/>)}
            {remarksVis && <Th col="remarks" label={REMARK_COL.label} sort={sort} onSort={clickHeader} width={widthOf("remarks")} onResize={onResize} onAutoFit={autoFit} scale={textScale} {...filterProps("remarks")}/>}
          </tr></thead>
          <tbody>
...
function Th({ col, label, sort, onSort, sticky, left, z, width, onResize, onAutoFit, scale, filterActive, filterOpen, filterValues, filterAllowed, onToggleFilter, onSetFilter, onCloseFilter }){
