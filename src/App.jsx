...
`;
const REL_GATE_DAYS = 30, FABRIC_CUTOFF_DAYS = 35, FABRIC_AFTER_LAB_APPROVAL_DAYS = 20, CFG_RULE_VERSION = 2, STYLE_W = 190;
...
const STAGES = [
  ...
  { key:"fabricIH",  label:"Fabric IH",    lead:FABRIC_AFTER_LAB_APPROVAL_DAYS, owner:"Merchant", flag:null, pred:"labAppr" },
...
const DEFAULT_CFG = { planningRuleVersion:CFG_RULE_VERSION, leads:Object.fromEntries(STAGES.map(s=>[s.key,s.lead])), stageOwners:{...DEFAULT_STAGE_OWNERS}, rework:{...{ fitSend:4, artwork:2, strikeOff:3, labDip:7, ppSample:4 }}, fabricCutoff:FABRIC_CUTOFF_DAYS, relGate:REL_GATE_DAYS, upcoming:{...UPCOMING_DEFAULT}, escalationRules:DEFAULT_ESCALATION_RULES.map(x=>({...x})), todoEscalationRows:true };
const normalizeCfg=(raw)=>{ const d=raw&&typeof raw==="object"?raw:{}; const leads={...DEFAULT_CFG.leads,...(d.leads||{})}; /* v1 displayed Fabric IH as 0 but ignored it and hard-coded 15 + delivery cutoff. Migrate that unusable legacy value once; v2 then honours every edited value, including 0. */ if((Number(d.planningRuleVersion)||0)<CFG_RULE_VERSION) leads.fabricIH=FABRIC_AFTER_LAB_APPROVAL_DAYS; return { ...DEFAULT_CFG, ...d, planningRuleVersion:CFG_RULE_VERSION, leads, stageOwners:{...DEFAULT_CFG.stageOwners,...(d.stageOwners||{})}, rework:{...DEFAULT_CFG.rework,...(d.rework||{})}, upcoming:{...DEFAULT_CFG.upcoming,...(d.upcoming||{})}, escalationRules:Array.isArray(d.escalationRules)&&d.escalationRules.length?d.escalationRules:DEFAULT_ESCALATION_RULES.map(x=>({...x})), todoEscalationRows:d.todoEscalationRows!==undefined?!!d.todoEscalationRows:DEFAULT_CFG.todoEscalationRows, labels:{...(d.labels||{})} }; };
...
function computeStyle(s, cfg, resendMap={}){
  ...
  // Delivery minus Fabric Cut-off is a displayed reference only. It must never drive a plan, risk tone or downstream date.
  ...
  const predDone=(st)=>{ if(st.pred==="__ord") return true; let p=st.pred; while(p&&p!=="__ord"&&!appl(p)) p=stById(p)?.pred; return (!p||p==="__ord")?true:done(p); };
  ...
  return { stages, frontier, nextPending, lastActual, lastActualKey, status, tone, idle, float, released, fitBranch, printBranch, fabricBranch, ppBranch, prodFileBranch, fabricCountdown, fabricCutoffRef, projRelease, projTone, releaseGate, releaseOnTrack, pct, ownerToChase, chaseOwners };
}
...
// Header-filter direct values must be derived from the real column definitions.
// A hand-maintained whitelist previously omitted Buyer, Age Group and Extra 1/2,
// leaving those filter menus blank even though the columns displayed real data.
const FILTER_STYLE_TEXT_KEYS=new Set([...INFO_COLS.filter(c=>c.kind==="text").map(c=>c.key),"remarks"]);
...
const USER_MERCHANT_LINKS_SETTING_ID="user_merchant_links_v1";
...
function MerchTracker({ me, onSignOut }){
  const APP_BUILD_LABEL="v31dr Build 20 Morning EOD + Merchant Filter";
  ...
  const [expOpen,setExpOpen]=useState(false); const [expMode,setExpMode]=useState(()=>{ try{ return localStorage.getItem("mt_exp_mode")||"full"; }catch(e){ return "full"; } }); const [expBuf,setExpBuf]=useState(()=>{ try{ const v=parseInt(localStorage.getItem("mt_exp_buf"),10); return (isFinite(v)&&v>=0&&v<=30)?v:2; }catch(e){ return 2; } }); const [expBuyerPlans,setExpBuyerPlans]=useState(()=>{ try{ return localStorage.getItem("mt_exp_buyer_plans")!=="0"; }catch(e){ return true; } }); const [expBuyerBuffer,setExpBuyerBuffer]=useState(()=>{ try{ const v=localStorage.getItem("mt_exp_buyer_buffer"); return v==null?localStorage.getItem("mt_exp_incbuf")==="1":v==="1"; }catch(e){ return false; } }); const [expBuyerFormat,setExpBuyerFormat]=useState(()=>{ try{ return localStorage.getItem("mt_exp_buyer_format")==="maxPavitra"?"maxPavitra":"standard"; }catch(e){ return "standard"; } }); const [expRelMode,setExpRelMode]=useState(()=>{ try{ return localStorage.getItem("mt_exp_relmode")||"detailed"; }catch(e){ return "detailed"; } });
  ...
  useEffect(()=>{ try{ localStorage.setItem("mt_exp_mode",expMode); localStorage.setItem("mt_exp_buf",String(expBuf)); localStorage.setItem("mt_exp_buyer_plans",expBuyerPlans?"1":"0"); localStorage.setItem("mt_exp_buyer_buffer",expBuyerBuffer?"1":"0"); localStorage.setItem("mt_exp_buyer_format",expBuyerFormat); localStorage.setItem("mt_exp_relmode",expRelMode); localStorage.setItem("mt_fr_scope",frScope); localStorage.setItem("mt_fr_case",frCase?"1":"0"); }catch(e){} },[expMode,expBuf,expBuyerPlans,expBuyerBuffer,expBuyerFormat,expRelMode,frScope,frCase]);
  ...
  const [comments,setComments]=useState({}); const [team,setTeam]=useState([]); const [userMerchantLinks,setUserMerchantLinks]=useState({}); const [threadCell,setThreadCell]=useState(null); const [cmText,setCmText]=useState("");
  ...
  const loadShared=async(opts={})=>{ const useCache=opts.useCache!==false; if(useCache) hydrateOfflineCache(); try{
    ...
    try{ const lmRes=await supabase.from("app_settings").select("data").eq("id",USER_MERCHANT_LINKS_SETTING_ID).maybeSingle(); const d=lmRes&&lmRes.data&&lmRes.data.data; setUserMerchantLinks(d&&typeof d==="object"?((d.links&&typeof d.links==="object")?d.links:d):{}); }catch(e){ setUserMerchantLinks({}); }
  ...
  const setFabricOrderColourDate=(orderNo,colour,field,val)=>{
    if(!["actual","revised"].includes(field)) return;
    const colourToken=fabricColourToken(colour);
    const targets=styles.filter(s=>String(s.orderNo||"")===String(orderNo||"")&&splitColoursAll(s.colour).some(c=>fabricColourToken(c)===colourToken));
    if(!targets.length) return;
    const currentValue=(s)=>{ const rec=fabricColourRecord(s,colour)||{}; return rec[field]||(!hasSpecificFabricColours(s)?(field==="actual"?(s.actuals&&s.actuals.fabricIH):(s.revs&&s.revs.fabricIH)):"")||""; };
    const changed=targets.filter(s=>String(currentValue(s)||"")!==String(val||""));
    if(!changed.length) return;
    const existingDifferent=changed.filter(s=>currentValue(s));
    if(existingDifferent.length){
      const verb=val?"replace":"clear";
      const ok=window.confirm(`This will ${verb} ${field} Fabric IH for ${changed.length} style${changed.length===1?"":"s"} in order ${orderNo}, colour ${colour}. Continue?`);
      if(!ok) return;
    }
    pushHistory();
    const ids=new Set(changed.map(s=>String(s.id)));
    setStyles(prev=>prev.map(s=>{
      ...
      next[colourToken]={...(next[colourToken]||{}),colour,[field]:val||""};
      return {...s,colourFabricIH:next};
    }));
    try{
      const auditRows=changed.map(s=>({ style_id:s.id, style_no:s.styleNo||"", col:"Fabric IH · "+colour, field:field==="actual"?"actual":"revised", old_val:currentValue(s)||null, new_val:val||null, actor_id:me.id, actor_name:me.name||me.email }));
      supabase.from("audit_log").insert(auditRows).then(()=>{}).catch(()=>{});
    }catch(_e){}
    flash();
  };
  ...
  const runExport=(mode,buf,buyerPlans,relMode,buyerBuffer=false,buyerFormat="standard")=>{ try{ let data, name, sheet="Tracker"; const B=Math.abs(buf||0);
    ...
    const buyerStageLabel=(k)=>({ techpack:"Techpack", fitSend:"Fit Sample Sent", fitAppr:"Fit Approval", artwork:"Artwork", artAppr:"Artwork Approval", strikeOff:"Strike-off", soAppr:"Strike-off Approval", labDip:"Lab Dip", labAppr:"Lab Dip Approval", fabricIH:"Fabric In-House", ppSample:"PP Sample", ppAppr:"PP Approval", prodFile:"Production File" }[k]||(STAGES.find(st=>st.key===k)||{}).label||k);
    const buyerDueDate=(r,useBuffer)=>{ const due=r&&(r.rev||r.plan); return due&&useBuffer&&B?addWorkdays(due,B):due; };
    const buyerDateCell=(s,c,k,includePlans,useBuffer)=>{ const r=stageRow(c,k); if(!r) return ""; const a=activeActualDate(s,c,k); if(a) return `Done · ${fmtY(a)}`; if(!includePlans||r.skipped||r.autoClosed) return ""; const due=buyerDueDate(r,useBuffer); return due?`Due · ${fmtY(due)}`:""; };
    const buyerPredReady=(s,c,k)=>{ if(k==="ppSample") return !!stageRow(c,"fabricIH")?.done && (!s.fitReq||!!stageRow(c,"fitAppr")?.done); if(k==="prodFile"){ const base=(s.ppBypass||!s.ppNeeded)?!!stageRow(c,"fabricIH")?.done:!!stageRow(c,"ppAppr")?.done; return base&&(!s.fitReq||!!stageRow(c,"fitAppr")?.done); } const st=STAGES.find(x=>x.key===k); if(!st||!st.pred||st.pred==="__ord") return true; let p=st.pred; while(p&&p!=="__ord"&&!stageRow(c,p)) p=(STAGES.find(x=>x.key===p)||{}).pred; return !p||p==="__ord"||!!stageRow(c,p)?.done; };
    const buyerPendingAction=(r)=>{ const normal={ techpack:"techpack to be issued", fitSend:"fit sample to be sent", fitAppr:"fit approval awaited", artwork:"artwork to be sent", artAppr:"artwork approval awaited", strikeOff:"strike-off to be sent", soAppr:"strike-off approval awaited", labDip:"lab dip to be sent", labAppr:"lab dip approval awaited", fabricIH:"fabric in-house awaited", ppSample:"PP sample to be sent", ppAppr:"PP approval awaited", prodFile:"production file to be released" }; const resend={ fitSend:"fit sample re-send pending", artwork:"revised artwork to be sent", strikeOff:"strike-off re-send pending", labDip:"lab dip re-send pending", ppSample:"PP sample re-send pending" }; const reapproval={ fitAppr:"fit re-approval awaited", artAppr:"artwork re-approval awaited", soAppr:"strike-off re-approval awaited", labAppr:"lab dip re-approval awaited", ppAppr:"PP re-approval awaited" }; return r.rework?(resend[r.key]||`${buyerStageLabel(r.key)} rework pending`):r.rejected?(reapproval[r.key]||`${buyerStageLabel(r.key)} re-approval awaited`):(normal[r.key]||`${buyerStageLabel(r.key)} pending`); };
    const buyerCurrentActivity=(s,c,useBuffer)=>{ if(c.released) return "Released"; let pending=(c.frontier?[...c.frontier]:[]).map(k=>stageRow(c,k)).filter(r=>r&&!r.done&&!r.autoClosed&&buyerPredReady(s,c,r.key)); if(!pending.length&&c.nextPending&&!c.nextPending.done&&!c.nextPending.autoClosed) pending=[c.nextPending]; pending.sort((a,b)=>stageOrderOf(a.key)-stageOrderOf(b.key)); const labels=pending.map(r=>{ const owner=REJECTABLE.includes(r.key)?"Buyer":"Our Team"; const due=buyerDueDate(r,useBuffer); const late=due&&TODAY>due?Math.max(1,netWorkdays(due,TODAY)||0):0; const overdue=late?` · ${late} working day${late===1?"":"s"} overdue`:""; return `${owner} — ${buyerPendingAction(r)}${overdue}`; }); return labels.join(" + ")||"Pending update"; };
    const buyerLastCompleted=(c)=>{ const r=c.lastActualKey?stageRow(c,c.lastActualKey):null; return r&&c.lastActual?`${buyerStageLabel(r.key)} — ${fmtY(c.lastActual)}`:""; };
    const maxPavitraGroupOf=(s)=>{ const text=`${s.family||""} ${s.age||""} ${s.styleNo||""}`.toLowerCase(); if(text.includes("girl")||String(s.styleNo||"").toLowerCase().startsWith("yg")) return "Girls"; if(text.includes("boy")||String(s.styleNo||"").toLowerCase().startsWith("yb")) return "Boys"; return "Other"; };
    const writeMaxPavitraExport=()=>{
      const useBuyerBuffer=!!(buyerPlans&&buyerBuffer&&B);
      const headers=["SL NO","STYLE","CATEGORY","IMAGE","FABRIC","SIZE GROUP","QUANTITY (SETS)","KOTARI","Remarks","REVISED COST","Buyer Remarks @ Pavithra L","Final Closed Cost","FIT SAMPLE","STRIKE OFF","LABDIP","PP SAMPLE","PRODUCTION DATE","FI DATE","REMARKS"];
      const grouped={ Girls:[], Boys:[], Other:[] };
      rows.forEach(row=>grouped[maxPavitraGroupOf(row.s)].push(row));
      const wb=XLSX.utils.book_new();
      const groups=["Girls","Boys","Other"].filter(g=>grouped[g].length);
      if(!groups.length){ alert("No filtered styles to export."); return; }
      groups.forEach(group=>{
        const groupRows=grouped[group];
        const buyers=[...new Set(groupRows.map(({s})=>s.buyer||s.brand).filter(Boolean))];
        const titleBuyer=buyers.length===1?buyers[0]:"LOB";
        const title=`${titleBuyer} - KW SLEEPWEAR${group==="Other"?"":` ${group.toUpperCase()}`}`;
        const body=groupRows.map(({s,c},i)=>{
          const current=buyerCurrentActivity(s,c,useBuyerBuffer);
          const remark=[current,s.remarks].filter(Boolean).join(" | ");
          const fiDate=buyerPlans&&s.delivery?(useBuyerBuffer?addWorkdays(parse(s.delivery),B):parse(s.delivery)):null;
          const fi=fiDate?`Due · ${fmtY(fiDate)}`:"";
          return [
            i+1,s.styleNo||"",group==="Other"?(s.family||""):group.toUpperCase(),"",
            s.fabricType||"",s.age||"",s.qty||0,
            "","","","","",
            buyerDateCell(s,c,"fitSend",buyerPlans,useBuyerBuffer),
            buyerDateCell(s,c,"strikeOff",buyerPlans,useBuyerBuffer),
            buyerDateCell(s,c,"labDip",buyerPlans,useBuyerBuffer),
            buyerDateCell(s,c,"ppSample",buyerPlans,useBuyerBuffer),
            buyerDateCell(s,c,"prodFile",buyerPlans,useBuyerBuffer),
            fi,remark
          ];
        });
        const aoa=[
        ...
        ws["!cols"]=[
          {wch:8},{wch:22},{wch:14},{wch:16},{wch:24},{wch:14},{wch:15},
          {wch:13,hidden:true},{wch:22,hidden:true},{wch:15,hidden:true},{wch:25,hidden:true},
          {wch:17},{wch:20},{wch:19},{wch:18},{wch:18},{wch:20},{wch:18},{wch:48}
        ];
        ws["!rows"]=[{hpt:8},{hpt:28},{hpt:60},...body.map(()=>({hpt:38}))];
        ws["!autofilter"]={ref:XLSX.utils.encode_range({s:{r:2,c:0},e:{r:body.length+2,c:headers.length-1}})};
        ...
        const titleStyle={fill:{fgColor:{rgb:group==="Boys"?"CC9900":"B4C6E7"}},font:{bold:true,sz:20,color:{rgb:group==="Boys"?"FFFFFF":"000000"}},alignment:{horizontal:"center",vertical:"center"}};
        const headerStyle={fill:{fgColor:{rgb:"C6E0B4"}},font:{bold:true,color:{rgb:"000000"}},alignment:{horizontal:"center",vertical:"center",wrapText:true},border:{top:{style:"thin",color:{rgb:"666666"}},bottom:{style:"thin",color:{rgb:"666666"}},left:{style:"thin",color:{rgb:"666666"}},right:{style:"thin",color:{rgb:"666666"}}}};
        const bodyStyle={alignment:{horizontal:"center",vertical:"center",wrapText:true},border:{top:{style:"thin",color:{rgb:"A6A6A6"}},bottom:{style:"thin",color:{rgb:"A6A6A6"}},left:{style:"thin",color:{rgb:"A6A6A6"}},right:{style:"thin",color:{rgb:"A6A6A6"}}}};
        for(let col=0;col<headers.length;col++){ const h=XLSX.utils.encode_cell({r:2,c:col}); if(ws[h]) ws[h].s=headerStyle; }
        const t=XLSX.utils.encode_cell({r:1,c:0}); if(ws[t]) ws[t].s=titleStyle;
        for(let r=3;r<body.length+3;r++) for(let col=0;col<headers.length;col++){ const ref=XLSX.utils.encode_cell({r,c:col}); if(ws[ref]) ws[ref].s=bodyStyle; }
        const sheetName=group==="Other"?"LOB styles Other":`LOB styles ${group}`;
        XLSX.utils.book_append_sheet(wb,ws,sheetName);
      });
      const suffix=buyerPlans?(useBuyerBuffer?`progress_buf${B}d`:"progress"):"actuals_only";
      XLSX.writeFile(wb,`max_pavitra_tna_${suffix}_${iso(TODAY)}.xlsx`);
    };
    if(mode==="buyer"&&buyerFormat==="maxPavitra"){ writeMaxPavitraExport(); return; }
    if(mode==="buyer"){ const useBuyerBuffer=!!(buyerPlans&&buyerBuffer&&B); const buyerStages=STAGES.filter(st=>st.key!=="techpack"&&rows.some(({c})=>!!stageRow(c,st.key))); data=rows.map(({s,c})=>{ const buyerBrand=[s.buyer,s.brand].filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i).join(" / "); const o={ "Order No":s.orderNo||"", "Style No":s.styleNo||"", "Colour":s.colour||"", "Buyer / Brand":buyerBrand, "Qty":s.qty||0, "Delivery":fmtIso(s.delivery), "Current Pending Action":buyerCurrentActivity(s,c,useBuyerBuffer), "Last Completed Activity":buyerLastCompleted(c) }; buyerStages.forEach(st=>{ o[buyerStageLabel(st.key)]=buyerDateCell(s,c,st.key,buyerPlans,useBuyerBuffer); }); o["Remarks"]=s.remarks||""; return o; }); name=buyerPlans?(useBuyerBuffer?`buyer_tna_progress_buf${B}d`:"buyer_tna_progress"):"buyer_tna_actuals_only"; sheet="Main Data"; }
    ...
    const ws=XLSX.utils.json_to_sheet(data); if(mode==="buyer"&&data&&data.length){ const headers=Object.keys(data[0]); ws["!autofilter"]={ ref:XLSX.utils.encode_range({ s:{r:0,c:0}, e:{r:data.length,c:Math.max(0,headers.length-1)} }) }; const widthOf=(h)=>h==="Current Pending Action"?48:h==="Last Completed Activity"?30:h==="Remarks"?42:h==="Colour"||h==="Buyer / Brand"?22:h==="Style No"?20:h==="Order No"?16:h==="Qty"?10:21; ws["!cols"]=headers.map(h=>({wch:widthOf(h)})); ws["!rows"]=[{hpt:22}]; ws["!margins"]={ left:0.25, right:0.25, top:0.5, bottom:0.5, header:0.2, footer:0.2 }; ws["!pageSetup"]={ orientation:"landscape", fitToWidth:1, fitToHeight:0 }; }
    const wb=XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb,ws,sheet); XLSX.writeFile(wb,name+"_"+iso(TODAY)+".xlsx"); }catch(e){ logAppError("export failed",e); alert("Export failed: "+(e.message||e)); } };
  ...
  const valueFor=(s,cc,col)=>{
    ...
    if(FILTER_STYLE_TEXT_KEYS.has(col)) return s[col]||"(Blanks)";
    ...
    if(STAGE_KEYS.includes(col)){ const r=(cc.stages||[]).find(x=>x.key===col); if(!r) return "— N/A"; if(r.skipped) return "SKIPPED"; if(r.autoClosed) return "AUTO-CLOSED"; if(r.actual) return fmt(r.actual)||"ACTUAL"; if(r.rework) return "↻ Redo & resend"; if(r.rejected) return r.reject?("✕ Rejected "+fmt(r.reject)):"✕ Rejected"; if(r.rev) return "✎ Revised "+fmt(r.rev); return "● Pending"; }
    return "(Blanks)";
  };
  ...
  return (
    <div ref={gridRef} tabIndex={0} onKeyDown={onKeyDown} onCopy={handleCopy} onPaste={handlePaste} onContextMenu={(e)=>{ const td=e.target.closest&&e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m) return; e.preventDefault(); const id=Number(m[1]), col=m[2]; if(!selKeys.has(`${id}:${col}`)) setSingleSelection(id,col); setCtxMenu({ x:e.clientX, y:e.clientY, id, col }); }} onMouseDown={(e)=>{ if(editing) return; if(e.button===2) return; if(e.target.closest && (e.target.closest("input")||e.target.closest("button")||e.target.closest("th"))) return; const td=e.target.closest && e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m) return; e.preventDefault(); gridRef.current&&gridRef.current.focus(); const id=Number(m[1]), col=m[2]; setCtxMenu(null); if((e.ctrlKey||e.metaKey)||e.shiftKey) return; setSel({ id, col }); setFocus(null); setMultiAreas([]); selectingRef.current=true; setDragSel(true); }} onMouseUp={()=>{ if(filling){ applyFillHandle(); setFilling(false); setFillFrom(null); setFillTo(null); } if(selectingRef.current){ selectingRef.current=false; setDragSel(false); } }} onMouseOver={(e)=>{ const td=e.target.closest && e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m) return; if(filling){ setFillTo({ id:Number(m[1]), col:m[2] }); return; } if(selectingRef.current){ setFocus({ id:Number(m[1]), col:m[2] }); } }} onClick={()=>{ finishEditing(); setCtxMenu(null); setFillOpen(false); setColsOpen(false); setFilterCol(null); setFrOpen(false); setExpOpen(false); setBellOpen(false); setPeersOpen(false); }}
          ...
          <div style={{ padding:18, fontSize:12, lineHeight:1.6 }}>
            ...
            {helpTab==="logic" && <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))", gap:12 }}>
              {[
                ...
                ["Working days", ["Sunday is ignored", "Lead days are working days", "Fabric IH follows Lab Dip Approval + its editable lead (20 days by default)", "If Lab Dip is not required, the Fabric IH lead starts from Order Date", "Delivery minus Fabric cut-off is reference-only"]],
      ...
      {usersOpen && <UsersPanel me={me} merchantNames={[...new Set((styles||[]).map(s=>String(s&&s.owner||"").trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b))} userMerchantLinks={userMerchantLinks} onLinksChange={setUserMerchantLinks} onTeamChange={setTeam} onClose={()=>setUsersOpen(false)}/>}
{history && (<div onClick={()=>setHistory(false)} style={{ position:"fixed", inset:0, background:"rgba(26,26,26,0.55)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}><div onClick={e=>e.stopPropagation()} style={{ background:"var(--surface)", border:"2px solid var(--ink)", boxShadow:"8px 8px 0 var(--ink)", width:640, maxWidth:"100%", maxHeight:"80vh", display:"flex", flexDirection:"column", fontFamily:"'JetBrains Mono',monospace" }}><div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 14px", borderBottom:"1px solid var(--line-3)" }}><div style={{ fontFamily:"'Archivo',sans-serif", fontWeight:800, fontSize:17 }}>Change history</div><button onClick={()=>setHistory(false)} style={{ border:"1px solid var(--ink)", background:"var(--bg)", cursor:"pointer", fontFamily:"inherit", fontSize:11, padding:"4px 9px" }}>Close</button></div><div style={{ padding:"8px 14px", borderBottom:"1px solid #f2f2f2" }}><input value={histFilter} onChange={e=>setHistFilter(e.target.value)} placeholder="filter by style no, field, stage, or person…" style={{ width:"100%", boxSizing:"border-box", fontFamily:"inherit", fontSize:11, padding:6, border:"1px solid var(--ink)" }}/></div><div style={{ overflowY:"auto", padding:"4px 0" }}>{auditBusy ? <div style={{ fontSize:11, color:"var(--muted-1)", padding:"24px", textAlign:"center" }}>Loading…</div> : (()=>{ const q=histFilter.trim().toLowerCase(); const fl=auditRows.filter(a=> !q || (a.style_no||"").toLowerCase().includes(q) || (a.field||"").toLowerCase().includes(q) || (a.actor_name||"").toLowerCase().includes(q) || (colLabelOf(a.col)||"").toLowerCase().includes(q)); if(fl.length===0) return <div style={{ fontSize:11, color:"var(--muted-1)", padding:"24px", textAlign:"center" }}>{auditRows.length===0?"No changes recorded yet.":"No matches."}</div>; return fl.map(a=>(<div key={a.id} style={{ padding:"7px 14px", borderBottom:"1px solid #f5f5f5", fontSize:11, display:"flex", gap:10, alignItems:"baseline" }}><span style={{ color:"var(--muted-1)", fontSize:9, whiteSpace:"nowrap", minWidth:80 }}>{tsShort(a.created_at)}</span><span style={{ flex:1 }}><b>{a.style_no||a.style_id}</b> · {colLabelOf(a.col)} <span style={{ color:"var(--muted-2)" }}>({a.field})</span><br/><span style={{ color:"var(--danger)" }}>{a.old_val||"—"}</span> <span style={{ color:"var(--muted-2)" }}>→</span> <span style={{ color:"var(--success)" }}>{a.new_val||"—"}</span> <span style={{ color:"var(--muted-1)" }}>· {a.actor_name}</span></span></div>)); })()}</div></div></div>)}
      ...
      <div className="mt-tracker-toolbar" style={{ display:"flex", gap:8, alignItems:"center", padding:"12px 22px 6px", flexWrap:"wrap" }}>
        ...
        <span style={{ position:"relative" }}><button onClick={(e)=>{ e.stopPropagation(); setExpOpen(o=>!o); }} title="export options" style={{ fontFamily:"inherit", fontSize:11, padding:"6px 11px", cursor:"pointer", border:"1px solid var(--ink)", background:expOpen?"var(--ink)":"var(--surface)", color:expOpen?"var(--bg)":"var(--ink)", fontWeight:700, display:"flex", alignItems:"center", gap:6 }}>⬇ Export</button>{expOpen && (<div onClick={e=>e.stopPropagation()} style={{ position:"absolute", top:"100%", left:0, marginTop:4, zIndex:370, background:"var(--surface)", border:"1px solid var(--ink)", boxShadow:"4px 4px 0 var(--ink)", padding:12, width:288 }}><div style={{ fontSize:11, fontWeight:700, marginBottom:8 }}>Export {rows.length} filtered styles</div>{[["full","Full \u2014 all columns (actual dates)"],["detail","Detailed summary \u2014 everything (toggles + every stage)"],["release","Release plan \u2014 for production (incl. blockers)"],["internal","Internal plan (with buffer)"],["buyer","Buyer TNA \u2014 crisp progress / actuals"]].map(([v,lbl])=>(<label key={v} style={{ display:"flex", gap:6, fontSize:10, padding:"3px 0", cursor:"pointer", alignItems:"flex-start" }}><input type="radio" checked={expMode===v} onChange={()=>setExpMode(v)} style={{ marginTop:1 }}/>{lbl}</label>))}{expMode==="release" && (<div style={{ display:"flex", gap:6, margin:"4px 0 2px" }}>{[["detailed","Detailed"],["summary","Summary"]].map(([v,l])=>(<button key={v} onClick={()=>setExpRelMode(v)} style={{ flex:1, fontFamily:"inherit", fontSize:10, fontWeight:700, padding:"4px 0", cursor:"pointer", border:"1px solid var(--ink)", background:expRelMode===v?"var(--ink)":"var(--surface)", color:expRelMode===v?"var(--bg)":"var(--ink)" }}>{l}</button>))}</div>)}{expMode==="buyer" && (<div style={{ borderTop:"1px solid var(--line-3)", marginTop:5, paddingTop:7 }}><div style={{ fontSize:9, fontWeight:900, color:"var(--muted-2)", textTransform:"uppercase", marginBottom:5 }}>Buyer format</div><div style={{ display:"flex", gap:6, marginBottom:6 }}>{[["standard","Standard"],["maxPavitra","Max / Pavitra"]].map(([v,l])=><button key={v} onClick={()=>setExpBuyerFormat(v)} style={{ flex:1, fontFamily:"inherit", fontSize:9.5, fontWeight:800, padding:"5px 4px", cursor:"pointer", border:"1px solid var(--ink)", background:expBuyerFormat===v?"var(--ink)":"var(--surface)", color:expBuyerFormat===v?"var(--bg)":"var(--ink)" }}>{l}</button>)}</div>{expBuyerFormat==="maxPavitra"&&<div style={{ fontSize:8.5, color:"var(--muted-1)", lineHeight:1.4, marginBottom:7 }}>Girls/Boys sheets in the buyer template; images omitted. Production Date uses Production File and FI Date uses Delivery.</div>}<div style={{ fontSize:9, fontWeight:900, color:"var(--muted-2)", textTransform:"uppercase", marginBottom:5 }}>Buyer date mode</div><div style={{ display:"flex", gap:6 }}>{[[true,"Progress TNA"],[false,"Actuals only"]].map(([v,l])=><button key={l} onClick={()=>setExpBuyerPlans(v)} style={{ flex:1, fontFamily:"inherit", fontSize:9.5, fontWeight:800, padding:"5px 4px", cursor:"pointer", border:"1px solid var(--ink)", background:expBuyerPlans===v?"var(--ink)":"var(--surface)", color:expBuyerPlans===v?"var(--bg)":"var(--ink)" }}>{l}</button>)}</div><div style={{ fontSize:8.5, color:"var(--muted-1)", lineHeight:1.4, marginTop:5 }}>{expBuyerPlans?"Completed dates say Done; pending dates say Due.":"Only completed dates marked Done; pending activity dates stay blank."}</div>{expBuyerPlans && <div style={{ marginTop:7, paddingTop:7, borderTop:"1px solid var(--line-3)" }}><label style={{ display:"flex", gap:6, alignItems:"center", fontSize:9.5, fontWeight:800, cursor:"pointer" }}><input type="checkbox" checked={expBuyerBuffer} onChange={e=>setExpBuyerBuffer(e.target.checked)}/>Apply plan buffer</label>{expBuyerBuffer && <div style={{ fontSize:8.5, color:"var(--muted-1)", lineHeight:1.45, marginTop:5 }}>Shift every remaining Due date by <input type="number" min={0} max={30} value={expBuf} onChange={e=>setExpBuf(Math.max(0,Math.min(30,Number(e.target.value)||0)))} style={{ width:40, fontFamily:"inherit", fontSize:10, padding:"2px 4px", border:"1px solid var(--ink)", margin:"0 3px" }}/> working days. Applied once to the whole pending cascade; Done dates never move.</div>}</div>}</div>)}{expMode==="internal" && (<div style={{ fontSize:10, color:"var(--muted-4)", margin:"6px 0 4px", lineHeight:1.5, background:"#f7f4ee", border:"1px solid #e6e0d4", padding:"6px 8px" }}>Add buffer to internal plan dates of <input type="number" min={0} max={30} value={expBuf} onChange={e=>setExpBuf(Math.max(0,Math.min(30,Number(e.target.value)||0)))} style={{ width:40, fontFamily:"inherit", fontSize:11, padding:"2px 4px", border:"1px solid var(--ink)", margin:"0 4px" }}/> working days.<div style={{ color:"var(--muted-1)", marginTop:4 }}>Buyer approvals (Fit / Art / S-O / Lab / PP Appr) get no extra buffer of their own, but follow the buffered internal dates. Actual dates stay unchanged.</div></div>)}<div style={{ display:"flex", gap:8, marginTop:10 }}><button onClick={()=>{ runExport(expMode,expBuf,expBuyerPlans,expRelMode,expBuyerBuffer,expBuyerFormat); setExpOpen(false); }} style={{ flex:1, fontFamily:"inherit", fontSize:11, fontWeight:700, padding:6, cursor:"pointer", border:"1px solid var(--ink)", background:"var(--accent)" }}>⬇ Export .xlsx</button><button onClick={()=>setExpOpen(false)} style={{ fontFamily:"inherit", fontSize:11, padding:"6px 10px", cursor:"pointer", border:"1px solid var(--ink)", background:"var(--bg)" }}><X size={12}/></button></div></div>)}</span>
...
Other existing dates in this column will be overwritten.`:`Fill this date into all ${ids.length} visible styles? Existing dates in that column will be overwritten.`):(`Clear this date for all ${ids.length} visible styles? This blanks them — only do this if you mean to.`); if(!window.confirm(_msg)){ setFillOpen(false); return; } try{ supabase.from("audit_log").insert({ style_id:null, style_no:"", col:key, field:mode==="revised"?"bulk revised":"bulk actual", old_val:`${ids.length} visible styles`, new_val:val||"CLEARED", actor_id:me.id, actor_name:me.name||me.email }).then(()=>{}).catch(()=>{}); }catch(e){} if(mode==="revised"){ if(!canEditRev(role)){ setFillOpen(false); return; } styles.filter(s=>idset.has(s.id)).forEach(s=>{ recordRevisionHistory(s,key,val,"bulk fill"); logRevisedEvent(s,key,val); }); ids.forEach(id=>{ const ck=id+":"+key+":revised"; if(val) clearedRef.current.delete(ck); else clearedRef.current.add(ck); }); setStyles(prev=>prev.map(s=>idset.has(s.id)?{...s,revs:{...(s.revs||{}),[key]:val||undefined}}:s)); flash(); setFillOpen(false); return; } if(!canEdit(role,key,"actual")){ setFillOpen(false); return; } if(!top){ ids.forEach(id=>{ if(resendClearIdSet.has(id)) return; const ck=id+":"+key+":actual"; if(val) clearedRef.current.delete(ck); else clearedRef.current.add(ck); }); if(resendClearRows.length){ resendClearRows.forEach(s=>_popResendActual(s,key)); } if(SEND_STAGE_KEYS.includes(key)&&val){ resendRows.forEach(s=>{ const appr=APPR_OF_SEND[key]; const round=activeRejectRound(s,key,appr)||1; const old=(s.actuals&&s.actuals[key])||""; insertStageEvent(s,key,appr,"resend_actual",round,val||null,old||null,val||null,`Bulk re-send ${round} actual entered`); recordResendActual(s,key,val,"bulk fill"); }); } } setStyles(prev=>prev.map(s=>idset.has(s.id)?(top?{...s,[key]:val||""}:(((val&&resendIdSet.has(s.id))||(!val&&resendClearIdSet.has(s.id)))?s:{...s,actuals:{...s.actuals,[key]:val||undefined}})):s)); flash(); setFillOpen(false); }}/>)}</div>
                      ...
                      <div className="mt-stage-cell-body" style={{ minHeight:Math.max(34,rowH-4), padding:"12px 30px 11px 30px", fontSize:11.2, color:cs.actual?"var(--ink)":"var(--muted-6)" }}>
                        ...
                        {cs.autoClosed ? (
                          ...
                          <span style={{ display:"flex", flexDirection:"column", lineHeight:1.2 }}>
                            <span style={{ fontSize:10.5, color:hasRev?"var(--revised)":isNext?"var(--accent)":"#c4c0b8" }}>{hasRev?"rev":"plan"} {fmt(hasRev?cs.rev:cs.plan)}</span>
                            {st.key==="fabricIH" && c.fabricCutoffRef && <span title="Reference only — does not change Fabric IH, downstream plans or risk" style={{ fontSize:8, color:"#a8a39a", whiteSpace:"nowrap" }}>cut-off ref {fmt(c.fabricCutoffRef)}</span>}
                            {editable?<span style={{ fontSize:10.5, color:isNext?"var(--accent)":"#c4c0b8", fontWeight:isNext?700:400 }}>{isNext?"▸ enter":"—"}</span>:<span style={{ fontSize:10, color:"var(--line-2)", display:"flex", alignItems:"center", gap:3 }}><Lock size={9}/>locked</span>}
                          </span>
                        )}
                      </div>
      ...
      {tab==="management" && <ManagementDashboardView computed={activeComputed} todoItems={todoItems} cfg={cfg} stageEvents={stageEvents} role={role} me={me} userMerchantLinks={userMerchantLinks} applyDrill={applyDrill} drillTodo={(obj)=>{ setTodoFilter(todoDrillFilterFromSlice({}, canonicalDrillSpec(obj||{}))); setTab("todo"); }}/>} 
      ...
      {tab==="fabriccolours" && <FabricColourInhouseView
        ...
        onSetOrderColourDate={setFabricOrderColourDate}
        onOpenStyles={(r)=>{ snapCurrent(); resetFilters(); setStatusFilter("All"); if(r.orderNo&&r.colour&&!r.styleNo){ setColFilters({orderNo:[r.orderNo]}); setSearch(r.colour); setSearchCol("colour"); } else { setSearch(r.styleNo||r.colour||""); setSearchCol(r.styleNo?"styleNo":"colour"); } setTrackerActivityFilter(["fabricIH"]); setTab("tracker"); const firstId=r.id||(r.styleIds&&r.styleIds[0]); if(firstId) requestAnimationFrame(()=>setTimeout(()=>scrollToCell(Number(firstId),"fabricIH"),80)); }}
      />}
      {tab==="todo" && <TodoView
        items={todoItems}
        cfg={cfg}
        ...
        role={role}
        me={me}
        userMerchantLinks={userMerchantLinks}
        filter={todoFilter}
      ...
      />}
      {tab==="discipline" && <ManagementDashboardView computed={activeComputed} todoItems={todoItems} cfg={cfg} stageEvents={stageEvents} role={role} me={me} userMerchantLinks={userMerchantLinks} disciplineOnly applyDrill={applyDrill} onDisciplineJump={(id,key)=>{ snapCurrent(); resetFilters(); setTab("tracker"); requestAnimationFrame(()=>setTimeout(()=>jumpToEnter(Number(id),key),60)); }} drillTodo={(obj)=>{ setTodoFilter(todoDrillFilterFromSlice({}, canonicalDrillSpec(obj||{}))); setTab("todo"); }}/>} 
      {tab==="dailyupdates" && <DailyUpdatesView computed={activeComputed} todoItems={todoItems} role={role} me={me} team={team} userMerchantLinks={userMerchantLinks} onJump={(id,key)=>{ snapCurrent(); resetFilters(); setTab("tracker"); requestAnimationFrame(()=>setTimeout(()=>jumpToEnter(Number(id),key),60)); }} onOpenTodo={(merchant)=>{ setTodoFilter(todoDrillFilterFromSlice({}, canonicalDrillSpec({ junior:merchant?[merchant]:[], status:"All" }))); setTab("todo"); }}/>} 
...
function FillPanel({ count, role, activeCol, onApply, onClose }){
  ...
  const stageOpts=STAGES.map(s=>({key:s.key,label:s.label}));
...
/* ========================= FABRIC IH BY COLOUR ========================= */
function FabricColourInhouseView({ computed, focus, canEditActual, canEditRevised, onSetDate, onSetOrderColourDate, onOpenStyles }){
  const [entryMode,setEntryMode]=useState("orderColour");
  const [q,setQ]=useState("");
  ...
  const orderColourRows=useMemo(()=>Object.values(baseRows.reduce((m,r)=>{
    const key=String(r.orderNo||"(blank order)")+"::"+fabricColourToken(r.colour);
    const x=m[key]||(m[key]={ key, orderNo:r.orderNo||"(blank order)", colour:r.colour, rows:[], styleIds:[], styleNos:[], owners:[], fabrics:[], buyers:[] });
    x.rows.push(r); if(!x.styleIds.some(id=>String(id)===String(r.id))) x.styleIds.push(r.id); if(r.styleNo&&!x.styleNos.includes(r.styleNo)) x.styleNos.push(r.styleNo); if(r.owner&&!x.owners.includes(r.owner)) x.owners.push(r.owner); if(r.fabric&&!x.fabrics.includes(r.fabric)) x.fabrics.push(r.fabric); const buyer=r.buyer||r.brand; if(buyer&&!x.buyers.includes(buyer)) x.buyers.push(buyer); return m;
  },{})).map(x=>{
    const actualValues=[...new Set(x.rows.map(r=>r.actual||""))]; const revisedValues=[...new Set(x.rows.map(r=>r.revised||""))];
    const complete=x.rows.filter(r=>r.status==="Complete").length; const pending=x.rows.length-complete;
    const statusName=pending===0?"Complete":x.rows.some(r=>r.status==="Overdue")?"Overdue":x.rows.some(r=>r.status==="Due soon")?"Due soon":"Planned";
    const dues=x.rows.map(r=>r.due).filter(Boolean).sort((a,b)=>a-b); const earliest=dues[0]||null; const latest=dues[dues.length-1]||null;
    const action=statusName==="Complete"?"No chase — fabric received for every matching style":statusName==="Overdue"?`Chase ${x.owners.join(", ")||"merchant"} / Store now; update this order-colour once confirmed`:statusName==="Due soon"?"Confirm mill dispatch and receipt date for this order-colour today":"Monitor this order-colour before the chase window";
    return {...x,styleCount:x.styleIds.length,complete,pending,status:statusName,actual:actualValues.length===1?actualValues[0]:"",revised:revisedValues.length===1?revisedValues[0]:"",mixedActual:actualValues.length>1,mixedRevised:revisedValues.length>1,earliest,latest,action};
  }).sort((a,b)=>{ const rank=x=>x.status==="Overdue"?0:x.status==="Due soon"?1:x.status==="Planned"?2:3; return rank(a)-rank(b)||(a.earliest&&b.earliest?a.earliest-b.earliest:0)||String(a.orderNo).localeCompare(String(b.orderNo))||String(a.colour).localeCompare(String(b.colour)); }),[baseRows]);
  const groupedFiltered=useMemo(()=>orderColourRows.filter(r=>{
    if(status==="pending"&&r.status==="Complete") return false;
    if(status==="overdue"&&r.status!=="Overdue") return false;
    if(status==="dueSoon"&&r.status!=="Due soon") return false;
    if(status==="complete"&&r.status!=="Complete") return false;
    if(owner!=="All"&&!r.owners.includes(owner)) return false;
    if(orderFilter!=="All"&&r.orderNo!==orderFilter) return false;
    if(colourFilter!=="All"&&r.colour!==colourFilter) return false;
    const hay=[r.orderNo,r.colour,...r.styleNos,...r.owners,...r.buyers,...r.fabrics].join(" ").toLowerCase();
    return !q||hay.includes(q.toLowerCase());
  }),[orderColourRows,status,owner,orderFilter,colourFilter,q]);
  ...
  const groupCounts=orderColourRows.reduce((m,r)=>{ m.total++; if(r.status==="Complete")m.complete++; else m.pending++; if(r.status==="Overdue")m.overdue++; if(r.status==="Due soon")m.dueSoon++; return m; },{total:0,pending:0,complete:0,overdue:0,dueSoon:0});
  const displayCounts=entryMode==="orderColour"?groupCounts:counts;
  ...
  const groupExportRows=groupedFiltered.map(r=>({"Order No":r.orderNo,"Colour":r.colour,"Styles Covered":r.styleNos.join(", "),"Style Count":r.styleCount,"Junior / Merchant":r.owners.join(", "),"Buyer / Brand":r.buyers.join(" / "),"Fabric":r.fabrics.join(", "),"Earliest Auto Plan":r.earliest?iso(r.earliest):"","Latest Auto Plan":r.latest?iso(r.latest):"","Revised Fabric IH":r.mixedRevised?"MIXED":r.revised||"","Actual Fabric IH":r.mixedActual?"MIXED":r.actual||"","Styles Complete":`${r.complete}/${r.styleCount}`,"Status":r.status,"Next Chase":r.action}));
  ...
  return <div style={{padding:"16px 22px",maxWidth:"none"}}>
    <div style={{display:"flex",justifyContent:"space-between",gap:16,alignItems:"flex-start",flexWrap:"wrap",marginBottom:12}}><div><div style={{fontFamily:"'Archivo',sans-serif",fontWeight:850,fontSize:23}}>Fabric In‑House by Colour</div><div style={{fontSize:11,color:"var(--muted-2)",marginTop:4,maxWidth:900,lineHeight:1.5}}>Enter Actual or Revised Fabric In‑House separately for every colour. The main Tracker uses the <b>later/latest actual date</b> only after all colours are complete, so PP and Production File do not move early.</div></div><ReportExportMenu title="Fabric In-House by Colour" prefix="fabric-ih-by-colour" sheets={[{label:entryMode==="orderColour"?"Order + Colour View":"Style-wise View",data:entryMode==="orderColour"?groupExportRows:exportRows,modes:["detailed"]}]} defaultMode="detailed"/></div>
    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>{statusBtn("pending",entryMode==="orderColour"?"Pending order-colours":"Pending style-colours",displayCounts.pending,"var(--ink)")}{statusBtn("overdue","Overdue",displayCounts.overdue,"var(--danger)")}{statusBtn("dueSoon","Due ≤7 days",displayCounts.dueSoon,"#8a5200")}{statusBtn("complete","Complete",displayCounts.complete,"var(--success)")}{statusBtn("all",entryMode==="orderColour"?"All order-colours":"All style-colours",displayCounts.total,"var(--info)")}</div>
    <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",padding:"10px 12px",marginBottom:10,border:"1px solid var(--line-2)",borderRadius:10,background:"var(--toolbar-subtle)"}}><span style={{fontSize:10,fontWeight:950,textTransform:"uppercase",color:"var(--muted-3)",marginRight:4}}>Entry mode</span><button onClick={()=>setEntryMode("orderColour")} style={{fontFamily:"inherit",fontSize:10,fontWeight:900,padding:"7px 11px",border:`2px solid ${entryMode==="orderColour"?"var(--ink)":"var(--line-2)"}`,borderRadius:8,background:entryMode==="orderColour"?"var(--ink)":"var(--surface)",color:entryMode==="orderColour"?"var(--bg)":"var(--ink)",cursor:"pointer"}}>Order + Colour Entry</button><button onClick={()=>setEntryMode("style")} style={{fontFamily:"inherit",fontSize:10,fontWeight:900,padding:"7px 11px",border:`2px solid ${entryMode==="style"?"var(--ink)":"var(--line-2)"}`,borderRadius:8,background:entryMode==="style"?"var(--ink)":"var(--surface)",color:entryMode==="style"?"var(--bg)":"var(--ink)",cursor:"pointer"}}>Style-wise Detail</button><span style={{fontSize:9.5,color:"var(--muted-2)"}}>{entryMode==="orderColour"?"One date applies to every style in that exact order + colour.":"Edit a single style without changing the other matching styles."}</span></div>
    ...
    <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginBottom:10}}><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search order / style / colour / fabric" style={{fontFamily:"inherit",fontSize:10,padding:"7px 9px",minWidth:260,border:"1px solid var(--line-2)",borderRadius:8}}/><select value={owner} onChange={e=>setOwner(e.target.value)} style={{fontFamily:"inherit",fontSize:10,padding:"7px 9px",border:"1px solid var(--line-2)",borderRadius:8}}>{owners.map(x=><option key={x}>{x}</option>)}</select><select value={orderFilter} onChange={e=>setOrderFilter(e.target.value)} style={{fontFamily:"inherit",fontSize:10,padding:"7px 9px",border:"1px solid var(--line-2)",borderRadius:8}}>{orders.map(x=><option key={x}>{x}</option>)}</select><select value={colourFilter} onChange={e=>setColourFilter(e.target.value)} style={{fontFamily:"inherit",fontSize:10,padding:"7px 9px",border:"1px solid var(--line-2)",borderRadius:8}}>{colours.map(x=><option key={x}>{x}</option>)}</select><button onClick={()=>{setQ("");setOwner("All");setOrderFilter("All");setColourFilter("All");setStatus("pending");}} style={{fontFamily:"inherit",fontSize:10,fontWeight:800,padding:"7px 10px",border:"1px solid var(--ink)",borderRadius:8,background:"var(--surface)",cursor:"pointer"}}>Clear</button><span style={{fontSize:10,color:"var(--muted-2)"}}>Showing {entryMode==="orderColour"?groupedFiltered.length:filtered.length} {entryMode==="orderColour"?"order-colour groups":"style-colour rows"}</span></div>
    {entryMode==="orderColour"&&<div style={{overflowX:"auto",border:"1px solid var(--line-3)",borderRadius:10,background:"var(--surface)"}}>
      <table style={{width:"100%",minWidth:1320,borderCollapse:"collapse",fontSize:10}}>
        <thead><tr style={{background:"var(--ink)",color:"var(--bg)"}}>{["Status","Order","Colour","Styles covered","Junior / Merchant","Fabric","Auto plan range","Revised Fabric IH","Actual Fabric IH","Completion","Next chase","Action"].map(h=><th key={h} style={{padding:"8px 9px",textAlign:"left",whiteSpace:"nowrap",fontWeight:850}}>{h}</th>)}</tr></thead>
        <tbody>{groupedFiltered.map(r=><tr key={r.key} style={{borderBottom:"1px solid var(--line-3)",background:r.status==="Overdue"?"#fff6f3":"transparent"}}>
          <td style={{padding:"8px 9px",fontWeight:950,color:r.status==="Complete"?"var(--success)":r.status==="Overdue"?"var(--danger)":r.status==="Due soon"?"#8a5200":"var(--muted-3)"}}>{r.status}</td>
          <td style={{padding:"8px 9px",fontWeight:950}}>{r.orderNo}<div style={{fontSize:8.5,color:"var(--muted-2)",fontWeight:650}}>{r.buyers.join(" / ")||"—"}</div></td>
          <td style={{padding:"8px 9px",fontWeight:950}}>{r.colour}</td>
          <td style={{padding:"8px 9px",minWidth:180}}><b>{r.styleCount} style{r.styleCount===1?"":"s"}</b><div title={r.styleNos.join(", ")} style={{fontSize:8.5,color:"var(--muted-2)",maxWidth:230,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.styleNos.join(", ")||"—"}</div></td>
          <td style={{padding:"8px 9px",fontWeight:800}}>{r.owners.join(", ")||"—"}</td>
          <td style={{padding:"8px 9px"}}>{r.fabrics.join(", ")||"—"}</td>
          <td style={{padding:"8px 9px",whiteSpace:"nowrap"}}>{r.earliest?fmt(r.earliest):"—"}{r.latest&&r.earliest&&r.latest.getTime()!==r.earliest.getTime()?` – ${fmt(r.latest)}`:""}</td>
          <td style={{padding:"6px 8px"}}><input type="date" disabled={!canEditRevised} value={r.mixedRevised?"":r.revised||""} onChange={e=>onSetOrderColourDate(r.orderNo,r.colour,"revised",e.target.value)} title={canEditRevised?`Applies revised date to all ${r.styleCount} styles in this exact order + colour`:"Revised date is locked for this role"} style={{fontFamily:"inherit",fontSize:10,padding:"5px 6px",border:"1px solid var(--revised)",borderRadius:7,background:canEditRevised?"var(--surface)":"var(--toolbar-subtle)",color:"var(--revised)"}}/>{r.mixedRevised?<div style={{fontSize:8,color:"var(--danger)",fontWeight:900,marginTop:3}}>Mixed dates · choose to replace all</div>:<div style={{fontSize:8,color:"var(--muted-2)",marginTop:3}}>applies to {r.styleCount} style{r.styleCount===1?"":"s"}</div>}</td>
          <td style={{padding:"6px 8px"}}><input type="date" disabled={!canEditActual} value={r.mixedActual?"":r.actual||""} onChange={e=>onSetOrderColourDate(r.orderNo,r.colour,"actual",e.target.value)} title={canEditActual?`Applies actual date to all ${r.styleCount} styles in this exact order + colour`:"Actual date is locked for this role"} style={{fontFamily:"inherit",fontSize:10,padding:"5px 6px",border:"1px solid var(--success)",borderRadius:7,background:canEditActual?"var(--surface)":"var(--toolbar-subtle)"}}/>{r.mixedActual?<div style={{fontSize:8,color:"var(--danger)",fontWeight:900,marginTop:3}}>Mixed dates · choose to replace all</div>:<div style={{fontSize:8,color:"var(--muted-2)",marginTop:3}}>applies to {r.styleCount} style{r.styleCount===1?"":"s"}</div>}</td>
          <td style={{padding:"8px 9px",fontWeight:900}}>{r.complete}/{r.styleCount} styles</td>
          <td style={{padding:"8px 9px",minWidth:220,color:r.status==="Overdue"?"var(--danger)":"var(--muted-3)",fontWeight:r.status==="Overdue"?850:650}}>{r.action}</td>
          <td style={{padding:"8px 9px"}}><button onClick={()=>onOpenStyles(r)} style={{fontFamily:"inherit",fontSize:9.5,fontWeight:900,padding:"5px 8px",cursor:"pointer",border:"1px solid var(--ink)",borderRadius:7,background:"var(--surface)",whiteSpace:"nowrap"}}>Open styles ({r.styleCount}) →</button></td>
        </tr>)}{!groupedFiltered.length&&<tr><td colSpan={12} style={{padding:18,textAlign:"center",color:"var(--muted-2)"}}>No order-colour groups match this view.</td></tr>}</tbody>
      </table>
    </div>}
    {entryMode==="style"&&<div style={{overflowX:"auto",border:"1px solid var(--line-3)",borderRadius:10,background:"var(--surface)"}}>
      <table style={{width:"100%",minWidth:1450,borderCollapse:"collapse",fontSize:10}}>
        <thead><tr style={{background:"var(--ink)",color:"var(--bg)"}}>{["Status","Order / Style","Junior / Merchant","Fabric","Colour","Auto Plan","Revised Fabric IH","Actual Fabric IH","Style completion","Main Tracker date","Next chase","Action"].map(h=><th key={h} style={{padding:"8px 9px",textAlign:"left",whiteSpace:"nowrap",fontWeight:850}}>{h}</th>)}</tr></thead>
        <tbody>{filtered.map(r=><tr key={`${r.id}:${fabricColourToken(r.colour)}`} style={{borderBottom:"1px solid var(--line-3)",background:r.status==="Overdue"?"#fff6f3":"transparent"}}><td style={{padding:"8px 9px",fontWeight:950,color:r.status==="Complete"?"var(--success)":r.status==="Overdue"?"var(--danger)":r.status==="Due soon"?"#8a5200":"var(--muted-3)"}}>{r.status}{r.status==="Overdue"?<div style={{fontSize:8.5}}>+{Math.abs(Number(r.days)||0)} wd</div>:null}</td><td style={{padding:"8px 9px"}}><b>{r.styleNo}</b><div style={{fontSize:8.5,color:"var(--muted-2)"}}>{r.orderNo} · {r.buyer||r.brand}</div></td><td style={{padding:"8px 9px",fontWeight:800}}>{r.owner||"—"}</td><td style={{padding:"8px 9px"}}>{r.fabric||"—"}</td><td style={{padding:"8px 9px",fontWeight:950}}>{r.colour}</td><td style={{padding:"8px 9px",whiteSpace:"nowrap"}}>{r.plan?fmt(r.plan):"—"}</td><td style={{padding:"6px 8px"}}><input type="date" disabled={!canEditRevised} value={r.revised||""} onChange={e=>onSetDate(r.id,r.colour,"revised",e.target.value)} title={canEditRevised?"Editable revised commitment for this colour":"Revised date is locked for this role"} style={{fontFamily:"inherit",fontSize:10,padding:"5px 6px",border:"1px solid var(--revised)",borderRadius:7,background:canEditRevised?"var(--surface)":"var(--toolbar-subtle)",color:"var(--revised)"}}/></td><td style={{padding:"6px 8px"}}><input type="date" disabled={!canEditActual} value={r.actual||""} onChange={e=>onSetDate(r.id,r.colour,"actual",e.target.value)} title={canEditActual?"Editable actual Fabric In-House for this colour":"Actual date is locked for this role"} style={{fontFamily:"inherit",fontSize:10,padding:"5px 6px",border:"1px solid var(--success)",borderRadius:7,background:canEditActual?"var(--surface)":"var(--toolbar-subtle)"}}/></td><td style={{padding:"8px 9px",fontWeight:850}}>{r.completeCount}/{r.colourCount} colours</td><td style={{padding:"8px 9px",fontWeight:900,color:r.styleLatest?"var(--success)":"var(--muted-2)"}}>{r.styleLatest?fmt(r.styleLatest):"Wait for all colours"}</td><td style={{padding:"8px 9px",minWidth:220,color:r.status==="Overdue"?"var(--danger)":"var(--muted-3)",fontWeight:r.status==="Overdue"?850:650}}>{r.action}</td><td style={{padding:"8px 9px"}}><button onClick={()=>onOpenStyles(r)} style={{fontFamily:"inherit",fontSize:9.5,fontWeight:900,padding:"5px 8px",cursor:"pointer",border:"1px solid var(--ink)",borderRadius:7,background:"var(--surface)"}}>Open style →</button></td></tr>)}{!filtered.length&&<tr><td colSpan={12} style={{padding:18,textAlign:"center",color:"var(--muted-2)"}}>No colour rows match this view.</td></tr>}</tbody>
      </table>
    </div>}
...
/* ========================= DAILY UPDATES ========================= */
function DailyUpdatesView({ computed, todoItems, role, me, team, userMerchantLinks={}, onJump, onOpenTodo }){
  const SUBMISSION_SETTING_ID="daily_update_submissions_v1";
  const MORNING_SETTING_PREFIX="daily_morning_8am_v1::";
  ...
  const [selectedUser,setSelectedUser]=useState(role==="junior"?"mine":"All");
  const [otherWork,setOtherWork]=useState("");
  const norm=(v)=>String(v||"").trim().toLowerCase().replace(/[^a-z0-9]+/g,"");
  const profileKey=(p)=>p&&p.id?`id:${p.id}`:`name:${norm(p&&(p.name||p.email))}`;
  const actorKey=(r)=>r&&(r.actor_id||r.author_id)?`id:${r.actor_id||r.author_id}`:`name:${norm(r&&(r.actor_name||r.author_name))}`;
  const myKey=profileKey(me);
  const merchantProfiles=useMemo(()=>{
    const list=(team||[]).filter(p=>String(p.role||"").toLowerCase()==="junior");
    if(me&&role==="junior"&&!list.some(p=>profileKey(p)===myKey)) list.push(me);
    return list.sort((a,b)=>String(a.name||a.email||"").localeCompare(String(b.name||b.email||"")));
  },[team,me,role]);
  const profileByKey=useMemo(()=>new Map(merchantProfiles.map(p=>[profileKey(p),p])),[merchantProfiles]);
  const displayName=(key,fallback="")=>{ const p=profileByKey.get(key); return (p&&(p.name||p.email))||fallback||key.replace(/^name:/,""); };
  const selectedKey=role==="junior"?myKey:(selectedUser==="mine"?myKey:selectedUser);
  const linkedMerchantForKey=(key)=>{ const p=profileByKey.get(key); return String((p&&userMerchantLinks[p.id])||"").trim(); };
  const unlinkedProfiles=merchantProfiles.filter(p=>!String(userMerchantLinks[p.id]||"").trim());
  const dayRange=()=>{ const start=new Date(`${date}T00:00:00+05:30`); const graceEnd=nextWorkingNoon(parse(date)); return { start:start.toISOString(), end:new Date((graceEnd?graceEnd.getTime():start.getTime()+ONE_DAY)+1000).toISOString() }; };
  const loadDay=async()=>{
    if(!date) return;
    setBusy(true);
    try{
      const {start,end}=dayRange();
      const fetchPaged=async(table)=>{ let out=[]; for(let from=0;from<10000;from+=1000){ const {data,error}=await supabase.from(table).select("*").gte("created_at",start).lt("created_at",end).order("created_at",{ascending:true}).range(from,from+999); if(error) throw error; out=out.concat(data||[]); if(!data||data.length<1000) break; } return out; };
      const [a,c,sr,mr]=await Promise.all([fetchPaged("audit_log"),fetchPaged("comments"),supabase.from("app_settings").select("data").eq("id",SUBMISSION_SETTING_ID).maybeSingle(),supabase.from("app_settings").select("data").eq("id",MORNING_SETTING_PREFIX+date).maybeSingle()]);
      setAudit(a||[]); setDayComments(c||[]); const sd=sr&&sr.data&&sr.data.data; setSubmissions(sd&&typeof sd==="object"?(sd.items||sd):{}); setMorningSnapshot(mr&&mr.data&&mr.data.data||null);
    }catch(e){ setAudit([]); setDayComments([]); setMorningSnapshot(null); }
    setBusy(false);
  };
  useEffect(()=>{ loadDay(); },[date]);
  useEffect(()=>{ if(role==="junior") setSelectedUser("mine"); },[role,myKey]);
  const pairById=useMemo(()=>new Map((computed||[]).map(x=>[String(x.s.id),x])),[computed]);
  const stageContext=(styleId,col)=>{
    const pair=pairById.get(String(styleId)); if(!pair) return {pair:null,key:col,label:stageLabelFromKeyGlobal(col)||col,plan:null,revised:null};
    const colourToken=parseFabricColourStageKey(col); const key=colourToken?"fabricIH":col; const st=(pair.c.stages||[]).find(x=>x.key===key); let revised=st&&st.rev;
    if(colourToken){ const rec=fabricColourMapOf(pair.s)[colourToken]||{}; revised=parse(rec.revised)||null; }
    const colourLabel=colourToken?(fabricColourRows(pair.s,st&&st.plan).find(x=>fabricColourToken(x.colour)===colourToken)||{}).colour:"";
    return {pair,key,label:(stageLabelFromKeyGlobal(key)||key)+(colourLabel?` · ${colourLabel}`:""),plan:st&&st.plan,revised};
  };
  const istTime=(day,hour,minute=0)=>new Date(`${day}T${String(hour).padStart(2,"0")}:${String(minute).padStart(2,"0")}:00+05:30`);
  const istDateISO=(value)=>{ const parts=new Intl.DateTimeFormat("en-GB",{timeZone:"Asia/Kolkata",year:"numeric",month:"2-digit",day:"2-digit"}).formatToParts(new Date(value)); const get=t=>(parts.find(p=>p.type===t)||{}).value||""; return `${get("year")}-${get("month")}-${get("day")}`; };
  const morningTime=istTime(date,8,0);
  const afterMorning=(at)=>{ const d=new Date(at||0); return Number.isFinite(d.getTime())&&d>=morningTime; };
  const nextWorkingNoon=(due)=>{ const d=parse(due); if(!d) return null; const next=addWorkdays(d,1); return istTime(iso(next),12,0); };
  const entryTiming=(due,at)=>{
    const d=parse(due), entered=new Date(at||0), grace=nextWorkingNoon(d);
    if(!d||!Number.isFinite(entered.getTime())||!grace) return {late:false,days:0,label:"No due-date comparison"};
    if(entered<=grace) return {late:false,days:0,label:entered>istTime(iso(d),23,59)?"Next working-day grace":"On time"};
    const entryDay=new Date(istDateISO(entered)+"T00:00:00+05:30");
    const days=Math.max(1,netWorkdays(d,entryDay)||1);
    return {late:true,days,label:`+${days} working day${days===1?"":"s"} late`};
  };
  const netRows=useMemo(()=>{
    const groups=new Map();
    (audit||[]).filter(r=>r&&r.style_id&&r.col).forEach(r=>{ const field=String(r.field||"value").toLowerCase(); if(["format fill","format note","bulk action","upload"].includes(field)) return; const key=[r.style_id,r.col,field].join("::"); const g=groups.get(key)||{key,rows:[]}; g.rows.push(r); groups.set(key,g); });
    const out=[];
    groups.forEach(g=>{
      const sorted=g.rows.sort((a,b)=>new Date(a.created_at)-new Date(b.created_at)); const effective=[];
      sorted.forEach(r=>{ const p=effective[effective.length-1]; const duplicate=p&&actorKey(p)===actorKey(r)&&String(p.old_val||"")===String(r.old_val||"")&&String(p.new_val||"")===String(r.new_val||"")&&Math.abs(new Date(r.created_at)-new Date(p.created_at))<20000; if(!duplicate) effective.push(r); });
      const first=effective[0],last=effective[effective.length-1]; if(!first||!last) return;
      const field=String(last.field||"value").toLowerCase(); const previous=String(first.old_val||""); const finalValue=String(last.new_val||"");
      const kind=field.includes("actual")?"actual":field.includes("revised")?"revised":field.includes("reject")?"rejected":field.includes("waiv")||field.includes("skip")?"waived":"other";
      const ctx=stageContext(last.style_id,last.col); const pair=ctx.pair; const expected=ctx.revised||ctx.plan||null; const finalDate=parse(finalValue); let result="Updated"; let movedLater=false; let variance=null; let expectedValue=""; let expectedSource="";
      if(kind==="actual"){ expectedValue=expected?iso(expected):""; expectedSource=ctx.revised?"Revised":"Auto Plan"; variance=expected&&finalDate?netWorkdays(expected,finalDate):null; result=variance==null?"Completed":variance>0?`${variance} working day${variance===1?"":"s"} late`:variance<0?`${Math.abs(variance)} working day${variance===-1?"":"s"} early`:"On commitment date"; }
      if(kind==="revised"){ const priorDate=parse(previous)||ctx.plan||null; expectedValue=priorDate?iso(priorDate):""; expectedSource=previous?"Previous Revised":"Auto Plan"; variance=priorDate&&finalDate?netWorkdays(priorDate,finalDate):null; movedLater=variance>0; result=variance>0?`Commitment moved ${variance} working day${variance===1?"":"s"} later`:variance<0?`Commitment improved ${Math.abs(variance)} working day${variance===-1?"":"s"}`:"Commitment updated"; }
      const meaningful=!!finalValue&&finalValue!==previous; const p=pair&&pair.s;
      const timing=entryTiming(expectedValue||(expected?iso(expected):""),last.created_at);
      out.push({ key:g.key, styleId:last.style_id, styleNo:(p&&p.styleNo)||last.style_no||last.style_id, orderNo:(p&&p.orderNo)||"", colour:(p&&p.colour)||"", buyer:(p&&(p.buyer||p.brand))||"", stageKey:ctx.key, rawCol:last.col, activity:ctx.label, field, kind, previous, finalValue, expectedValue, expectedSource, dueBefore:expectedValue||(expected?iso(expected):""), result, variance, movedLater, meaningful, entryLate:timing.late, entryLateDays:timing.days, entryTimingLabel:timing.label, correction:!meaningful||effective.length>1, editCount:effective.length, actorKey:actorKey(last), actorName:last.actor_name||displayName(actorKey(last)), at:last.created_at });
    });
    const actualActivities=new Set(out.filter(r=>r.meaningful&&r.kind==="actual").map(r=>`${r.styleId}::${r.rawCol}`));
    return out.filter(r=>!(r.kind==="revised"&&actualActivities.has(`${r.styleId}::${r.rawCol}`))).sort((a,b)=>new Date(b.at)-new Date(a.at));
  },[audit,pairById]);
  const atomicMeaningfulRows=netRows.filter(r=>r.meaningful&&["actual","revised","rejected","waived"].includes(r.kind));
  const meaningfulRows=useMemo(()=>{
    const grouped=new Map();
    atomicMeaningfulRows.forEach(r=>{
      const isOrderColour=String(r.rawCol||"").startsWith(FABRIC_COLOUR_STAGE_PREFIX);
      const key=isOrderColour?[r.actorKey,r.orderNo,r.rawCol,r.kind,r.finalValue].join("::"):r.key;
      const x=grouped.get(key)||{...r,key,styleIds:[],styleNos:[],styleCount:0};
      if(!x.styleIds.some(id=>String(id)===String(r.styleId))) x.styleIds.push(r.styleId);
      if(r.styleNo&&!x.styleNos.includes(r.styleNo)) x.styleNos.push(r.styleNo);
      x.styleCount=x.styleIds.length; if(new Date(r.at)>new Date(x.at)) x.at=r.at;
      grouped.set(key,x);
    });
    return [...grouped.values()].map(r=>({...r,styleId:r.styleIds[0]||r.styleId,styleNo:r.styleCount>1?r.styleNos.join(", "):r.styleNo,result:r.styleCount>1?`${r.result} · ${r.styleCount} styles covered`:r.result})).sort((a,b)=>new Date(b.at)-new Date(a.at));
  },[netRows]);
  const reportGraceEnd=nextWorkingNoon(parse(date));
  const belongsToReport=(r)=>{ if(!afterMorning(r&&r.at)||!reportGraceEnd||new Date(r.at)>reportGraceEnd) return false; if(istDateISO(r.at)===date) return true; const due=parse(r.expectedValue||r.dueBefore); return !!(due&&due<=parse(date)); };
  const correctionRows=netRows.filter(r=>r.correction&&belongsToReport(r));
  const commentRows=(dayComments||[]).map(c=>{ const pair=pairById.get(String(c.style_id)); return {...c,actorKey:actorKey(c),styleNo:(pair&&pair.s.styleNo)||c.style_id,orderNo:(pair&&pair.s.orderNo)||"",activity:stageLabelFromKeyGlobal(c.col)||c.col}; });
  const reportRows=meaningfulRows.filter(belongsToReport);
  const reportComments=commentRows.filter(c=>afterMorning(c.created_at)&&istDateISO(c.created_at)===date);
  const taskMerchantKey=(t)=>{ const names=[]; if(t&&t.junior) names.push(t.junior); if(t&&Array.isArray(t.juniors)) names.push(...t.juniors); const exactLinked=merchantProfiles.find(p=>{ const linked=norm(userMerchantLinks[p.id]); return linked&&names.some(name=>norm(name)===linked); }); if(exactLinked) return profileKey(exactLinked); for(const name of names){ const n=norm(name); const p=merchantProfiles.find(x=>{ const a=norm(x.name),b=norm(x.email),c=norm(String(x.email||"").split("@")[0]); return n&&(n===a||n===b||n===c||(n.length>=4&&(a.includes(n)||n.includes(a)))); }); if(p) return profileKey(p); } return names[0]?`name:${norm(names[0])}`:""; };
  const todaySelected=date===iso(TODAY);
  const taskDue=(t)=>parse(t&&t.dueUsed)||parse(t&&t.exp)||parse(t&&t.plan)||null;
  const styleIdsOf=(x)=>[...new Set((Array.isArray(x&&x.styleIds)&&x.styleIds.length?x.styleIds:[x&&(x.id||x.styleId)]).filter(v=>v!=null&&String(v)!=="").map(String))].sort();
  const taskIdentity=(t)=>{ const stage=String(t&&t.rawCol||t&&t._stageKey||t&&t.activityKey||t&&t.key||""); if(t&&t.isColour&&(stage==="fabricIH"||stage.startsWith(FABRIC_COLOUR_STAGE_PREFIX))) return `colour::${norm(t.orderNo)}::${stage.startsWith(FABRIC_COLOUR_STAGE_PREFIX)?stage:fabricColourStageKey(t.colour||t.styleNo||"")}`; return `${styleIdsOf(t).join(",")}::${stage}`; };
  const rowIdentities=(r)=>{ const raw=String(r&&r.rawCol||r&&r.stageKey||""); if(raw.startsWith(FABRIC_COLOUR_STAGE_PREFIX)) return [`colour::${norm(r.orderNo)}::${raw}`,...styleIdsOf(r).map(id=>`${id}::${raw}`)]; return [taskIdentity(r),...styleIdsOf(r).map(id=>`${id}::${raw}`)]; };
  const commentIdentities=(c)=>{ const raw=String(c&&c.col||""); return [`${String(c&&c.style_id||"")}::${raw}`]; };
  const serializeTask=(t)=>({ identity:taskIdentity(t), styleIds:styleIdsOf(t), id:t.id||null, orderNo:t.orderNo||"", styleNo:t.isColour?(t.styleNos||[]).join(", "):(t.styleNo||""), colour:t.isColour?(t.colour||t.styleNo||""):(t.colour||""), activity:t._activityLabel||t.activity||stageLabelFromKeyGlobal(t._stageKey||t.activityKey||t.key), stageKey:t._stageKey||t.activityKey||t.key||"", rawCol:t.rawCol||t._stageKey||t.activityKey||t.key||"", due:taskDue(t)?iso(taskDue(t)):"", merchantKey:taskMerchantKey(t), trackerMerchant:String(t.junior||((t.juniors||[])[0])||""), isColour:!!t.isColour });
  const taskFromRow=(r)=>{ const pair=pairById.get(String(r.styleId)); const owner=pair&&pair.s&&pair.s.owner||""; return serializeTask({id:r.styleId,styleIds:r.styleIds,orderNo:r.orderNo,styleNo:r.styleNo,colour:r.colour,activity:r.activity,activityKey:r.rawCol||r.stageKey,key:r.rawCol||r.stageKey,rawCol:r.rawCol||r.stageKey,exp:parse(r.expectedValue||r.dueBefore),junior:owner,isColour:String(r.rawCol||"").startsWith(FABRIC_COLOUR_STAGE_PREFIX)}); };
  const reconstructedMorningTasks=useMemo(()=>{
    if(!todaySelected||new Date()<morningTime) return [];
    const dueNow=(todoItems||[]).filter(t=>{ const d=taskDue(t); return !!(d&&d<=TODAY); }).map(serializeTask);
    const acted=reportRows.filter(r=>{ const d=parse(r.expectedValue||r.dueBefore); return !!(d&&d<=parse(date)); }).map(taskFromRow);
    const byId=new Map(); [...dueNow,...acted].forEach(t=>{ if(t.identity&&!byId.has(t.identity)) byId.set(t.identity,t); });
    return [...byId.values()];
  },[todaySelected,todoItems,reportRows,date]);
  useEffect(()=>{
    if(!todaySelected||busy||snapshotBusy||morningSnapshot||new Date()<morningTime||morningTime.getDay()===0) return;
    let live=true; setSnapshotBusy(true);
    (async()=>{ try{ const payload={date,capturedFor:"08:00 IST",capturedAt:new Date().toISOString(),reconstructedFromAudit:true,tasks:reconstructedMorningTasks}; const latest=await supabase.from("app_settings").select("data").eq("id",MORNING_SETTING_PREFIX+date).maybeSingle(); const existing=latest&&latest.data&&latest.data.data; if(live&&existing) setMorningSnapshot(existing); else { const {error}=await supabase.from("app_settings").upsert({id:MORNING_SETTING_PREFIX+date,data:payload}); if(error) throw error; if(live) setMorningSnapshot(payload); } }catch(e){ /* keep live fallback visible; retry on Refresh */ } if(live) setSnapshotBusy(false); })();
    return ()=>{ live=false; };
  },[todaySelected,busy,morningSnapshot,date,reconstructedMorningTasks.length]);
  const morningTasks=Array.isArray(morningSnapshot&&morningSnapshot.tasks)?morningSnapshot.tasks:(todaySelected?reconstructedMorningTasks:[]);
  const baselineLookup=useMemo(()=>{ const m=new Map(); morningTasks.forEach(t=>{ m.set(t.identity,t); (t.styleIds||[]).forEach(id=>m.set(`${id}::${t.rawCol||t.stageKey}`,t)); }); return m; },[morningTasks]);
  const baselineForRow=(r)=>rowIdentities(r).map(id=>baselineLookup.get(id)).find(Boolean)||null;
  const baselineForComment=(c)=>commentIdentities(c).map(id=>baselineLookup.get(id)).find(Boolean)||null;
  const attributedRows=reportRows.map(r=>{ const base=baselineForRow(r); return {...r,reportMerchantKey:(base&&base.merchantKey)||r.actorKey,morningIdentity:base&&base.identity||""}; });
  const attributedComments=reportComments.map(c=>{ const base=baselineForComment(c); return {...c,reportMerchantKey:(base&&base.merchantKey)||c.actorKey,morningIdentity:base&&base.identity||""}; });
  const actionRowsByMorning=useMemo(()=>{ const m=new Map(); attributedRows.forEach(r=>{ if(!r.morningIdentity) return; const arr=m.get(r.morningIdentity)||[]; arr.push(r); m.set(r.morningIdentity,arr); }); return m; },[attributedRows]);
  const commentsByMorning=useMemo(()=>{ const m=new Map(); attributedComments.forEach(c=>{ if(!c.morningIdentity) return; const arr=m.get(c.morningIdentity)||[]; arr.push(c); m.set(c.morningIdentity,arr); }); return m; },[attributedComments]);
  const outcomeOf=(t)=>{ const rows=actionRowsByMorning.get(t.identity)||[]; if(rows.some(r=>r.kind==="actual")) return "actual"; if(rows.some(r=>r.kind==="revised")) return "revised"; if((commentsByMorning.get(t.identity)||[]).length) return "chased"; return "untouched"; };
  const submissionFor=(key)=>submissions[`${date}::${key}`]||null;
  const lastWorkAt=(key)=>{ const times=[...attributedRows.filter(r=>r.reportMerchantKey===key).map(r=>new Date(r.at).getTime()),...attributedComments.filter(r=>r.reportMerchantKey===key).map(r=>new Date(r.created_at).getTime())].filter(Number.isFinite); return times.length?Math.max(...times):0; };
  const userKeys=useMemo(()=>{ const s=new Set(merchantProfiles.map(profileKey)); morningTasks.forEach(t=>t.merchantKey&&s.add(t.merchantKey)); attributedRows.forEach(r=>s.add(r.reportMerchantKey)); attributedComments.forEach(r=>s.add(r.reportMerchantKey)); return [...s].filter(Boolean); },[merchantProfiles,morningTasks,attributedRows.length,attributedComments.length]);
  const summaryFor=(key)=>{
    const rows=attributedRows.filter(r=>r.reportMerchantKey===key), comments=attributedComments.filter(r=>r.reportMerchantKey===key), corrections=correctionRows.filter(r=>r.actorKey===key), requiredTasks=morningTasks.filter(t=>t.merchantKey===key), outcomes=requiredTasks.map(t=>outcomeOf(t)), sub=submissionFor(key), last=lastWorkAt(key);
    const extraIds=new Set(); rows.filter(r=>!r.morningIdentity).forEach(r=>extraIds.add(rowIdentities(r)[0])); comments.filter(c=>!c.morningIdentity).forEach(c=>extraIds.add(commentIdentities(c)[0]));
    const actualRows=rows.filter(r=>r.kind==="actual"), revisedRows=rows.filter(r=>r.kind==="revised");
    return { key,name:displayName(key,rows[0]&&rows[0].actorName),trackerMerchant:linkedMerchantForKey(key)||((requiredTasks[0]&&requiredTasks[0].trackerMerchant)||""),required:requiredTasks.length,handled:outcomes.filter(x=>x!=="untouched").length,actual:actualRows.length,actualLate:actualRows.filter(r=>r.entryLate).length,revised:revisedRows.length,revisedLate:revisedRows.filter(r=>r.entryLate).length,movedLater:revisedRows.filter(r=>r.movedLater).length,chased:outcomes.filter(x=>x==="chased").length,additional:extraIds.size,rejected:rows.filter(r=>r.kind==="rejected").length,waived:rows.filter(r=>r.kind==="waived").length,meaningful:rows.length,comments:comments.length,corrections:corrections.length,untouched:outcomes.filter(x=>x==="untouched").length,submission:sub,changedAfter:!!(sub&&last>new Date(sub.submittedAt||0).getTime())};
  };
  const summaries=userKeys.map(summaryFor).sort((a,b)=>Number(!b.submission)-Number(!a.submission)||b.untouched-a.untouched||String(a.name).localeCompare(String(b.name)));
  const visibleRows=selectedKey==="All"?attributedRows:attributedRows.filter(r=>r.reportMerchantKey===selectedKey);
  const visibleComments=selectedKey==="All"?attributedComments:attributedComments.filter(r=>r.reportMerchantKey===selectedKey);
  const visibleCorrections=selectedKey==="All"?correctionRows:correctionRows.filter(r=>r.actorKey===selectedKey);
  const visibleUntouched=morningTasks.filter(t=>(selectedKey==="All"||t.merchantKey===selectedKey)&&outcomeOf(t)==="untouched");
  const emptySummary={required:0,handled:0,actual:0,actualLate:0,revised:0,revisedLate:0,movedLater:0,chased:0,additional:0,meaningful:0,comments:0,corrections:0,untouched:0};
  const visibleSummary=selectedKey==="All"?summaries.reduce((m,r)=>{ Object.keys(emptySummary).forEach(k=>m[k]+=(Number(r[k])||0)); return m; },{...emptySummary}):(summaries.find(r=>r.key===selectedKey)||emptySummary);
  const mySubmission=submissionFor(myKey);
  useEffect(()=>{ const s=submissionFor(myKey); setOtherWork((s&&s.otherWork)||""); },[date,submissions,myKey]);
  const submitMyDay=async()=>{
    const linked=linkedMerchantForKey(myKey); if(role==="junior"&&!linked){ alert("Your login must be linked to the exact Tracker merchant in Management → Users before submitting."); return; }
    const myRows=attributedRows.filter(r=>r.reportMerchantKey===myKey); const mySummary=summaryFor(myKey);
    setSubmitBusy(true);
    try{
      const latestRes=await supabase.from("app_settings").select("data").eq("id",SUBMISSION_SETTING_ID).maybeSingle(); const raw=latestRes&&latestRes.data&&latestRes.data.data; const items={...((raw&&raw.items)||raw||{})};
      const cutoff=Date.now()-120*ONE_DAY; Object.keys(items).forEach(k=>{ const d=parse(String(k).slice(0,10)); if(d&&d.getTime()<cutoff) delete items[k]; });
      const entry={ date,userId:me&&me.id||null,userName:(me&&(me.name||me.email))||"",trackerMerchant:linked,submittedAt:new Date().toISOString(),otherWork:otherWork.trim(),morningSnapshotCapturedAt:morningSnapshot&&morningSnapshot.capturedAt||"",snapshot:{required:mySummary.required,handledRequired:mySummary.handled,meaningful:myRows.length,actual:mySummary.actual,actualLate:mySummary.actualLate,revised:mySummary.revised,revisedLate:mySummary.revisedLate,chasedOnly:mySummary.chased,additionalAfter8:mySummary.additional,comments:mySummary.comments,corrections:mySummary.corrections,untouched:mySummary.untouched}};
      items[`${date}::${myKey}`]=entry; const {error}=await supabase.from("app_settings").upsert({id:SUBMISSION_SETTING_ID,data:{items}}); if(error) throw error; setSubmissions(items);
    }catch(e){ alert("Daily update submission failed: "+(e.message||e)); }
    setSubmitBusy(false);
  };
  const card=(label,value,color,sub)=><div style={{background:"var(--surface)",border:"1px solid var(--line-2)",borderRadius:12,padding:"11px 13px",minWidth:145,flex:"1 1 145px"}}><div style={{fontSize:9,fontWeight:950,textTransform:"uppercase",color:"var(--muted-2)"}}>{label}</div><div style={{fontSize:26,fontWeight:950,color:color||"var(--ink)",lineHeight:1.1,marginTop:3}}>{value}</div>{sub&&<div style={{fontSize:8.5,color:"var(--muted-2)",marginTop:3}}>{sub}</div>}</div>;
  const inputStyle={fontFamily:"inherit",fontSize:10,padding:"7px 9px",border:"1px solid var(--line-2)",borderRadius:8,background:"var(--surface)"};
  const summaryData=summaries.map(r=>({"Merchant Login":r.name,"Linked Tracker Merchant":r.trackerMerchant||"NOT LINKED","8 AM Workload":r.required,"Workload Handled":r.handled,"Actual Completed":r.actual,"Actual Late":r.actualLate,"Net Revised":r.revised,"Revised Late":r.revisedLate,"Chased Only":r.chased,"Untouched":r.untouched,"Additional After 8 AM":r.additional,"Submitted":r.submission?(r.changedAfter?"Changes after submission":"Yes"):"No","Corrections / Repeated Edits":r.corrections,"Submitted At":r.submission?new Date(r.submission.submittedAt).toLocaleString():"","Other Work Note":r.submission&&r.submission.otherWork||""}));
  const detailData=visibleRows.map(r=>({"Merchant":r.trackerMerchant||displayName(r.reportMerchantKey,r.actorName),"Entered By":r.actorName,"Order No":r.orderNo,"Style No":r.styleNo,"Colour":r.colour,"Buyer / Brand":r.buyer,"Activity":r.activity,"Update Type":r.kind==="actual"?"Actual completed":r.kind==="revised"?"Net revised commitment":r.kind,"Expected Date Used":r.expectedValue||r.dueBefore,"Expected Basis":r.expectedSource,"Final Value":r.finalValue,"Entry Timing":r.entryTimingLabel,"Late Working Days":r.entryLateDays||0,"Morning Task":r.morningIdentity?"Yes":"No — additional","Result":r.result,"Edits Collapsed":r.editCount,"Final Update Time":new Date(r.at).toLocaleString("en-IN",{timeZone:"Asia/Kolkata"})}));
  const dueData=visibleUntouched.map(t=>({"Merchant":t.trackerMerchant||displayName(t.merchantKey,""),"Order No":t.orderNo||"","Style / Colour":t.isColour?t.colour||"":t.styleNo||"","Activity":t.activity||stageLabelFromKeyGlobal(t.stageKey),"8 AM Due":t.due||"","Outcome":"Untouched"}));
  return <div style={{padding:"18px 22px 36px"}}>
    <div style={{display:"flex",justifyContent:"space-between",gap:14,alignItems:"flex-start",flexWrap:"wrap",marginBottom:12}}><div><div style={{fontFamily:"'Archivo',sans-serif",fontWeight:850,fontSize:23}}>Daily Updates</div><div style={{fontSize:11,color:"var(--muted-2)",marginTop:4,maxWidth:900,lineHeight:1.5}}>Merchant-wise end-of-day report against the fixed 8:00 AM IST Today &amp; Behind workload. Actual, Revised, Chased only and Untouched are exclusive outcomes; repeated edits collapse to one final/net update.</div></div><ReportExportMenu title="Merchant Daily Updates" prefix="merchant-daily-updates" sheets={[{label:"Merchant Summary",data:summaryData,modes:["summary","detailed"],detailData},{label:"Update Detail",data:detailData,modes:["detailed"]},{label:"8 AM Untouched",data:dueData,modes:["detailed"]}]} defaultMode="detailed"/></div>
    <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",padding:"10px 12px",background:"var(--toolbar-subtle)",border:"1px solid var(--line-2)",borderRadius:10,marginBottom:10}}><label style={{fontSize:10,fontWeight:900}}>Report date <input type="date" value={date} max={iso(TODAY)} onChange={e=>setDate(e.target.value)} style={{...inputStyle,marginLeft:5}}/></label>{role!=="junior"&&<select value={selectedUser} onChange={e=>setSelectedUser(e.target.value)} style={inputStyle}><option value="All">Merchant: all</option>{merchantProfiles.map(p=><option key={profileKey(p)} value={profileKey(p)}>{userMerchantLinks[p.id]||"NOT LINKED"} · {p.name||p.email}</option>)}</select>}<button onClick={loadDay} disabled={busy||snapshotBusy} style={{...inputStyle,fontWeight:900,cursor:"pointer"}}>{busy||snapshotBusy?"Loading…":"Refresh"}</button><span style={{fontSize:9,fontWeight:850,color:morningSnapshot?"var(--success)":todaySelected?"#b45309":"var(--danger)"}}>{morningSnapshot?`8 AM workload fixed · ${morningTasks.length} tasks`:todaySelected?"Preparing fixed 8 AM workload…":"No saved 8 AM snapshot for this date"}</span><span style={{fontSize:9,color:"var(--muted-2)"}}>Next-working-day entry grace ends at 12:00 PM IST · Sundays excluded</span></div>
    {unlinkedProfiles.length>0&&<div style={{padding:"9px 11px",border:"1px solid var(--danger)",borderRadius:9,background:"#fff4f1",color:"var(--danger)",fontSize:10,fontWeight:850,marginBottom:10}}>{role==="junior"?"Your login is not linked to a Tracker Junior Merchant. Ask Management to link it in Users.":`${unlinkedProfiles.length} merchant login${unlinkedProfiles.length===1?" is":"s are"} not linked. Open Users → Linked tracker merchant.`}</div>}
    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>{card("8 AM workload",visibleSummary.required,"var(--ink)","due today + all older pending")}{card("Workload handled",visibleSummary.handled,"var(--success)","Actual + Revised + Chased only")}{card("Actual completed",`${visibleSummary.actual}${visibleSummary.actualLate?` (${visibleSummary.actualLate} late)`:""}`,"var(--success)","late count stays inside total")}{card("Net revised",`${visibleSummary.revised}${visibleSummary.revisedLate?` (${visibleSummary.revisedLate} late)`:""}`,"var(--revised)","compared with previous effective due")}{card("Chased only",visibleSummary.chased,"var(--info)","comment added; still pending")}{card("Untouched",visibleSummary.untouched,visibleSummary.untouched?"var(--danger)":"var(--success)","no Actual, Revised or chase")}{card("Additional after 8 AM",visibleSummary.additional,"#b45309","not in morning workload")}</div>
    {role!=="junior"&&<div style={{background:"var(--surface)",border:"1px solid var(--line-2)",borderRadius:12,overflowX:"auto",marginBottom:12}}><table style={{width:"100%",minWidth:980,borderCollapse:"collapse",fontSize:10}}><thead><tr style={{background:"var(--ink)",color:"var(--bg)"}}>{["Merchant","Login","8 AM workload","Handled","Actual","Revised","Chased only","Untouched","Additional","Submission"].map(h=><th key={h} style={{padding:"8px 9px",textAlign:"left",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>{summaries.map(r=><tr key={r.key} onClick={()=>setSelectedUser(r.key)} style={{borderBottom:"1px solid var(--line-3)",cursor:"pointer",background:selectedKey===r.key?"var(--accent-tint)":"transparent"}}><td style={{padding:"8px 9px",fontWeight:950,color:r.trackerMerchant?"var(--info)":"var(--danger)"}}>{r.trackerMerchant||"NOT LINKED"} →</td><td style={{padding:"8px 9px",fontSize:9,color:"var(--muted-2)"}}>{r.name}</td><td style={{padding:"8px 9px",fontWeight:900}}>{r.required}</td><td style={{padding:"8px 9px",color:"var(--success)",fontWeight:900}}>{r.handled}</td><td style={{padding:"8px 9px",color:"var(--success)",fontWeight:900}}>{r.actual}{r.actualLate?` (${r.actualLate} late)`:""}</td><td style={{padding:"8px 9px",color:"var(--revised)",fontWeight:900}}>{r.revised}{r.revisedLate?` (${r.revisedLate} late)`:""}</td><td style={{padding:"8px 9px"}}>{r.chased}</td><td style={{padding:"8px 9px",color:r.untouched?"var(--danger)":"var(--success)",fontWeight:900}}>{r.untouched}</td><td style={{padding:"8px 9px"}}>{r.additional}</td><td style={{padding:"8px 9px",fontWeight:900,color:!r.submission?"var(--danger)":r.changedAfter?"#b45309":"var(--success)"}}>{!r.submission?"Not submitted":r.changedAfter?"Changes after submission":"Submitted"}</td></tr>)}</tbody></table></div>}
    <div style={{background:"var(--surface)",border:"1px solid var(--line-2)",borderRadius:12,overflowX:"auto",marginBottom:12}}><div style={{padding:"10px 12px",fontFamily:"'Archivo',sans-serif",fontWeight:850,fontSize:14}}>Update detail · {visibleRows.length}</div><table style={{width:"100%",minWidth:1380,borderCollapse:"collapse",fontSize:10}}><thead><tr style={{background:"var(--toolbar-subtle)"}}>{["Merchant / entered by","Order / Style","Activity","Update","Previous effective due","Final value","Entry timing","Morning task","Outcome","Edits"].map(h=><th key={h} style={{padding:"8px 9px",textAlign:"left",borderBottom:"1px solid var(--line-2)",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>{visibleRows.map(r=><tr key={r.key} onClick={()=>onJump&&onJump(r.styleId,r.stageKey)} style={{borderBottom:"1px solid var(--line-3)",cursor:"pointer",background:r.entryLate?"#fff1ed":r.movedLater?"#fff8f0":"transparent"}}><td style={{padding:"8px 9px",fontWeight:850}}>{displayName(r.reportMerchantKey,r.actorName)}<div style={{fontSize:8.5,color:"var(--muted-2)"}}>entered by {r.actorName}</div></td><td style={{padding:"8px 9px"}}><b>{r.styleNo}</b><div style={{fontSize:8.5,color:"var(--muted-2)"}}>{r.orderNo}{r.colour?` · ${r.colour}`:""}</div></td><td style={{padding:"8px 9px",fontWeight:850}}>{r.activity}</td><td style={{padding:"8px 9px",fontWeight:900,color:r.kind==="actual"?"var(--success)":r.kind==="revised"?"var(--revised)":"var(--danger)"}}>{r.kind==="actual"?"Actual completed":r.kind==="revised"?"Net revised":r.kind}</td><td style={{padding:"8px 9px",whiteSpace:"nowrap"}}>{r.expectedValue||r.dueBefore||"—"}<div style={{fontSize:8.5,color:"var(--muted-2)"}}>{r.expectedSource||""}</div></td><td style={{padding:"8px 9px",fontWeight:900}}>{r.finalValue||"—"}</td><td style={{padding:"8px 9px",fontWeight:950,color:r.entryLate?"var(--danger)":"var(--success)"}}>{r.entryTimingLabel}{r.entryLate&&<div style={{fontSize:8.5}}>exact delay: {r.entryLateDays} wd</div>}</td><td style={{padding:"8px 9px",fontWeight:850,color:r.morningIdentity?"var(--success)":"#b45309"}}>{r.morningIdentity?"Yes":"Additional"}</td><td style={{padding:"8px 9px",color:r.movedLater||Number(r.variance)>0&&r.kind==="actual"?"var(--danger)":"var(--muted-3)",fontWeight:850}}>{r.result}</td><td style={{padding:"8px 9px",color:r.editCount>1?"#b45309":"var(--muted-2)"}}>{r.editCount}{r.editCount>1?" collapsed":""}</td></tr>)}{!visibleRows.length&&<tr><td colSpan={10} style={{padding:18,textAlign:"center",color:"var(--muted-2)"}}>No Actual, Revised, Rejected or Waived updates after 8:00 AM for this selection.</td></tr>}</tbody></table></div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(360px,1fr))",gap:12,marginBottom:12}}><div style={{background:"var(--surface)",border:"1px solid var(--line-2)",borderRadius:12,padding:12}}><div style={{fontFamily:"'Archivo',sans-serif",fontWeight:850,fontSize:14}}>Chase comments entered after 8 AM · {visibleComments.length}</div><div style={{fontSize:8.5,color:"var(--muted-2)",marginTop:2}}>If the same task later gets Actual or Revised, it is not double-counted as “Chased only.”</div><div style={{maxHeight:240,overflowY:"auto",marginTop:7}}>{visibleComments.slice(0,100).map(c=><button key={c.id} onClick={()=>onJump&&onJump(c.style_id,c.col)} style={{display:"block",width:"100%",textAlign:"left",border:"none",borderBottom:"1px solid var(--line-3)",background:"transparent",padding:"7px 0",fontFamily:"inherit",cursor:"pointer"}}><div style={{fontSize:9,color:"var(--muted-2)"}}>{c.author_name} · {c.orderNo} · {c.styleNo} · {c.activity}</div><div style={{fontSize:10.5,marginTop:2}}>{c.body}</div></button>)}{!visibleComments.length&&<div style={{padding:"12px 0",fontSize:10,color:"var(--muted-2)"}}>No chase comments entered after 8 AM.</div>}</div></div><div style={{background:"var(--surface)",border:"1px solid var(--line-2)",borderRadius:12,padding:12}}><div style={{display:"flex",justifyContent:"space-between",gap:8,alignItems:"baseline"}}><div style={{fontFamily:"'Archivo',sans-serif",fontWeight:850,fontSize:14}}>8 AM work untouched · {visibleUntouched.length}</div>{selectedKey!=="All"&&linkedMerchantForKey(selectedKey)&&<button onClick={()=>onOpenTodo&&onOpenTodo(linkedMerchantForKey(selectedKey))} style={{...inputStyle,cursor:"pointer",fontWeight:900}}>Open merchant in To-Do →</button>}</div><div style={{maxHeight:240,overflowY:"auto",marginTop:7}}>{visibleUntouched.slice(0,100).map((t,i)=><div key={`${t.identity}:${i}`} style={{padding:"7px 0",borderBottom:"1px solid var(--line-3)",fontSize:10}}><b>{t.styleNo||t.colour||"—"}</b> · {t.orderNo||""}<div style={{fontSize:9,color:"var(--danger)",marginTop:2}}>{t.activity||stageLabelFromKeyGlobal(t.stageKey)} · due {t.due||"—"}</div></div>)}{!visibleUntouched.length&&<div style={{padding:"12px 0",fontSize:10,color:"var(--success)",fontWeight:850}}>No 8 AM task is untouched for this selection.</div>}</div></div></div>
    <details style={{background:"var(--surface)",border:"1px solid var(--line-2)",borderRadius:12,padding:12,marginBottom:12}}><summary style={{fontFamily:"'Archivo',sans-serif",fontWeight:850,fontSize:13,cursor:"pointer"}}>Corrections / repeated edits · {visibleCorrections.length} <span style={{fontFamily:"inherit",fontSize:9,color:"var(--muted-2)",fontWeight:650}}>not counted as additional work</span></summary><div style={{marginTop:8,maxHeight:260,overflowY:"auto"}}>{visibleCorrections.map(r=><div key={r.key} style={{padding:"7px 0",borderBottom:"1px solid var(--line-3)",fontSize:10}}><b>{r.actorName}</b> · {r.orderNo} · {r.styleNo} · {r.activity}<div style={{fontSize:9,color:"#b45309",marginTop:2}}>{r.editCount} effective edit{r.editCount===1?"":"s"} · {r.meaningful?`final ${r.finalValue}`:"reverted/no net change"}</div></div>)}{!visibleCorrections.length&&<div style={{fontSize:10,color:"var(--success)"}}>No corrections or repeated edits.</div>}</div></details>
    {role==="junior"&&<div style={{background:"var(--surface)",border:"2px solid var(--ink)",borderRadius:12,padding:14}}><div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"flex-start",flexWrap:"wrap"}}><div><div style={{fontFamily:"'Archivo',sans-serif",fontWeight:900,fontSize:16}}>Review and submit EOD report</div><div style={{fontSize:9.5,color:"var(--muted-2)",marginTop:3}}>The report above is automatic. You do not need to rewrite completed work or tomorrow’s priorities.</div></div><div style={{fontSize:10,fontWeight:900,color:!mySubmission?"var(--danger)":lastWorkAt(myKey)>new Date(mySubmission.submittedAt||0).getTime()?"#b45309":"var(--success)"}}>{!mySubmission?"Not submitted":lastWorkAt(myKey)>new Date(mySubmission.submittedAt||0).getTime()?"Changes made after submission · resubmit":"Submitted · "+new Date(mySubmission.submittedAt).toLocaleTimeString("en-IN",{timeZone:"Asia/Kolkata",hour:"2-digit",minute:"2-digit"})}</div></div><label style={{display:"block",fontSize:9,fontWeight:900,marginTop:10}}>OPTIONAL — IMPORTANT WORK NOT RECORDED IN THE TRACKER<textarea value={otherWork} onChange={e=>setOtherWork(e.target.value)} placeholder="Usually leave blank. Example: Called mill about delayed black fabric for Order F1." rows={2} style={{...inputStyle,width:"100%",resize:"vertical",display:"block",marginTop:4}}/></label><div style={{display:"flex",justifyContent:"flex-end",marginTop:10}}><button onClick={submitMyDay} disabled={submitBusy||busy||snapshotBusy||!morningSnapshot} style={{fontFamily:"inherit",fontSize:11,fontWeight:950,padding:"9px 15px",border:"1px solid var(--ink)",borderRadius:9,background:"var(--accent)",color:"var(--ink)",cursor:submitBusy?"wait":"pointer",opacity:morningSnapshot?1:.55}}>{submitBusy?"Submitting…":mySubmission?"Resubmit EOD Report":"Submit EOD Report"}</button></div></div>}
  </div>;
}
...
function ManagementDashboardView({ computed, todoItems, cfg, stageEvents, role, me, userMerchantLinks={}, disciplineOnly=false, applyDrill, onDisciplineJump, drillTodo }){
  const [target,setTarget]=useState("tracker"); // where bar/owner/activity drills go: Tracker or To-Do only. Escalation cards route to To-Do with Type=Escalation.
  ...
  // not whether the underlying TNA commitment was achieved. Entry is accepted through 12:00 PM IST
  // on the next working day; Sunday is excluded through the shared working-day helpers.
  const disciplineNorm=(v)=>String(v||"").toLowerCase().replace(/[^a-z0-9]+/g,"");
  const disciplineMeKeys=[disciplineNorm(me&&userMerchantLinks[me.id]),disciplineNorm(me&&me.name),disciplineNorm(me&&me.email),disciplineNorm(String(me&&me.email||"").split("@")[0])].filter(Boolean);
  ...
  (stageEvents||[]).forEach(e=>{
    ...
    const priorDue=type==="revised"?(parse(e.old_value)||(row&&row.plan)||null):((row&&(row.rev||row.plan))||eventDate);
    ...
    const graceDeadline=priorDue?new Date(`${iso(addWorkdays(priorDue,1))}T12:00:00+05:30`):null;
    const onTime=!graceDeadline||createdRaw<=graceDeadline;
    disciplineEvents.push({ kind:type==="revised"?"Revised":"Actual", owner:disciplineOwnerOf(pair.s), styleId:pair.s.id, styleNo:pair.s.styleNo||"", orderNo:pair.s.orderNo||"", stageKey:e.stage_key, stage:stageLabelFromKeyGlobal(e.stage_key)||e.stage_key, due:priorDue, entered:createdRaw, enteredValue:eventDate, lag, onTime, delay:onTime?0:Math.max(1,lag||1) });
  });
  ...
  disciplineFc.forEach(({s,c})=>{
    ...
    (c.frontier?[...c.frontier]:[]).forEach(k=>{
      ...
      const graceEnd=new Date(`${iso(addWorkdays(due,1))}T12:00:00+05:30`);
      const nowLive=new Date();
      const status=nowLive>graceEnd?"Missing":(TODAY>due?"Grace":"Not due");
      if(status==="Not due") return;
      disciplineOpen.push({ kind:"Open", owner:disciplineOwnerOf(s), styleId:s.id, styleNo:s.styleNo||"", orderNo:s.orderNo||"", stageKey:k, stage:stageReviewLabel(s,r), due, graceEnd, status, delay:status==="Missing"?Math.max(1,(netWorkdays(due,TODAY)||0)):0, revised:!!r.rev });
    });
  });
  const disciplineAgg={};
  ...
  const disciplinePanel=(<>
    ...
    {!disciplineRangeValid?<div style={{ padding:"9px 10px", border:"1px solid var(--danger)", background:"var(--tint-late)", color:"var(--danger)", fontSize:10, fontWeight:800 }}>Choose a valid From and To range.</div>:<>
      <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:10 }}>
        ...
        {card("Updated on time",disciplineTeam.onTime,"var(--success)",null,"through next working day 12 PM IST")}
      ...
      <div style={{ fontSize:9, color:"var(--muted-2)", marginTop:9, lineHeight:1.5 }}>{role==="junior"?"Junior view is restricted to styles whose Junior / Style Owner matches the signed-in profile. ":"Senior/management view shows the selected team slice. "}Click a merchant to open their Tracker rows; click an exception to open the exact activity cell. Missing and grace cards open the actionable To-Do rows. Actual or Revised entry is on time through 12:00 PM IST on the next working day; Sunday is excluded. Detailed edit history stays in Entry Log.</div>
    </>}
  </>);
  if(disciplineOnly){
...
/* ========================= TO-DO ========================= */
function TodoView({ items, cfg, setCfg, canEditSettings, role, me, userMerchantLinks={}, filter, setFilter, onJump }){
  const OWNER_COLOR2=OWNER_COLOR;
  ...
  const [showDetailed,setShowDetailed]=useState(()=>{ try{ return localStorage.getItem("mt_todo_detailed")==="1"; }catch(e){ return false; } });
  ...
  useEffect(()=>{ try{ localStorage.setItem("mt_todo_detailed",showDetailed?"1":"0"); }catch(e){} },[showDetailed]);
  ...
  const linkedMerchant=String((me&&userMerchantLinks&&userMerchantLinks[me.id])||"").trim();
  useEffect(()=>{
    if(role!=="junior"||!linkedMerchant) return;
    const next=cleanTodoFilter({...(filter||{}),junior:[linkedMerchant]});
    setTf(next);
  ...
  const clearTodoFilters=()=>{ const next=role==="junior"&&linkedMerchant?{junior:[linkedMerchant]}:{}; setTf(next); setFilter&&setFilter(next); };
  ...
  const head=<div style={{ minWidth:minTableWidth, display:"grid", gridTemplateColumns:GRID, alignItems:"end", columnGap:0, borderBottom:"2px solid var(--ink)", background:"var(--surface)", position:"sticky", top:0, zIndex:5 }}>
    ...
    {headCell("junior","Merchant",hsel("junior",juniors,"Merchant"))}
  ...
  const data=sortedShown.map(t=>({ "Status":attentionOf(t), "Due Priority":t.overdue?"Overdue":"Upcoming", "Risk Bucket":t.priorityBucket||"Daily Chase", "Priority Score":Number(t.priorityScore)||0, "Priority Reason":t.priorityReason||"", "To-Do Section":itemGroup(t)==="fabricLab"?"Fabric / Lab Dip":"Activities", "Type":t.todoType||"Activity", "Order No":t.orderNo||"", "Style / Colour":t.isColour?String(t.colour||""):t.styleNo, "Grouped Count":t.isColour?(t.count||0):"", "Merchant":t.junior||"", "Activity":t._activityLabel||t.activity||"", "Branch":t.branch||"", "Chase Label":t.owner||"", "Escalation Owner":t.escalationOwner||"", "Escalation Level":t.escalationLevel||"", "Escalation Action":t.escalationAction||"", "Original Plan":t.originalPlan?fmt(t.originalPlan):"", "Due Used":t.dueUsed?fmt(t.dueUsed):(t.exp?fmt(t.exp):""), "Days Late / Left":t.overdue?Math.abs(t.du):t.du, "Drift vs Original Days":Number(t.driftOriginal)||0, "Revision Count":Number(t.revisionCount)||0, "Rejection Round":Number(t.rejectionRound)||0, "Style ID":t.id, "Stage Key":t.key, "Activity Key":t.activityKey||t.key, "Fit":t.fit||"", "Family":t.family||"", "Brand":t.brand||"", "Fabric":t.fabric||"", "Buyer":t.buyer||"" }));
  ...
  const focusRows=[...shown].filter(t=>t.todoType!=="Escalation").sort((a,b)=>{ const ar=attentionOf(a)==="Critical"?3:attentionOf(a)==="Overdue"?2:1, br=attentionOf(b)==="Critical"?3:attentionOf(b)==="Overdue"?2:1; return br-ar||(Number(b.priorityScore)||0)-(Number(a.priorityScore)||0)||(a.exp&&b.exp?a.exp-b.exp:0)||todoRowIdentity(a).localeCompare(todoRowIdentity(b)); }).slice(0,12);
  const focusRow=(t,i)=>{ const status=attentionOf(t); const tone=cardTone(status); const itemName=t.isColour?(t.colour||"Colour task"):(t.styleNo||"Style task"); const order=t.orderNo||((t.orderNos||[])[0])||"—"; return <div key={todoRowIdentity(t)+"::focus::"+i} style={{ display:"grid", gridTemplateColumns:"92px minmax(220px,1.3fr) minmax(170px,1fr) minmax(145px,.8fr) 115px", gap:10, alignItems:"center", padding:"10px 12px", borderTop:i?"1px solid var(--line-3)":"none", background:i%2?"#fffdf8":"var(--surface)" }}>
    <span style={{ display:"inline-flex", justifyContent:"center", border:`1px solid ${tone.bd}`, background:tone.bg, color:tone.fg, borderRadius:999, padding:"4px 7px", fontSize:9.5, fontWeight:950 }}>{status}</span>
    <span style={{ minWidth:0 }}><b style={{ display:"block", fontSize:11.5, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{t._activityLabel||t.activity||"Task"}</b><span style={{ display:"block", fontSize:9.5, color:"var(--muted-2)", marginTop:2, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{itemName} · Order {order}</span></span>
    <span style={{ minWidth:0 }}><b style={{ display:"block", fontSize:10.5, color:OWNER_COLOR2[t.owner]||"var(--ink)", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>Chase {t.owner||"Unassigned"}</b><span style={{ display:"block", fontSize:9.5, color:"var(--muted-2)", marginTop:2, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{t.junior?`Merchant: ${t.junior}`:"Merchant not assigned"}</span></span>
    <span style={{ fontSize:10.5, fontWeight:850, color:status==="Upcoming"?"#8a5200":"var(--danger)" }}>{t.exp?fmt(t.exp):"No date"}<span style={{ display:"block", fontSize:9.5, marginTop:2 }}>{t.overdue?`${todoDaysLate(t)} working day${todoDaysLate(t)===1?"":"s"} late`:`${Math.max(0,Number(t.du)||0)} working day${Math.max(0,Number(t.du)||0)===1?"":"s"} left`}</span></span>
    <button onClick={()=>onJump(t)} style={{ fontFamily:"inherit", fontSize:10.5, fontWeight:900, padding:"7px 10px", cursor:"pointer", border:"1px solid var(--ink)", borderRadius:9, background:"var(--surface)", color:"var(--ink)" }}>{t.isColour?"Open styles":"Open task"} →</button>
  </div>; };
  ...
  const removeChip=(c)=>{ if(role==="junior"&&c.kind==="val"&&c.k==="junior") return; if(c.kind==="activity") set("activityKey",[]); else if(c.kind==="styleId"){ set("styleId",[]); set("styleIds",[]); } else { const cur=arrVal(tf[c.k]).map(String); set(c.k, cur.filter(x=>x!==String(c.val))); } };
  ...
  return (<div style={{ padding:"16px 22px", maxWidth:"none" }}>
    <div style={{ display:"flex", justifyContent:"space-between", gap:16, alignItems:"flex-start", flexWrap:"wrap", marginBottom:12 }}>
      <div><div style={{ fontFamily:"'Archivo',sans-serif", fontWeight:900, fontSize:23 }}>To-Do chase board</div><div style={{ fontSize:11.5, color:"var(--muted-2)", marginTop:4, lineHeight:1.45 }}>Start with Critical, clear Overdue next, then chase Upcoming before it becomes late. The full data is still available under Detailed task list.</div></div>
      <span style={{ fontSize:10, color:"var(--muted-2)", background:"var(--surface)", border:"1px solid var(--line-2)", borderRadius:999, padding:"6px 10px" }}>Showing {shown.length} of {displayItems.length}</span>
    </div>
    <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8, flexWrap:"wrap" }}>
      <div style={{ display:"inline-flex", borderRadius:8, overflow:"hidden" }}>{modeBtn("all","All work",rawDisplayItems.length)}{modeBtn("activity","Activities",activityCount)}{modeBtn("fabricLab","Fabric / Lab Dip",fabricLabCount)}</div>
      {role==="junior"?<span style={{ minWidth:190, minHeight:34, display:"inline-flex", alignItems:"center", padding:"6px 10px", border:"1px solid var(--line-2)", borderRadius:8, background:"var(--surface)", fontSize:10, fontWeight:900, color:linkedMerchant?"var(--ink)":"var(--danger)" }}>Merchant: {linkedMerchant||"not linked in Users"}</span>:<span style={{ minWidth:190 }}>{hsel("junior",juniors,"Merchant: all")}</span>}
      ...
      {anyF && <button onClick={clearTodoFilters} style={{ fontFamily:"inherit", fontSize:10, padding:"5px 9px", cursor:"pointer", border:"1px solid var(--danger)", background:"var(--surface)", color:"var(--danger)", fontWeight:700 }}>clear filters</button>}
    ...
    {activeChips.length>0 && <div style={{ display:"flex", alignItems:"center", gap:7, flexWrap:"wrap", marginBottom:9, padding:"7px 9px", border:"1px solid "+(isDrillContext?"var(--accent)":"var(--line-2)"), borderRadius:10, background:isDrillContext?"var(--accent-tint)":"var(--surface)" }}>
      ...
      {activeChips.map((c,i)=>{ const fixedMerchant=role==="junior"&&c.kind==="val"&&c.k==="junior"; return <span key={c.kind+":"+(c.k||"")+":"+(c.val||c.label)+":"+i} style={{ display:"inline-flex", alignItems:"center", gap:5, fontSize:10, fontWeight:700, padding:"3px 5px 3px 9px", borderRadius:999, border:"1px solid var(--ink)", background:"var(--surface)", color:"var(--ink)", whiteSpace:"nowrap" }}>{c.label}{!fixedMerchant&&<button onClick={()=>removeChip(c)} title="remove this filter" style={{ border:"none", background:"transparent", cursor:"pointer", padding:0, lineHeight:0, color:"var(--muted-2)", display:"inline-flex" }}><X size={12}/></button>}</span>; })}
      <button onClick={clearTodoFilters} style={{ fontFamily:"inherit", fontSize:10, fontWeight:800, padding:"3px 9px", cursor:"pointer", border:"1px solid var(--danger)", borderRadius:999, background:"var(--surface)", color:"var(--danger)" }}>Clear all</button>
    </div>}
    ...
    <div style={{ display:"flex", gap:10, flexWrap:"wrap", marginBottom:12 }}>{card("Critical",statusCounts.critical,">5 working days late","var(--danger)",()=>set("attention",["Critical"]))}{card("Overdue",statusCounts.overdue,"1–5 working days late","var(--danger)",()=>set("attention",["Overdue"]))}{card("Upcoming",statusCounts.upcoming,"within watch window","#7a560f",()=>set("attention",["Upcoming"]))}{card("All statuses",statusCounts.all,"within current chase filter",null,()=>set("attention",[]))}</div>
    <div style={{ border:"1px solid var(--line-2)", borderRadius:12, background:"var(--surface)", boxShadow:"var(--card-shadow)", overflow:"hidden", marginBottom:12 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", gap:10, padding:"11px 12px", background:"#fffaf1", borderBottom:"1px solid var(--line-3)" }}><div><b style={{ fontFamily:"'Archivo',sans-serif", fontSize:14 }}>Priority work queue</b><span style={{ display:"block", fontSize:9.5, color:"var(--muted-2)", marginTop:2 }}>Most urgent first · up to 12 tasks · respects the filters above</span></div><button onClick={()=>setShowDetailed(true)} style={{ fontFamily:"inherit", fontSize:10, fontWeight:900, border:"1px solid var(--ink)", borderRadius:8, background:"var(--surface)", padding:"6px 9px", cursor:"pointer" }}>View all {shown.length} tasks</button></div>
      {focusRows.length?focusRows.map(focusRow):<div style={{ fontSize:11, color:"var(--success)", padding:"16px 12px", fontWeight:800 }}>Nothing due or coming up in this chase view. ✓</div>}
    </div>
    <details style={{ border:"1px solid var(--line-2)", borderRadius:12, background:"var(--surface)", boxShadow:"var(--card-shadow)", marginBottom:10, overflow:"hidden" }}><summary style={{ cursor:"pointer", padding:"11px 12px", fontSize:11.5, fontWeight:950, background:"#fffdf8" }}>Browse by department · {departmentSummary.length} departments</summary><div style={{ padding:12, borderTop:"1px solid var(--line-3)" }}><div style={{ fontSize:9.5, color:"var(--muted-2)", marginBottom:8 }}>Click a department for all its work, or select Upcoming / Overdue / Critical individually.</div><div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:8 }}>{departmentTile(allDepartmentCounts,true)}{departmentSummary.map(d=>departmentTile(d,false))}</div></div></details>
    <details style={{ border:"1px solid var(--line-2)", borderRadius:12, background:"var(--surface)", boxShadow:"var(--card-shadow)", marginBottom:10, overflow:"hidden" }}><summary style={{ cursor:"pointer", padding:"11px 12px", fontSize:11.5, fontWeight:950, background:"#fffdf8" }}>Browse by activity · {activitySummary.length} activity groups</summary><div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(230px,1fr))", gap:9, padding:12, borderTop:"1px solid var(--line-3)" }}>{activitySummary.map(a=><button key={a.activity} onClick={()=>applyTodoActivity(a.activity,a.activityKey)} style={{ border:"1px solid var(--line-3)", background:"var(--bg)", textAlign:"left", padding:"10px 11px", cursor:"pointer", fontFamily:"inherit", borderRadius:10 }}><div style={{ fontSize:12, fontWeight:950, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", marginBottom:7 }}>{a.activity}</div><div style={{ display:"flex", gap:7, flexWrap:"wrap" }}><span style={{ display:"inline-flex", alignItems:"center", gap:4, background:"#fff4d8", color:"#8a5200", border:"1px solid #d58a13", borderRadius:999, padding:"3px 7px", fontSize:11, fontWeight:950 }}>U <b style={{ fontSize:15 }}>{a.upcoming}</b></span><span style={{ display:"inline-flex", alignItems:"center", gap:4, background:"#ffe4dc", color:"#b82117", border:"1px solid #c5251a", borderRadius:999, padding:"3px 7px", fontSize:11, fontWeight:950 }}>O <b style={{ fontSize:15 }}>{a.overdue}</b></span><span style={{ display:"inline-flex", alignItems:"center", gap:4, background:"#ffd4cc", color:"#9f1712", border:"1px solid #9f1712", borderRadius:999, padding:"3px 7px", fontSize:11, fontWeight:950 }}>Critical <b style={{ fontSize:15 }}>{a.critical}</b></span></div></button>)}</div></details>
    <details open={showDetailed} onToggle={e=>setShowDetailed(e.currentTarget.open)} style={{ border:"1px solid var(--line-2)", borderRadius:12, background:"var(--surface)", boxShadow:"var(--card-shadow)", overflow:"hidden" }}><summary style={{ cursor:"pointer", padding:"12px", fontSize:11.5, fontWeight:950, background:showDetailed?"var(--accent-tint)":"#fffdf8" }}>Detailed task list &amp; filters · {shown.length} rows</summary>{showDetailed && <div style={{ borderTop:"1px solid var(--line-3)" }}><div style={{ overflowX:"auto", background:"var(--surface)" }}>{head}<div key={`todo-rows:${todoMode}:${tfSig}`} data-todo-row-count={shown.length}>{sortedShown.length?sortedShown.map(row):<div style={{ fontSize:11, color:"var(--muted-1)", padding:"12px" }}>Nothing due or coming up. 👍</div>}</div></div><div style={{ fontSize:9, color:"var(--muted-7)", padding:"9px 12px" }}>Click any column label to sort; click again to reverse it. Multiple filters are supported. Critical = overdue by more than 5 working days.</div></div>}</details>
  </div>);
}
...
function SettingsView({ cfg, setCfg, canEdit }){
  ...
  return (<div style={{ padding:"18px 22px", maxWidth:1180 }}>
    ...
    <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(360px,1fr))", gap:14 }}>
      <div style={section}>{sectionHead("leads","Stage lead times","Working days each stage takes after its predecessor.",STAGES.length+" stages")}{open==="leads" && <div style={{ padding:"4px 15px 14px" }}>{STAGES.map(st=><React.Fragment key={st.key}>{row(st.label, inp(cfg.leads[st.key], v=>setLead(st.key,v)), st.key==="fabricIH"?"From Lab Dip Approval; from Order Date when Lab Dip is not required.":undefined)}</React.Fragment>)}</div>}</div>
      ...
      <div style={section}>{sectionHead("gates","Delivery references and gate","Working-day dates counted backwards from delivery.","1 reference · 1 gate")}{open==="gates" && <div style={{ padding:"4px 15px 14px" }}>{row("Fabric cut-off reference", inp(cfg.fabricCutoff, v=>setTop("fabricCutoff",v)), "Remark/reference only — never changes Fabric IH, downstream plans or risk")}{row("Release gate", inp(cfg.relGate, v=>setTop("relGate",v)), "Prod file / release risk gate")}</div>}</div>
...
function LoginScreen(){
  const [email,setEmail]=useState(""); const [pw,setPw]=useState(""); const [mode,setMode]=useState("in"); const [msg,setMsg]=useState(""); const [busy,setBusy]=useState(false); const [remember,setRemember]=useState(true); const [showLoginPassword,setShowLoginPassword]=useState(false);
  ...
  const sendRecovery=async()=>{
    if(!email.trim()){ setMsg("Enter the user's email above, then tap Forgot password again."); return; }
    setBusy(true); setMsg("");
    try{
      const redirectTo=(typeof window!=="undefined")?`${window.location.origin}${window.location.pathname}`:undefined;
      const { error }=await supabase.auth.resetPasswordForEmail(email.trim(), redirectTo?{ redirectTo }:undefined);
      if(error) throw error;
      setMsg("Password reset email sent. Open the newest email; the link will show a Set new password screen.");
  ...
  return (<AuthShell>
    ...
    <form onSubmit={e=>{ e.preventDefault(); submit(); }}>
      ...
      <input style={inp} type={showLoginPassword?"text":"password"} name="password" autoComplete={mode==="in"?"current-password":"new-password"} placeholder="••••••••" value={pw} onChange={e=>setPw(e.target.value)}/>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, flexWrap:"wrap", margin:"4px 0 16px" }}><label style={{ display:"flex", alignItems:"center", gap:7, fontSize:11, color:"var(--muted-4)", cursor:"pointer" }}><input type="checkbox" checked={showLoginPassword} onChange={e=>setShowLoginPassword(e.target.checked)}/>Show password</label><label style={{ display:"flex", alignItems:"center", gap:7, fontSize:11, color:"var(--muted-4)", cursor:"pointer" }}><input type="checkbox" checked={remember} onChange={e=>setRemember(e.target.checked)}/>Keep me signed in <span style={{ color:"var(--muted-1)" }}>(otherwise 12h)</span></label></div>
    ...
    {mode==="in" && <button type="button" disabled={busy} onClick={sendRecovery} style={{ display:"block", margin:"0 auto 10px", border:"none", background:"transparent", color:"var(--accent)", cursor:busy?"wait":"pointer", fontFamily:"inherit", fontSize:11, fontWeight:800 }}>Forgot password?</button>}
...
function PasswordRecoveryScreen({ session, recoveryError, onComplete }){
  const [pw,setPw]=useState(""); const [pw2,setPw2]=useState(""); const [show,setShow]=useState(false); const [busy,setBusy]=useState(false); const [msg,setMsg]=useState(""); const [done,setDone]=useState(false);
  const finish=async()=>{
    if(pw.length<10){ setMsg("Use at least 10 characters."); return; }
    if(!/[a-z]/.test(pw)||!/[A-Z]/.test(pw)||!/[0-9]/.test(pw)){ setMsg("Include an uppercase letter, lowercase letter and number."); return; }
  ...
  const inp={ width:"100%", fontFamily:"inherit", fontSize:13, padding:"12px 13px", border:"1px solid var(--line-2)", borderRadius:10, marginBottom:12, boxSizing:"border-box", background:"var(--surface)", outline:"none" };
  if(done) return <AuthShell><div style={{ fontFamily:"'Archivo',sans-serif", fontWeight:800, fontSize:25, marginBottom:8 }}>Password changed</div><div style={{ fontSize:12, color:"var(--muted-3)", lineHeight:1.6, marginBottom:18 }}>The new password now works in both Merchant Tracker and Production Tracker because they share the same Supabase login.</div><button onClick={onComplete} style={{ width:"100%", fontFamily:"inherit", fontSize:13, fontWeight:800, padding:12, cursor:"pointer", border:"1px solid var(--ink)", borderRadius:10, background:"var(--ink)", color:"var(--bg)" }}>Return to sign in</button></AuthShell>;
  if(!session) return <AuthShell><div style={{ fontFamily:"'Archivo',sans-serif", fontWeight:800, fontSize:25, marginBottom:8 }}>Recovery link could not be opened</div><div style={{ fontSize:12, color:"var(--muted-3)", lineHeight:1.6, marginBottom:10 }}>Use the newest password-reset email. Recovery links are single-use and can expire.</div>{recoveryError&&<div style={{ fontSize:10.5, color:"var(--danger)", lineHeight:1.5, marginBottom:14 }}>{recoveryError}</div>}<button onClick={onComplete} style={{ width:"100%", fontFamily:"inherit", fontSize:13, fontWeight:800, padding:12, cursor:"pointer", border:"1px solid var(--ink)", borderRadius:10, background:"var(--ink)", color:"var(--bg)" }}>Return to sign in</button></AuthShell>;
  return <AuthShell>
    <div style={{ fontFamily:"'Archivo',sans-serif", fontWeight:800, fontSize:27, marginBottom:4 }}>Set new password</div>
    <div style={{ fontSize:12, color:"var(--muted-2)", marginBottom:20 }}>For <b>{session.user&&session.user.email}</b></div>
    <label style={{ display:"block", fontSize:10, fontWeight:800, color:"var(--muted-3)", textTransform:"uppercase", letterSpacing:0.8, marginBottom:5 }}>New password</label>
    <input style={inp} type={show?"text":"password"} autoComplete="new-password" value={pw} onChange={e=>setPw(e.target.value)}/>
    <label style={{ display:"block", fontSize:10, fontWeight:800, color:"var(--muted-3)", textTransform:"uppercase", letterSpacing:0.8, marginBottom:5 }}>Confirm password</label>
    <input style={inp} type={show?"text":"password"} autoComplete="new-password" value={pw2} onChange={e=>setPw2(e.target.value)} onKeyDown={e=>{ if(e.key==="Enter") finish(); }}/>
    <label style={{ display:"flex", alignItems:"center", gap:8, fontSize:10.5, color:"var(--muted-2)", marginBottom:14 }}><input type="checkbox" checked={show} onChange={e=>setShow(e.target.checked)}/>Show password</label>
    <button disabled={busy} onClick={finish} style={{ width:"100%", fontFamily:"inherit", fontSize:13, fontWeight:800, padding:12, cursor:busy?"wait":"pointer", border:"1px solid var(--ink)", borderRadius:10, background:"var(--ink)", color:"var(--bg)" }}>{busy?"Changing…":"Change password"}</button>
    <div style={{ fontSize:10, color:"var(--muted-2)", marginTop:12, lineHeight:1.5 }}>Minimum 10 characters with uppercase, lowercase and a number.</div>
    {msg&&<div style={{ fontSize:11, color:"var(--danger)", marginTop:12, lineHeight:1.45, border:"1px solid var(--tint-late)", background:"#fff4f1", borderRadius:10, padding:10 }}>{msg}</div>}
  </AuthShell>;
}
...
function UsersPanel({ me, merchantNames=[], userMerchantLinks={}, onLinksChange, onTeamChange, onClose }){
  const [list,setList]=useState(null); const [busy,setBusy]=useState(false);
  const [passwordUser,setPasswordUser]=useState(null); const [pw,setPw]=useState(""); const [pw2,setPw2]=useState(""); const [pwMsg,setPwMsg]=useState(""); const [showPw,setShowPw]=useState(false);
  const load=async()=>{ const { data }=await supabase.from("profiles").select("*").order("created_at",{ ascending:true }); setList(data||[]); if(onTeamChange) onTeamChange(data||[]); };
  useEffect(()=>{ load(); },[]);
  ...
  const setMerchantLink=async(id,value)=>{
    setBusy(true);
    try{
      const latest=await supabase.from("app_settings").select("data").eq("id",USER_MERCHANT_LINKS_SETTING_ID).maybeSingle(); if(latest&&latest.error) throw latest.error; const raw=latest&&latest.data&&latest.data.data; const current=raw&&typeof raw==="object"?((raw.links&&typeof raw.links==="object")?raw.links:raw):{};
      const next={...current}; if(value) next[id]=value; else delete next[id];
      const {error}=await supabase.from("app_settings").upsert({id:USER_MERCHANT_LINKS_SETTING_ID,data:{links:next,updatedAt:new Date().toISOString(),updatedBy:me&&me.id}}); if(error) throw error;
      if(onLinksChange) onLinksChange(next);
    }catch(e){ alert("Merchant link could not be saved: "+(e.message||e)); }
    setBusy(false);
  };
  const openPassword=(u)=>{ setPasswordUser(u); setPw(""); setPw2(""); setPwMsg(""); setShowPw(false); };
  const closePassword=()=>{ setPasswordUser(null); setPw(""); setPw2(""); setPwMsg(""); };
  const setPassword=async()=>{
    if(!passwordUser) return;
    if(pw.length<10){ setPwMsg("Use at least 10 characters."); return; }
    if(!/[a-z]/.test(pw)||!/[A-Z]/.test(pw)||!/[0-9]/.test(pw)){ setPwMsg("Include an uppercase letter, lowercase letter and number."); return; }
    if(pw!==pw2){ setPwMsg("The two passwords do not match."); return; }
    if(!window.confirm(`Change the sign-in password for ${passwordUser.name||passwordUser.email}?`)) return;
    setBusy(true); setPwMsg("");
    try{
      const { data,error }=await supabase.functions.invoke("admin-set-user-password",{ body:{ target_user_id:passwordUser.id, new_password:pw } });
      if(error) throw error;
      if(!data||data.ok!==true) throw new Error((data&&data.error)||"Password could not be changed.");
      setPw(""); setPw2(""); setPwMsg("Password changed. Give the new password to the user securely; it is not stored in the tracker.");
    }catch(e){ setPwMsg((e&&e.message)||"Password could not be changed. Confirm the admin-set-user-password function is deployed."); }
    setBusy(false);
  };
  ...
  return (<div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(26,26,26,0.55)", zIndex:400, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
    <div onClick={e=>e.stopPropagation()} style={{ background:"var(--bg)", border:"2px solid var(--ink)", boxShadow:"8px 8px 0 var(--ink)", width:980, maxWidth:"100%", maxHeight:"88vh", overflowY:"auto", padding:22, fontFamily:"'JetBrains Mono',monospace" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}><div style={{ fontFamily:"'Archivo',sans-serif", fontWeight:800, fontSize:18 }}>Users, roles &amp; passwords</div><button onClick={onClose} title="Close users panel" style={{ border:"1px solid var(--line-2)", background:"var(--surface)", cursor:"pointer", borderRadius:9, padding:"7px 10px", display:"inline-flex", alignItems:"center", gap:5 }}><X size={16}/><span style={{ fontSize:11, fontWeight:800 }}>Close</span></button></div>
      <p style={{ fontSize:11, color:"var(--muted-3)", lineHeight:1.55, marginBottom:12 }}>Set the role, then link every Junior login to the exact Junior Merchant name used in Tracker rows. This link drives that person’s Today &amp; Behind workload, Daily Updates required count and To-Do drilldown. Passwords can also be changed directly and are never saved in tracker history.</p>
      {list===null ? <div style={{ fontSize:12, color:"var(--muted-2)" }}>Loading…</div> : list.length===0 ? <div style={{ fontSize:12, color:"var(--muted-2)" }}>No users yet — have your team sign up from the login screen.</div> : (
        <div style={{ border:"1px solid var(--ink)", background:"var(--surface)" }}>{list.map((u,i)=>(<div key={u.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 10px", borderBottom:i<list.length-1?"1px solid #eee7da":"none" }}>
          <div style={{ flex:1, minWidth:0 }}><input value={u.name||""} placeholder="(set name)" onChange={e=>editName(u.id,e.target.value)} onBlur={e=>{ e.target.style.border="1px solid transparent"; e.target.style.background="transparent"; saveName(u.id,e.target.value); }} onKeyDown={e=>{ if(e.key==="Enter") e.target.blur(); }} style={{ fontFamily:"inherit", fontSize:12, fontWeight:700, width:"100%", border:"1px solid transparent", background:"transparent", padding:"2px 4px", boxSizing:"border-box" }} onFocus={e=>{ e.target.style.border="1px solid var(--ink)"; e.target.style.background="var(--surface)"; }} />
            <div style={{ fontSize:10, color:"var(--muted-1)", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", padding:"0 4px" }}>{u.email}{u.id===(me&&me.id)?" · you":""}</div></div>
          <select disabled={busy} value={u.role||"pending"} onChange={e=>setR(u.id,e.target.value)} style={{ fontFamily:"inherit", fontSize:11, padding:"6px 7px", border:"1px solid var(--ink)", background:u.role==="pending"?"#fbeaea":"var(--accent-tint)" }}>{ROLE_OPTS.map(r=>(<option key={r} value={r}>{r==="pending"?"— pending —":(ROLES[r]||{}).label||r}</option>))}</select>
          {u.role==="junior"?<select disabled={busy} value={userMerchantLinks[u.id]||""} onChange={e=>setMerchantLink(u.id,e.target.value)} title="Exact Junior Merchant name used in Tracker" style={{ fontFamily:"inherit", fontSize:10.5, padding:"6px 7px", minWidth:185, border:"1px solid "+(userMerchantLinks[u.id]?"var(--success)":"var(--danger)"), background:userMerchantLinks[u.id]?"#eef8f1":"#fff4f1", fontWeight:800 }}><option value="">— Link tracker merchant —</option>{merchantNames.map(name=><option key={name} value={name}>{name}</option>)}</select>:<span style={{minWidth:185,fontSize:9,color:"var(--muted-1)"}}>Link applies to Junior role</span>}
          <button disabled={busy} onClick={()=>openPassword(u)} style={{ fontFamily:"inherit", fontSize:10.5, fontWeight:850, padding:"6px 9px", cursor:busy?"wait":"pointer", border:"1px solid var(--ink)", borderRadius:8, background:passwordUser&&passwordUser.id===u.id?"var(--accent)":"var(--surface)" }}>Set password</button>
        </div>))}</div>
      )}
      {passwordUser && <div style={{ marginTop:14, border:"1px solid var(--ink)", borderRadius:12, padding:14, background:"#fffaf1" }}>
        <div style={{ display:"flex", justifyContent:"space-between", gap:12, alignItems:"flex-start", marginBottom:10 }}><div><b style={{ fontFamily:"'Archivo',sans-serif", fontSize:14 }}>Set a new password</b><span style={{ display:"block", fontSize:10, color:"var(--muted-2)", marginTop:3 }}>{passwordUser.name||passwordUser.email} · {passwordUser.email}</span></div><button onClick={closePassword} style={{ border:"none", background:"transparent", cursor:"pointer", color:"var(--muted-2)" }}><X size={16}/></button></div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr auto", gap:9, alignItems:"end" }}><label style={{ fontSize:9.5, fontWeight:850 }}>NEW PASSWORD<input type={showPw?"text":"password"} autoComplete="new-password" value={pw} onChange={e=>setPw(e.target.value)} style={{ display:"block", width:"100%", boxSizing:"border-box", marginTop:5, fontFamily:"inherit", fontSize:12, padding:"8px", border:"1px solid var(--ink)", borderRadius:8 }}/></label><label style={{ fontSize:9.5, fontWeight:850 }}>CONFIRM PASSWORD<input type={showPw?"text":"password"} autoComplete="new-password" value={pw2} onChange={e=>setPw2(e.target.value)} onKeyDown={e=>{ if(e.key==="Enter") setPassword(); }} style={{ display:"block", width:"100%", boxSizing:"border-box", marginTop:5, fontFamily:"inherit", fontSize:12, padding:"8px", border:"1px solid var(--ink)", borderRadius:8 }}/></label><button disabled={busy} onClick={setPassword} style={{ fontFamily:"inherit", fontSize:11, fontWeight:900, padding:"9px 12px", cursor:busy?"wait":"pointer", border:"1px solid var(--ink)", borderRadius:8, background:"var(--ink)", color:"var(--bg)" }}>{busy?"Changing…":"Change password"}</button></div>
        <label style={{ display:"flex", alignItems:"center", gap:7, marginTop:9, fontSize:10, color:"var(--muted-2)", cursor:"pointer" }}><input type="checkbox" checked={showPw} onChange={e=>setShowPw(e.target.checked)}/>Show password while typing</label>
        <div style={{ fontSize:9.5, color:"var(--muted-2)", marginTop:7 }}>Minimum 10 characters with uppercase, lowercase and a number. Existing logged-in sessions may remain active until they sign out or expire.</div>
        {pwMsg && <div style={{ fontSize:10.5, marginTop:9, color:pwMsg.startsWith("Password changed")?"var(--success)":"var(--danger)", fontWeight:800, lineHeight:1.45 }}>{pwMsg}</div>}
      </div>}
    </div>
  </div>);
}
...
export default function App(){
  ...
  const [recovery,setRecovery]=useState(()=>{ if(typeof window==="undefined") return false; const h=new URLSearchParams(window.location.hash.replace(/^#/,"")),q=new URLSearchParams(window.location.search); return h.get("type")==="recovery"||q.get("type")==="recovery"||q.has("token_hash")||q.has("code"); });
  const [recoveryError,setRecoveryError]=useState("");
  const clearRecovery=()=>{ try{ window.history.replaceState({},document.title,window.location.pathname); }catch(e){} setRecovery(false); setRecoveryError(""); };
  useEffect(()=>{
    let active=true;
    const { data:sub }=supabase.auth.onAuthStateChange((event,s)=>{ if(event==="PASSWORD_RECOVERY") setRecovery(true); if(active){ setSession(s||null); if(!s) setProfile(null); } });
    (async()=>{
      try{
        const url=new URL(window.location.href),h=new URLSearchParams(url.hash.replace(/^#/,""));
        const recoveryRequested=h.get("type")==="recovery"||url.searchParams.get("type")==="recovery"||url.searchParams.has("token_hash")||url.searchParams.has("code");
        if(recoveryRequested&&active) setRecovery(true);
        let { data:current }=await supabase.auth.getSession();
        if(recoveryRequested&&!current.session){
          const code=url.searchParams.get("code"),tokenHash=url.searchParams.get("token_hash"),accessToken=h.get("access_token"),refreshToken=h.get("refresh_token");
          let result=null;
          if(tokenHash) result=await supabase.auth.verifyOtp({ token_hash:tokenHash, type:"recovery" });
          else if(code) result=await supabase.auth.exchangeCodeForSession(code);
          else if(accessToken&&refreshToken) result=await supabase.auth.setSession({ access_token:accessToken, refresh_token:refreshToken });
          if(result&&result.error) throw result.error;
          current=(await supabase.auth.getSession()).data;
        }
        if(active) setSession(current.session||null);
      }catch(e){ if(active){ setRecoveryError(e.message||String(e)); setSession(null); } }
    })();
    return ()=>{ active=false; try{ sub.subscription.unsubscribe(); }catch(e){} };
  },[]);
  ...
  if(recovery) return <PasswordRecoveryScreen session={session} recoveryError={recoveryError} onComplete={clearRecovery}/>;
  if(session && profile===undefined) return <Splash text="Loading…"/>;
