  ...
  return (
    <div ref={gridRef} tabIndex={0} onKeyDown={onKeyDown} onCopy={handleCopy} onPaste={handlePaste} onMouseDown={(e)=>{ if(editing) return; if(e.target.closest && (e.target.closest("input")||e.target.closest("button")||e.target.closest("th"))) return; const td=e.target.closest && e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m||e.shiftKey) return; e.preventDefault(); gridRef.current&&gridRef.current.focus(); setSel({ id:Number(m[1]), col:m[2] }); setFocus(null); selectingRef.current=true; setDragSel(true); }} onMouseUp={()=>{ if(filling){ applyFillHandle(); setFilling(false); setFillFrom(null); setFillTo(null); } if(selectingRef.current){ selectingRef.current=false; setDragSel(false); } }} onMouseOver={(e)=>{ const td=e.target.closest && e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m) return; if(filling){ setFillTo({ id:Number(m[1]), col:m[2] }); return; } if(selectingRef.current){ setFocus({ id:Number(m[1]), col:m[2] }); } }} onClick={()=>{ finishEditing(); setFillOpen(false); setColsOpen(false); setFilterCol(null); setFrOpen(false); setExpOpen(false); setBellOpen(false); setPeersOpen(false); }}
      ...
      <div style={{ display:"flex", gap:0, padding:"0 22px", background:"var(--ink)", borderBottom:"1px solid #3a362e", position:"relative", zIndex:155, pointerEvents:"auto" }}>
        {[["tracker","Tracker"],["dashboard","Dashboard"],["management","Management"],["escalation","Escalation"],["todo","To-Do"],["entrylog","Entry Log"],["settings","Settings"],["help","Help"]].map(([k,lab])=>(<button key={k} onClick={(e)=>{ e.stopPropagation(); setTab(k); if(k==="entrylog") loadAuditRows(); }} style={{ fontFamily:"'Archivo',sans-serif", fontWeight:700, fontSize:12, letterSpacing:0.3, padding:"9px 16px", cursor:"pointer", border:"none", borderBottom:tab===k?"3px solid var(--accent)":"3px solid transparent", background:"transparent", color:tab===k?"var(--bg)":"#9a958c" }}>{lab}{k==="todo"&&todoItems.length?` · ${todoItems.length}`:k==="entrylog"&&errorLog.length?` · ${errorLog.length}`:""}</button>))}
      </div>
...
function OperationalDashboardView({ computed, todoItems, cfg, applyDrill, drillTodo }){
  ...
  const sel=(label,val,opts,onChange)=>(<select multiple value={arrOf(val)} onChange={e=>{ const a=[...e.target.selectedOptions].map(o=>o.value).filter(Boolean); onChange(a.length?a:undefined); }} title="Hold Ctrl/Cmd to select multiple" style={{ fontFamily:"inherit", fontSize:10, padding:"4px 7px", border:"1px solid var(--ink)", background:arrOf(val).length?"var(--accent-tint)":"var(--surface)", maxWidth:170, minHeight:44 }}><option value="">{label}: all</option>{opts.map(o=>(<option key={o} value={o}>{o}</option>))}</select>);
...
function ManagementDashboardView({ computed, todoItems, cfg, applyDrill, drillTodo }){
  ...
  fc.forEach(({s,c})=>{
    ...
    (c.stages||[]).forEach(r=>{
      ...
      addPerf(chaseDelay,r.owner||"(No chase label)",r.owner||"(No chase label)",delay,duration);
  ...
  fc.forEach(({s,c})=>{ (c.stages||[]).forEach(r=>{ const due=r.rev||r.plan; const start=stageStartFor(s,c,r); const delayDue=(due&&r.actual)?netWorkdays(due,r.actual):null; const duration=(start&&r.actual)?Math.max(0,netWorkdays(start,r.actual)||0):null; currentSliceStageDetail.push({ "Order No":s.orderNo||"", "Style No":s.styleNo||"", "Buyer / Brand":s.buyer||s.brand||"", "Junior / Style Owner":s.owner||"", "Stage":r.label||"", "Chase Label":r.owner||"", "State":stageState(r), "Auto Plan Date":r.plan?fmt(r.plan):"", "Revised Date":r.rev?fmt(r.rev):"", "Due Used":due?fmt(due):"", "Actual Date":r.actual?fmt(r.actual):"", "Start Used":start?fmt(start):"", "Delay vs Due Days":delayDue==null?"":r1(delayDue), "Actual Duration Days":duration==null?"":r1(duration), "Actionable Frontier?":(c.frontier&&c.frontier.has(r.key))?"YES":"NO" }); }); });
  ...
  const sel=(label,val,opts,onChange)=>(<select multiple value={arrOf(val)} onChange={e=>{ const a=[...e.target.selectedOptions].map(o=>o.value).filter(Boolean); onChange(a.length?a:undefined); }} title="Hold Ctrl/Cmd to select multiple" style={{ fontFamily:"inherit", fontSize:10, padding:"5px 8px", border:"1px solid var(--line-2)", borderRadius:8, background:arrOf(val).length?"var(--accent-tint)":"var(--surface)", maxWidth:170, minHeight:46 }}><option value="">{label}: all</option>{opts.map(o=>(<option key={o} value={o}>{o}</option>))}</select>);
  ...
  const performanceRows = perfMode==="delay"
    ? (perfView==="stage" ? makeDelayAgg(lateRecords,r=>r.stage).slice(0,10).map(r=>({ ...r, displayLabel:r.label, right:`${r1(r.delaySum)}d total`, sub:`${r.delayed} delayed entries · avg ${fmtDays(avg(r.delaySum,r.delayed))} · worst ${fmtDays(r.maxDelay)} · use export for detail`, color:"var(--danger)", jump:null })) : chaseDelayRows.map(r=>({ ...r, displayLabel:r.label, right:`${r1(r.delaySum)}d total`, sub:`${r.delayed} delayed entries · avg ${fmtDays(avg(r.delaySum,r.delayed))} · worst ${fmtDays(r.maxDelay)} · use export for detail`, color:"var(--danger)", jump:null })))
    : (perfView==="stage" ? stageRows.slice(0,10).map(r=>({ ...r, displayLabel:r.label, right:`${fmtDays(avg(r.durSum,r.durN))} / ${fmtDays(avg(r.delaySum,r.n))}`, sub:`${r.owner||""} · ${r.n} done · ${r.lateN} late · net performance shown · use export for detail`, color:r.lateN?"var(--danger)":"var(--success)", jump:null })) : deptRows.map(r=>({ ...r, displayLabel:r.label, right:`${fmtDays(avg(r.durSum,r.durN))} avg · ${fmtDays(avg(r.delaySum,r.n))} net`, sub:`${r.n} completed · ${r.lateN} late · use export for detail`, color:r.lateN?"var(--danger)":"var(--success)", jump:null })));
