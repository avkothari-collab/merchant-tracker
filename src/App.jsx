...
const DEFAULT_CFG = { leads:Object.fromEntries(STAGES.map(s=>[s.key,s.lead])), stageOwners:Object.fromEntries(STAGES.map(s=>[s.key,s.owner])), rework:{...{ fitSend:4, artwork:2, strikeOff:3, labDip:7, ppSample:4 }}, fabricCutoff:FABRIC_CUTOFF_DAYS, relGate:REL_GATE_DAYS, upcoming:{...UPCOMING_DEFAULT} };
...
function computeStyle(s, cfg){
  ...
  const leadOf=(st)=>{ const v=cfg&&cfg.leads&&cfg.leads[st.key]; return v==null?st.lead:v; }; const rwOf=(st)=>{ const v=cfg&&cfg.rework&&cfg.rework[st.key]; return v==null?(REWORK_DAYS[st.key]||st.lead):v; }; const ownerOf=(k)=>{ const st=STAGES.find(x=>x.key===k); return (cfg&&cfg.stageOwners&&cfg.stageOwners[k]) || (st&&st.owner) || "Merchant"; }; const CUTD=(cfg&&cfg.fabricCutoff!=null)?cfg.fabricCutoff:FABRIC_CUTOFF_DAYS; const GATED=(cfg&&cfg.relGate!=null)?cfg.relGate:REL_GATE_DAYS;
  ...
  const chaseCount={};
  if(!released) STAGES.forEach(st=>{ if(appl(st.key)&&!done(st.key)&&predDone(st)){ const ow=ownerOf(st.key); chaseCount[ow]=(chaseCount[ow]||0)+1; } });
...
function MerchTracker({ me, onSignOut }){
  ...
  const buildUploadPreview=(br)=>{ try{ const { aoa, hr, sn, mapping }=br; const fi={}; Object.entries(mapping||{}).forEach(([k,v])=>{ const n=Number(v); if(Number.isFinite(n) && n>=0) fi[k]=n; }); if(fi.styleNo==null||fi.orderNo==null){ setBulkResult({...br, mapping:true, mapError:"Map both Style No and Order No before continuing."}); return; }
    const recs=[], errors=[]; for(let i=hr+1;i<aoa.length;i++){ const row=aoa[i]||[]; const styleNo=String(row[fi.styleNo]??"").trim(); const orderNo=String(row[fi.orderNo]??"").trim(); if(!styleNo&&!orderNo) continue; if(!styleNo||!orderNo){ errors.push({ rowNo:i+1, orderNo, styleNo, type:"Missing mandatory key", detail:"Style No and Order No are both mandatory.", blocking:true }); continue; } const rec={ styleNo, orderNo, rowNo:i+1 }; ["sampleFit","family","colour","brand","fabricType","owner","setId","setRole","age"].forEach(f=>{ if(fi[f]!=null){ const v=String(row[fi[f]]??"").trim(); if(v) rec[f]=v; } }); if(fi.qty!=null){ const q=String(row[fi.qty]??"").replace(/[^0-9.]/g,""); if(q) rec.qty=Number(q)||0; } if(fi.ordRec!=null){ const d=toISO(row[fi.ordRec]); if(d) rec.ordRec=d; else if(String(row[fi.ordRec]??"").trim()) errors.push({ rowNo:i+1, orderNo, styleNo, type:"Invalid date", detail:"Order Received could not be read", to:String(row[fi.ordRec]??""), blocking:false }); } if(fi.delivery!=null){ const d=toISO(row[fi.delivery]); if(d) rec.delivery=d; else if(String(row[fi.delivery]??"").trim()) errors.push({ rowNo:i+1, orderNo, styleNo, type:"Invalid date", detail:"Delivery Date could not be read", to:String(row[fi.delivery]??""), blocking:false }); } ["fitReq","printReq","soReq","ppBypass","labDipReq","ppNeeded"].forEach(f=>{ if(fi[f]!=null && String(row[fi[f]]??"").trim()!=="") rec[f]=toBool(row[fi[f]]); }); recs.push(rec); }
  ...
  const roleNamesForChase=(owner,s)=>{ const clean=(v)=>String(v||"").trim(); const first=(v)=>clean(v).split(/\s+/)[0]||clean(v); if(owner==="Merchant"){ if(clean(s.owner)) return [first(s.owner)]; const names=team.filter(t=>MERCH_ROLES.includes(t.role)).map(t=>first(t.name||t.email)).filter(Boolean); return names.length?names:["Merchant"]; } if(owner==="CAD") return team.filter(t=>t.role==="cad").map(t=>first(t.name||t.email)).filter(Boolean); if(owner==="Designer") return team.filter(t=>t.role==="designer").map(t=>first(t.name||t.email)).filter(Boolean); if(owner==="Mill"||owner==="Store") return team.filter(t=>t.role==="store").map(t=>first(t.name||t.email)).filter(Boolean); return [owner]; };
  const chaseDisplay=(owner,s)=>{ const names=roleNamesForChase(owner,s); const short=names.length?names.slice(0,3).join(" / ")+(names.length>3?" +"+(names.length-3):""):owner; return owner+": "+short; };
  const SAVED_VIEWS=[ ["","Saved view: none"], ["myOverdue","My overdue"], ["dueThisWeek","Due this week"], ["buyerPending","Buyer approval pending"], ["fabricPending","Fabric pending"], ["ppPending","PP pending"], ["deliveryRisk","Delivery risk"], ["following","Followed styles"], ["rework","Rejected / rework"], ["released","Released"] ];
    ...
    <div ref={gridRef} tabIndex={0} onKeyDown={onKeyDown} onCopy={handleCopy} onPaste={handlePaste} onMouseDown={(e)=>{ if(editing) return; if(e.target.closest && (e.target.closest("input")||e.target.closest("button")||e.target.closest("th"))) return; const td=e.target.closest && e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m||e.shiftKey) return; e.preventDefault(); gridRef.current&&gridRef.current.focus(); setSel({ id:Number(m[1]), col:m[2] }); setFocus(null); selectingRef.current=true; setDragSel(true); }} onMouseUp={()=>{ if(filling){ applyFillHandle(); setFilling(false); setFillFrom(null); setFillTo(null); } if(selectingRef.current){ selectingRef.current=false; setDragSel(false); } }} onMouseOver={(e)=>{ const td=e.target.closest && e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m) return; if(filling){ setFillTo({ id:Number(m[1]), col:m[2] }); return; } if(selectingRef.current){ setFocus({ id:Number(m[1]), col:m[2] }); } }} onClick={()=>{ finishEditing(); setFillOpen(false); setColsOpen(false); setFilterCol(null); setFrOpen(false); setExpOpen(false); setBellOpen(false); setPeersOpen(false); }}
      ...
      {tab==="help" && (<div style={{ padding:"18px 22px 36px" }}>
        <div style={{ background:"var(--surface)", border:"1px solid var(--ink)", boxShadow:"4px 4px 0 var(--ink)", maxWidth:980 }}>
          ...
          <div style={{ padding:18, fontSize:12, lineHeight:1.65 }}>
            {helpTab==="guide" && <div><h3 style={{ fontFamily:"'Archivo',sans-serif", marginTop:0 }}>Daily user guide</h3><p><b>Tracker</b> is the live sheet. Click once to select, double-click or press F2 to edit, and use Ctrl/Cmd+C / Ctrl/Cmd+V for Excel-like copy-paste.</p><p><b>Saved views</b> quickly filter the sheet for My Overdue, Due This Week, Buyer Approval Pending, Fabric Pending, PP Pending, Delivery Risk, Followed Styles, Rework, and Released.</p><p><b>Review</b> shows recent changes, all comments, comments involving you, and browser-session errors.</p><p><b>Upload styles</b> uses Order No + Style No as the unique key and asks you to confirm differences before applying.</p></div>}
            {helpTab==="faq" && <div><h3 style={{ fontFamily:"'Archivo',sans-serif", marginTop:0 }}>FAQ</h3><p><b>Why is a cell amber?</b> The activity is pending/upcoming or waiting on an action.</p><p><b>Why is a cell red?</b> It is overdue, rejected, or creating delivery risk.</p><p><b>Can duplicate style numbers exist?</b> Yes only when the Order No is different. The app treats Order No + Style No as the unique key.</p><p><b>Can I change stage owners?</b> Management/Senior can change default stage owners in Settings. This updates Chase/To-Do responsibility.</p><p><b>Does Help change data?</b> No. Help is read-only.</p></div>}
            {helpTab==="entry" && <div><h3 style={{ fontFamily:"'Archivo',sans-serif", marginTop:0 }}>Entry guide</h3><p><b>Actual date:</b> double-click a stage cell and enter the date when the activity is actually completed/sent/approved.</p><p><b>Revised date:</b> use revised dates when plan changes but the actual is not completed yet.</p><p><b>Reject:</b> use rejection only on approval stages. The tracker will push the related resend/rework stage forward.</p><p><b>Waive/Skip:</b> use skip only when the activity is formally not required. Do not use it to hide late work.</p><p><b>Comments:</b> comment on the exact cell. Use @Name to involve someone.</p></div>}
            {helpTab==="logic" && <div><h3 style={{ fontFamily:"'Archivo',sans-serif", marginTop:0 }}>Logic guide</h3><p>The tracker calculates plan dates using working days and ignores Sundays.</p><p>Date priority is <b>Actual &gt; Skip &gt; Revised &gt; Auto Plan</b>. Once actual is entered, it becomes the effective date for downstream stages.</p><p>Production File follows PP Approval when PP is required. If PP is bypassed or not needed, Production File follows Fabric In-House.</p><p>Chase is calculated from the current actionable pending stages. Stage owner defaults are editable in Settings and drive Chase/To-Do owner buckets.</p></div>}
          </div>
        </div>
      </div>)}
      ...
      {bulkOpen && (<div onClick={()=>{ setBulkOpen(false); setBulkResult(null); setUploadSkip(new Set()); }} style={{ position:"fixed", inset:0, background:"rgba(26,26,26,0.55)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
        <div onClick={e=>e.stopPropagation()} style={{ background:"var(--bg)", border:"2px solid var(--ink)", boxShadow:"8px 8px 0 var(--ink)", width:700, maxWidth:"100%", maxHeight:"86vh", overflowY:"auto", padding:22, fontFamily:"'JetBrains Mono',monospace" }}>
          ...
          {bulkResult && bulkResult.mapping && !bulkResult.error && (<div style={{ marginBottom:12, border:"1px solid var(--ink)", background:"var(--surface)", padding:12 }}>
            ...
            <div style={{ display:"flex", gap:8, marginTop:12 }}><button onClick={()=>buildUploadPreview(bulkResult)} style={{ fontFamily:"inherit", fontSize:11, fontWeight:700, padding:"7px 12px", cursor:"pointer", border:"1px solid var(--ink)", background:"var(--accent)" }}>Continue preview</button><button onClick={()=>setBulkResult(null)} style={{ fontFamily:"inherit", fontSize:11, padding:"7px 12px", cursor:"pointer", border:"1px solid var(--ink)", background:"var(--surface)" }}>Clear file</button></div>
          </div>)}
            ...
            {rows.map(({s,c},rowIdx)=>{ const t=TONE_STYLE[c.tone]; const sk=cellKey(s.id,"__style"); const styBg=bgFor(s.id,"__style","var(--surface)"); return (
              <tr key={s.id} role="row">
                ...
                {visInfo.map(col=>{
                  ...
                  else if(col.key==="chase") content=(!c.chaseOwners||c.chaseOwners.length===0)?<span style={{color:"var(--line-2)"}}>—</span>:(<span style={{ display:"flex", gap:3, flexWrap:"wrap" }}>{c.chaseOwners.map(o=>{ const names=roleNamesForChase(o.owner,s); const txt=(names&&names.length?names.slice(0,2).join("/")+(names.length>2?"+":""):o.owner)+(o.count>1?` ×${o.count}`:""); return <span key={o.owner} title={`${chaseDisplay(o.owner,s)} · ${o.count} open ${o.count>1?"branches":"branch"}`} style={{ fontSize:9, fontWeight:700, padding:"2px 5px", background:(OWNER_COLOR[o.owner]||"var(--muted-2)")+"22", color:OWNER_COLOR[o.owner]||"var(--muted-4)", whiteSpace:"nowrap" }}>{txt}</span>; })}</span>);
                  ...
                  return (
                    <td key={st.key} id={`cell-${s.id}-${st.key}`} onClick={(e)=>onCellClick(e,s.id,st.key)} onDoubleClick={(e)=>{ e.stopPropagation(); if(editable) beginDate(s.id,st.key,"actual"); }}
                      ...
                      <div style={{ minHeight:38, padding:"5px 8px", fontSize:12.5, color:cs.actual?"var(--ink)":"var(--muted-6)" }}>
                        ...
                        ) : cs.actual ? (<span style={{ display:"flex", flexDirection:"column", lineHeight:1.25 }}><span style={{ display:"flex", alignItems:"center", gap:4 }}><Check size={11} color={OWNER_COLOR[(cfg.stageOwners&&cfg.stageOwners[st.key])||st.owner]}/>{fmt(cs.actual)}</span>{cs.histReject && <span style={{ fontSize:8, color:"#b03020", fontWeight:700 }}>↻ was REJ {fmt(cs.histReject)}</span>}</span>) : cs.rejected ? (
                          <span style={{ display:"flex", flexDirection:"column", lineHeight:1.25 }}>
...
function SettingsView({ cfg, setCfg, canEdit }){
  ...
  const setStageOwner=(k,val)=> setCfg(c=>({ ...c, stageOwners:{ ...(c.stageOwners||DEFAULT_CFG.stageOwners), [k]: val||DEFAULT_CFG.stageOwners[k] } }));
  ...
  const ownerSel=(value,onChange)=>(<select disabled={!canEdit} value={value||"Merchant"} onChange={e=>onChange(e.target.value)} style={{ width:112, fontFamily:"inherit", fontSize:11, padding:"4px 6px", border:"1px solid var(--muted-6)", outline:"none", background:canEdit?"var(--surface)":"#f3f1ec" }}>{["Merchant","CAD","Buyer","Designer","Mill"].map(o=><option key={o} value={o}>{o}</option>)}</select>);
  ...
  return (<div style={{ padding:"18px 22px", maxWidth:1100 }}>
    ...
    <div style={{ display:"flex", gap:18, flexWrap:"wrap" }}>
      ...
      <div style={box}>
        <div style={head}>DEFAULT STAGE OWNERS</div><div style={sub}>Controls Chase / To-Do responsibility. It does not change edit permissions.</div>
        {STAGES.map(st=>(<div key={st.key} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"3px 0", fontSize:11, gap:10 }}><span>{st.label}</span>{ownerSel((cfg.stageOwners&&cfg.stageOwners[st.key])||st.owner, v=>setStageOwner(st.key,v))}</div>))}
      </div>
