...
function MerchTracker({ me, onSignOut }){
  ...
  const renderMentions=(body)=> String(body||"").split(/(@\S+)/).map((p,i)=> p.charAt(0)==="@" ? <b key={i} style={{ color:"#1d4ed8" }}>{p}</b> : <span key={i}>{p}</span>);
  ...
  const ndFieldFor=(key)=>{ const ni={ width:"100%", boxSizing:"border-box", border:"1px solid #e7dcc2", outline:"none", background:"#fff", fontFamily:"inherit", fontSize:11, padding:"3px 4px" }; const stop=e=>e.stopPropagation(); const ent=e=>{ if(e.key==="Enter") addNewStyle(); };
  ...
  const NoteTri=({k})=>{ const arr=comments[k]; if(!arr||!arr.length) return null; const un=arr.filter(x=>!x.resolved).length; const cl=un>0?"#c0392b":"#9aa0a6"; return <span title={un>0?(un+" open comment(s)"):"comments resolved"} style={{ position:"absolute", top:0, right:0, width:0, height:0, borderTop:"9px solid "+cl, borderLeft:"9px solid transparent" }}/>; };
  ...
  return (
    <div ref={gridRef} tabIndex={0} onKeyDown={onKeyDown} onCopy={handleCopy} onPaste={handlePaste} onMouseDown={(e)=>{ if(editing) return; if(e.target.closest && (e.target.closest("input")||e.target.closest("button")||e.target.closest("th"))) return; const td=e.target.closest && e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m||e.shiftKey) return; e.preventDefault(); setSel({ id:Number(m[1]), col:m[2] }); setFocus(null); selectingRef.current=true; setDragSel(true); }} onMouseUp={()=>{ if(filling){ applyFillHandle(); setFilling(false); setFillFrom(null); setFillTo(null); } if(selectingRef.current){ selectingRef.current=false; setDragSel(false); } }} onMouseOver={(e)=>{ const td=e.target.closest && e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m) return; if(filling){ setFillTo({ id:Number(m[1]), col:m[2] }); return; } if(selectingRef.current){ setFocus({ id:Number(m[1]), col:m[2] }); } }} onClick={()=>{ finishEditing(); setFillOpen(false); setColsOpen(false); setFilterCol(null); setFrOpen(false); setExpOpen(false); setBellOpen(false); }}
      ...
      <div style={{ background:"#1a1a1a", color:"#f4f0e8", padding:"14px 22px", display:"flex", alignItems:"center", justifyContent:"space-between", borderBottom:"3px solid #d97706" }}>
        <div style={{ display:"flex", flexDirection:"column", gap:3 }}><span style={{ fontFamily:"'Archivo',sans-serif", fontWeight:800, fontSize:20, letterSpacing:-0.2, lineHeight:1 }}>KOTHARI SPORTS <span style={{ fontWeight:600, fontSize:13, color:"#8f8a80" }}>AND</span> APPARELS</span><span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:9, color:"#9a958c", letterSpacing:1.2 }}>MERCH<span style={{ color:"#d97706" }}>·</span>TRACKER · PRE-PRODUCTION TRACKER</span></div>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}><span style={{ fontSize:11, color: saveState==="error"?"#e8746b":saveState==="saving"?"#d9b46a":saveState==="saved"?"#7fd1a8":"#6a665e" }}>{saveState==="error"?"⚠ save failed":saveState==="saving"?"… saving":saveState==="saved"?"● saved to cloud":"○ connected"}</span><span style={{ fontSize:11, color:"#cfc9bf", whiteSpace:"nowrap" }}>{(me&&(me.name||me.email))||""} · <b style={{ color:"#d97706" }}>{(ROLES[role]||{}).label||role}</b></span>{peers.length>0 && (<span style={{ display:"flex", alignItems:"center", gap:3 }} title={peers.map(p=>p.name).join(", ")}>{peers.slice(0,6).map(p=>(<span key={p.id} title={(p.name||"")+" · "+((ROLES[p.role]||{}).label||p.role)} style={{ width:18, height:18, borderRadius:9, background:colorFor(p.id), color:"#fff", fontSize:8, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center" }}>{initials(p.name)}</span>))}<span style={{ fontSize:9, color:"#9b958a", marginLeft:2 }}>{peers.length} here</span></span>)}<button onClick={(e)=>{ e.stopPropagation(); openHistory(); }} title="Change history" style={{ fontFamily:"inherit", fontSize:10, padding:"5px 8px", cursor:"pointer", border:"1px solid #4a463e", background:"transparent", color:"#cfc9bf", display:"inline-flex", alignItems:"center" }}><Clock size={13}/></button><span style={{ position:"relative" }}><button onClick={(e)=>{ e.stopPropagation(); setBellOpen(o=>!o); }} title="Notifications" style={{ fontFamily:"inherit", fontSize:10, padding:"5px 8px", cursor:"pointer", border:"1px solid #4a463e", background:bellOpen?"#4a463e":"transparent", color:"#cfc9bf", position:"relative", display:"inline-flex", alignItems:"center" }}><Bell size={13}/>{unreadCount>0 && <span style={{ position:"absolute", top:-5, right:-5, background:"#e8746b", color:"#fff", fontSize:8, fontWeight:700, minWidth:14, height:14, borderRadius:7, display:"flex", alignItems:"center", justifyContent:"center", padding:"0 3px" }}>{unreadCount>9?"9+":unreadCount}</span>}</button>{bellOpen && (<div onClick={e=>e.stopPropagation()} style={{ position:"absolute", top:"100%", right:0, marginTop:6, zIndex:90, background:"#fff", color:"#1a1a1a", border:"1px solid #1a1a1a", boxShadow:"4px 4px 0 #1a1a1a", width:320, maxHeight:380, display:"flex", flexDirection:"column" }}><div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 10px", borderBottom:"1px solid #eee" }}><span style={{ fontSize:11, fontWeight:700 }}>Notifications{unreadCount>0?" ("+unreadCount+" new)":""}</span><span style={{ display:"flex", gap:8 }}>{unreadCount>0 && <button onClick={markAllRead} style={{ fontSize:9, border:"none", background:"transparent", color:"#1d4ed8", cursor:"pointer", fontFamily:"inherit", fontWeight:700 }}>Mark all read</button>}{inbox.length>0 && <button onClick={clearInbox} style={{ fontSize:9, border:"none", background:"transparent", color:"#c0392b", cursor:"pointer", fontFamily:"inherit", fontWeight:700 }}>Clear</button>}</span></div><div style={{ overflowY:"auto" }}>{inbox.length===0 ? <div style={{ fontSize:10, color:"#999", padding:"14px 10px", textAlign:"center" }}>No notifications yet.</div> : inbox.map(n=>(<div key={n.id} onClick={()=>openNotif(n)} style={{ padding:"8px 10px", borderBottom:"1px solid #f2f2f2", cursor:"pointer", background:n.read?"#fff":"#fff7ec", display:"flex", gap:7, alignItems:"flex-start" }}><span style={{ width:6, height:6, borderRadius:6, marginTop:4, flex:"0 0 auto", background:n.read?"transparent":"#d97706" }}/><span style={{ flex:1 }}><span style={{ fontSize:11, lineHeight:1.35, display:"block" }}>{n.body}</span><span style={{ fontSize:9, color:"#999" }}>{tsShort(n.created_at)}</span></span></div>))}</div></div>)}</span>{canManageUsers(role) && <button onClick={(e)=>{ e.stopPropagation(); setUsersOpen(true); }} style={{ fontFamily:"inherit", fontSize:10, padding:"5px 9px", cursor:"pointer", border:"1px solid #4a463e", background:"transparent", color:"#cfc9bf" }}>Users</button>}<button onClick={(e)=>{ e.stopPropagation(); onSignOut&&onSignOut(); }} style={{ fontFamily:"inherit", fontSize:10, padding:"5px 9px", cursor:"pointer", border:"1px solid #4a463e", background:"transparent", color:"#cfc9bf" }}>Sign out</button></div>
      </div>
...
{history && (<div onClick={()=>setHistory(false)} style={{ position:"fixed", inset:0, background:"rgba(26,26,26,0.55)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}><div onClick={e=>e.stopPropagation()} style={{ background:"#fff", border:"2px solid #1a1a1a", boxShadow:"8px 8px 0 #1a1a1a", width:640, maxWidth:"100%", maxHeight:"80vh", display:"flex", flexDirection:"column", fontFamily:"'JetBrains Mono',monospace" }}><div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 14px", borderBottom:"1px solid #eee" }}><div style={{ fontFamily:"'Archivo',sans-serif", fontWeight:800, fontSize:17 }}>Change history</div><button onClick={()=>setHistory(false)} style={{ border:"1px solid #1a1a1a", background:"#f4f0e8", cursor:"pointer", fontFamily:"inherit", fontSize:11, padding:"4px 9px" }}>Close</button></div><div style={{ padding:"8px 14px", borderBottom:"1px solid #f2f2f2" }}><input value={histFilter} onChange={e=>setHistFilter(e.target.value)} placeholder="filter by style no, field, stage, or person…" style={{ width:"100%", boxSizing:"border-box", fontFamily:"inherit", fontSize:11, padding:6, border:"1px solid #1a1a1a" }}/></div><div style={{ overflowY:"auto", padding:"4px 0" }}>{auditBusy ? <div style={{ fontSize:11, color:"#999", padding:"24px", textAlign:"center" }}>Loading…</div> : (()=>{ const q=histFilter.trim().toLowerCase(); const fl=auditRows.filter(a=> !q || (a.style_no||"").toLowerCase().includes(q) || (a.field||"").toLowerCase().includes(q) || (a.actor_name||"").toLowerCase().includes(q) || (colLabelOf(a.col)||"").toLowerCase().includes(q)); if(fl.length===0) return <div style={{ fontSize:11, color:"#999", padding:"24px", textAlign:"center" }}>{auditRows.length===0?"No changes recorded yet.":"No matches."}</div>; return fl.map(a=>(<div key={a.id} style={{ padding:"7px 14px", borderBottom:"1px solid #f5f5f5", fontSize:11, display:"flex", gap:10, alignItems:"baseline" }}><span style={{ color:"#999", fontSize:9, whiteSpace:"nowrap", minWidth:80 }}>{tsShort(a.created_at)}</span><span style={{ flex:1 }}><b>{a.style_no||a.style_id}</b> · {colLabelOf(a.col)} <span style={{ color:"#888" }}>({a.field})</span><br/><span style={{ color:"#c0392b" }}>{a.old_val||"—"}</span> <span style={{ color:"#888" }}>→</span> <span style={{ color:"#1f6f54" }}>{a.new_val||"—"}</span> <span style={{ color:"#999" }}>· {a.actor_name}</span></span></div>)); })()}</div></div></div>)}
      {bulkOpen && (<div onClick={()=>{ setBulkOpen(false); setBulkResult(null); }} style={{ position:"fixed", inset:0, background:"rgba(26,26,26,0.55)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
        <div onClick={e=>e.stopPropagation()} style={{ background:"#f4f0e8", border:"2px solid #1a1a1a", boxShadow:"8px 8px 0 #1a1a1a", width:560, maxWidth:"100%", maxHeight:"86vh", overflowY:"auto", padding:22, fontFamily:"'JetBrains Mono',monospace" }}>
          ...
          {bulkResult && !bulkResult.error && (<div style={{ marginBottom:12 }}>
            ...
            {bulkResult.updates.length>0 && <div style={{ maxHeight:140, overflowY:"auto", border:"1px solid #ddd", background:"#fff", padding:8, fontSize:10 }}><div style={{ fontWeight:700, marginBottom:4 }}>Changes to existing styles:</div>{bulkResult.updates.slice(0,40).map(u=>(<div key={u.id} style={{ padding:"2px 0", borderBottom:"1px solid #f0ece3" }}><b>{u.styleNo}</b> — {Object.keys(u.chg).join(", ")}</div>))}{bulkResult.updates.length>40 && <div style={{ color:"#999" }}>…and {bulkResult.updates.length-40} more</div>}</div>}{bulkResult.dupes&&bulkResult.dupes.length>0 && <div style={{ fontSize:10, color:"#c0392b", fontWeight:700, marginTop:8, background:"#fbeaea", border:"1px solid #e8b4ae", padding:"6px 8px" }}>⚠ {bulkResult.dupes.length} row(s) repeat the same Order + Style in this file — skipped, no duplicates created.</div>}{bulkResult.noOrder && <div style={{ fontSize:9, color:"#a06000", marginTop:6 }}>No "Order No" column found — matched by Style No only. Add an Order No column to match on Order + Style.</div>}
          </div>)}
      ...
      <div style={{ display:"flex", gap:10, alignItems:"center", padding:"12px 22px 6px", flexWrap:"wrap" }}>
        ...
        <button onClick={(e)=>{ e.stopPropagation(); setFollowFilter(v=>!v); }} title="show only styles you follow" style={{ fontFamily:"inherit", fontSize:11, padding:"6px 11px", cursor:"pointer", border:"1px solid #1a1a1a", background:followFilter?"#d97706":"#fff", color:followFilter?"#fff":"#1a1a1a", display:"inline-flex", alignItems:"center", gap:5 }}><Star size={12} fill={followFilter?"#fff":"none"}/> Following</button>
        ...
        <span style={{ position:"relative" }}><button onClick={(e)=>{ e.stopPropagation(); setExpOpen(o=>!o); }} title="export options" style={{ fontFamily:"inherit", fontSize:11, padding:"6px 11px", cursor:"pointer", border:"1px solid #1a1a1a", background:expOpen?"#1a1a1a":"#fff", color:expOpen?"#f4f0e8":"#1a1a1a", fontWeight:700, display:"flex", alignItems:"center", gap:6 }}>⬇ Export</button>{expOpen && (<div onClick={e=>e.stopPropagation()} style={{ position:"absolute", top:"100%", left:0, marginTop:4, zIndex:75, background:"#fff", border:"1px solid #1a1a1a", boxShadow:"4px 4px 0 #1a1a1a", padding:12, width:288 }}><div style={{ fontSize:11, fontWeight:700, marginBottom:8 }}>Export {rows.length} filtered styles</div>{[["full","Full \u2014 all columns (actual dates)"],["detail","Detailed summary \u2014 everything (toggles + every stage)"],["release","Release plan \u2014 for production (incl. blockers)"],["internal","Internal plan (with buffer)"],["buyer","Buyer view \u2014 key details (printable)"]].map(([v,lbl])=>(<label key={v} style={{ display:"flex", gap:6, fontSize:10, padding:"3px 0", cursor:"pointer", alignItems:"flex-start" }}><input type="radio" checked={expMode===v} onChange={()=>setExpMode(v)} style={{ marginTop:1 }}/>{lbl}</label>))}{expMode==="release" && (<div style={{ display:"flex", gap:6, margin:"4px 0 2px" }}>{[["detailed","Detailed"],["summary","Summary"]].map(([v,l])=>(<button key={v} onClick={()=>setExpRelMode(v)} style={{ flex:1, fontFamily:"inherit", fontSize:10, fontWeight:700, padding:"4px 0", cursor:"pointer", border:"1px solid #1a1a1a", background:expRelMode===v?"#1a1a1a":"#fff", color:expRelMode===v?"#f4f0e8":"#1a1a1a" }}>{l}</button>))}</div>)}{expMode==="buyer" && (<label style={{ display:"flex", gap:6, fontSize:10, padding:"4px 0", cursor:"pointer", alignItems:"flex-start", borderTop:"1px solid #eee", marginTop:4, paddingTop:6 }}><input type="checkbox" checked={expIncBuf} onChange={e=>setExpIncBuf(e.target.checked)} style={{ marginTop:1 }}/>Include internal buffered plan dates <span style={{ color:"#999" }}>(for your team only)</span></label>)}{(expMode==="internal"||(expMode==="buyer"&&expIncBuf)) && (<div style={{ fontSize:10, color:"#555", margin:"6px 0 4px", lineHeight:1.5, background:"#f7f4ee", border:"1px solid #e6e0d4", padding:"6px 8px" }}>Add buffer to internal plan dates of <input type="number" min={0} max={30} value={expBuf} onChange={e=>setExpBuf(Math.max(0,Math.min(30,Number(e.target.value)||0)))} style={{ width:40, fontFamily:"inherit", fontSize:11, padding:"2px 4px", border:"1px solid #1a1a1a", margin:"0 4px" }}/> working days.<div style={{ color:"#999", marginTop:4 }}>Buyer approvals (Fit / Art / S-O / Lab / PP Appr) and all actual dates stay unchanged.</div></div>)}<div style={{ display:"flex", gap:8, marginTop:10 }}><button onClick={()=>{ runExport(expMode,expBuf,expIncBuf,expRelMode); setExpOpen(false); }} style={{ flex:1, fontFamily:"inherit", fontSize:11, fontWeight:700, padding:6, cursor:"pointer", border:"1px solid #1a1a1a", background:"#d97706" }}>⬇ Export .xlsx</button><button onClick={()=>setExpOpen(false)} style={{ fontFamily:"inherit", fontSize:11, padding:"6px 10px", cursor:"pointer", border:"1px solid #1a1a1a", background:"#f4f0e8" }}><X size={12}/></button></div></div>)}</span>
      ...
      <div style={{ display:"flex", gap:10, alignItems:"center", padding:"0 22px 10px", flexWrap:"wrap", fontSize:10, color:"#777" }}>
        ...
        <span style={{ marginLeft:10, position:"relative" }}>
          <button onClick={(e)=>{ e.stopPropagation(); if(sel){ setThreadCell(sel); setCmText(""); } }} disabled={!sel} style={{ fontFamily:"inherit", fontSize:10, padding:"4px 9px", cursor:sel?"pointer":"not-allowed", border:"1px solid #1a1a1a", background:"#fff", display:"inline-flex", alignItems:"center", gap:5, opacity:sel?1:0.4 }}><MessageSquare size={12}/> {sel&&(comments[cellKey(sel.id,sel.col)]||[]).length?("comments ("+(comments[cellKey(sel.id,sel.col)]||[]).length+")"):"comment"}</button><button onClick={(e)=>{ e.stopPropagation(); if(sel) openHistory(sel.id,sel.col); }} disabled={!sel} title="Change history for this cell" style={{ fontFamily:"inherit", fontSize:10, padding:"4px 9px", cursor:sel?"pointer":"not-allowed", border:"1px solid #1a1a1a", background:"#fff", display:"inline-flex", alignItems:"center", gap:5, opacity:sel?1:0.4 }}><Clock size={12}/> history</button><button onClick={(e)=>{ e.stopPropagation(); if(sel) toggleFollow(sel.id); }} disabled={!sel} title="Get notified when this style changes" style={{ fontFamily:"inherit", fontSize:10, padding:"4px 9px", cursor:sel?"pointer":"not-allowed", border:"1px solid #1a1a1a", background:(sel&&follows.has(sel.id))?"#fff3df":"#fff", color:(sel&&follows.has(sel.id))?"#b45309":"#1a1a1a", display:"inline-flex", alignItems:"center", gap:5, opacity:sel?1:0.4 }}><Star size={12} fill={(sel&&follows.has(sel.id))?"#d97706":"none"}/> {sel&&follows.has(sel.id)?"following":"follow"}</button>
          {threadCell && (()=>{ const ck=cellKey(threadCell.id,threadCell.col); const list=(comments[ck]||[]).slice().sort((a,b)=>new Date(a.created_at)-new Date(b.created_at)); const anyOpen=list.some(x=>!x.resolved); const mq=(cmText.match(/@(\S*)$/)||[])[1]; const sugg = mq!=null ? team.filter(t=>String(t.name||t.email||"").toLowerCase().includes(mq.toLowerCase())).slice(0,5) : [];
                return (<span onClick={e=>e.stopPropagation()} style={{ position:"absolute", top:"100%", left:0, marginTop:4, zIndex:85, background:"#fff", border:"1px solid #1a1a1a", boxShadow:"4px 4px 0 #1a1a1a", padding:10, width:290, display:"block" }}>
                  <div style={{ fontSize:10, fontWeight:700, marginBottom:6, display:"flex", justifyContent:"space-between", alignItems:"center" }}><span>Comments · {cellRef(threadCell)}</span>{list.length>0 && <button onClick={()=>resolveThread(threadCell)} style={{ fontSize:9, border:"1px solid #1a1a1a", background:anyOpen?"#1f6f54":"#eee", color:anyOpen?"#fff":"#888", cursor:"pointer", padding:"1px 7px", fontWeight:700 }}>{anyOpen?"Resolve":"Resolved \u2713"}</button>}</div>
                  <div style={{ maxHeight:170, overflowY:"auto", marginBottom:6 }}>{list.length===0 && <div style={{ fontSize:10, color:"#999" }}>No comments yet. Use @ to mention a teammate.</div>}{list.map(cm=>(<div key={cm.id} style={{ marginBottom:7, opacity:cm.resolved?0.5:1 }}><div style={{ fontSize:9, color:"#888", display:"flex", justifyContent:"space-between", alignItems:"center" }}><span><b style={{ color:colorFor(cm.author_id||cm.author_name) }}>{cm.author_name||"?"}</b> · {tsShort(cm.created_at)}</span>{(cm.author_id===(me&&me.id)||canAdmin(role)) && <button onClick={()=>deleteComment(cm)} title="Delete comment" style={{ border:"none", background:"transparent", cursor:"pointer", color:"#c0392b", padding:0, display:"inline-flex", alignItems:"center" }}><Trash2 size={11}/></button>}</div><div style={{ fontSize:11, whiteSpace:"pre-wrap", wordBreak:"break-word" }}>{renderMentions(cm.body)}</div></div>))}</div>
                  <div style={{ position:"relative" }}><textarea autoFocus value={cmText} onChange={e=>setCmText(e.target.value)} onKeyDown={e=>{ e.stopPropagation(); if(e.key==="Enter"&&(e.metaKey||e.ctrlKey)){ e.preventDefault(); postComment(); } }} placeholder="Comment…  @ to mention" style={{ width:"100%", height:46, fontFamily:"inherit", fontSize:11, border:"1px solid #ccc", outline:"none", resize:"none", boxSizing:"border-box" }}/>
                  {sugg.length>0 && <div style={{ position:"absolute", bottom:"100%", left:0, right:0, background:"#fff", border:"1px solid #1a1a1a", maxHeight:110, overflowY:"auto", zIndex:5 }}>{sugg.map(t=>(<div key={t.id} onClick={()=>setCmText(cmText.replace(/@(\S*)$/, "@"+String(t.name||t.email||"").replace(/\s+/g,"")+" "))} style={{ fontSize:10, padding:"4px 7px", cursor:"pointer", borderBottom:"1px solid #f0ece2" }}>{t.name||t.email} <span style={{ color:"#999" }}>· {(ROLES[t.role]||{}).label||t.role}</span></div>))}</div>}</div>
                  <div style={{ display:"flex", gap:6, marginTop:6 }}><button onClick={postComment} style={{ ...chip, flex:1, background:"#d97706" }}>Post (⌘↵)</button><button onClick={()=>{ setThreadCell(null); setCmText(""); }} style={chip}>Close</button></div>
                </span>); })()}
        </span>
...
function LoginScreen(){
  ...
  return (<AuthShell>
    <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:10, letterSpacing:1.5, color:"#d97706", fontWeight:700, marginBottom:6 }}>KOTHARI SPORTS AND APPARELS</div>
    ...
    <button type="submit" disabled={busy} style={{ width:"100%", fontFamily:"inherit", fontSize:13, fontWeight:700, padding:"10px", cursor:busy?"wait":"pointer", border:"1px solid #1a1a1a", background:"#1a1a1a", color:"#f4f0e8", marginBottom:10 }}>{busy?"…":(mode==="in"?"Sign in":"Create account")}</button>
    </form>
    <div style={{ fontSize:11, textAlign:"center" }}><span style={{ color:"#888" }}>{mode==="in"?"New here? ":"Have an account? "}</span><button type="button" onClick={()=>{ setMode(mode==="in"?"up":"in"); setMsg(""); }} style={{ border:"none", background:"transparent", color:"#d97706", cursor:"pointer", fontFamily:"inherit", fontSize:11, fontWeight:700 }}>{mode==="in"?"Create account":"Sign in"}</button></div>
