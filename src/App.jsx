...
function MerchTracker({ me, onSignOut }){
  const APP_BUILD_LABEL="v31dp Build 18 Password Recovery";
  ...
  return (
    <div ref={gridRef} tabIndex={0} onKeyDown={onKeyDown} onCopy={handleCopy} onPaste={handlePaste} onContextMenu={(e)=>{ const td=e.target.closest&&e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m) return; e.preventDefault(); const id=Number(m[1]), col=m[2]; if(!selKeys.has(`${id}:${col}`)) setSingleSelection(id,col); setCtxMenu({ x:e.clientX, y:e.clientY, id, col }); }} onMouseDown={(e)=>{ if(editing) return; if(e.button===2) return; if(e.target.closest && (e.target.closest("input")||e.target.closest("button")||e.target.closest("th"))) return; const td=e.target.closest && e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m) return; e.preventDefault(); gridRef.current&&gridRef.current.focus(); const id=Number(m[1]), col=m[2]; setCtxMenu(null); if((e.ctrlKey||e.metaKey)||e.shiftKey) return; setSel({ id, col }); setFocus(null); setMultiAreas([]); selectingRef.current=true; setDragSel(true); }} onMouseUp={()=>{ if(filling){ applyFillHandle(); setFilling(false); setFillFrom(null); setFillTo(null); } if(selectingRef.current){ selectingRef.current=false; setDragSel(false); } }} onMouseOver={(e)=>{ const td=e.target.closest && e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m) return; if(filling){ setFillTo({ id:Number(m[1]), col:m[2] }); return; } if(selectingRef.current){ setFocus({ id:Number(m[1]), col:m[2] }); } }} onClick={()=>{ finishEditing(); setCtxMenu(null); setFillOpen(false); setColsOpen(false); setFilterCol(null); setFrOpen(false); setExpOpen(false); setBellOpen(false); setPeersOpen(false); }}
      ...
      {usersOpen && <UsersPanel me={me} onClose={()=>setUsersOpen(false)}/>}
...
/* ========================= DAILY UPDATES ========================= */
function DailyUpdatesView({ computed, todoItems, role, me, team, onJump, onOpenTodo }){
  ...
  const taskMerchantKey=(t)=>{ const names=[]; if(t&&t.junior) names.push(t.junior); if(t&&Array.isArray(t.juniors)) names.push(...t.juniors); for(const name of names){ const n=norm(name); const p=merchantProfiles.find(x=>{ const a=norm(x.name),b=norm(x.email),c=norm(String(x.email||"").split("@")[0]); return n&&(n===a||n===b||n===c||(n.length>=4&&(a.includes(n)||n.includes(a)))); }); if(p) return profileKey(p); } return names[0]?`name:${norm(names[0])}`:""; };
  ...
  const summaryFor=(key)=>{ const rows=meaningfulRows.filter(r=>r.actorKey===key), comments=commentRows.filter(r=>r.actorKey===key), corrections=correctionRows.filter(r=>r.actorKey===key), sub=submissionFor(key), last=lastWorkAt(key); return { key,name:displayName(key,rows[0]&&rows[0].actorName),actual:rows.filter(r=>r.kind==="actual").length,revised:rows.filter(r=>r.kind==="revised").length,movedLater:rows.filter(r=>r.kind==="revised"&&r.movedLater).length,rejected:rows.filter(r=>r.kind==="rejected").length,waived:rows.filter(r=>r.kind==="waived").length,meaningful:rows.length,comments:comments.length,corrections:corrections.length,untouched:untouchedFor(key).length,submission:sub,changedAfter:!!(sub&&last>new Date(sub.submittedAt||0).getTime())}; };
  ...
  const visibleSummary=selectedKey==="All"?summaries.reduce((m,r)=>{ m.actual+=r.actual;m.revised+=r.revised;m.movedLater+=r.movedLater;m.meaningful+=r.meaningful;m.comments+=r.comments;m.corrections+=r.corrections;m.untouched+=r.untouched;return m; },{actual:0,revised:0,movedLater:0,meaningful:0,comments:0,corrections:0,untouched:0}):(summaries.find(r=>r.key===selectedKey)||{actual:0,revised:0,movedLater:0,meaningful:0,comments:0,corrections:0,untouched:0});
  ...
  const submitMyDay=async()=>{
    ...
    try{
      ...
      const entry={ date,userId:me&&me.id||null,userName:(me&&(me.name||me.email))||"",submittedAt:new Date().toISOString(),blockers:blockers.trim(),helpNeeded:helpNeeded.trim(),tomorrow:tomorrow.trim(),snapshot:{meaningful:myRows.length,actual:myRows.filter(r=>r.kind==="actual").length,revised:myRows.filter(r=>r.kind==="revised").length,movedLater:lateMoves,comments:commentRows.filter(r=>r.actorKey===myKey).length,corrections:correctionRows.filter(r=>r.actorKey===myKey).length,untouched:untouchedFor(myKey).length}};
  ...
  return <div style={{padding:"18px 22px 36px"}}>
    ...
    <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",padding:"10px 12px",background:"var(--toolbar-subtle)",border:"1px solid var(--line-2)",borderRadius:10,marginBottom:10}}><label style={{fontSize:10,fontWeight:900}}>Report date <input type="date" value={date} max={iso(TODAY)} onChange={e=>setDate(e.target.value)} style={{...inputStyle,marginLeft:5}}/></label>{role!=="junior"&&<select value={selectedUser} onChange={e=>setSelectedUser(e.target.value)} style={inputStyle}><option value="All">All merchants</option>{merchantProfiles.map(p=><option key={profileKey(p)} value={profileKey(p)}>{p.name||p.email}</option>)}</select>}<button onClick={loadDay} disabled={busy} style={{...inputStyle,fontWeight:900,cursor:"pointer"}}>{busy?"Loading…":"Refresh"}</button><span style={{fontSize:9,color:"var(--muted-2)"}}>IST day · complete selected-day audit query, not the 300-row Entry Log display limit</span></div>
    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>{card("Meaningful updates",visibleSummary.meaningful,"var(--ink)","final/net outcomes")}{card("Actual completed",visibleSummary.actual,"var(--success)","Revised/Auto → Actual")}{card("Net revised",visibleSummary.revised,"var(--revised)","one final commitment")}{card("Moved later",visibleSummary.movedLater,visibleSummary.movedLater?"var(--danger)":"var(--success)","reason required")}{card("Chase comments",visibleSummary.comments,"var(--info)","outcomes recorded")}{card("Due untouched",todaySelected?visibleSummary.untouched:"—",visibleSummary.untouched?"var(--danger)":"var(--success)",todaySelected?"open in To-Do":"current-only metric")}{card("Corrections",visibleSummary.corrections,visibleSummary.corrections?"#b45309":"var(--muted-2)","not counted as work")}</div>
    {role!=="junior"&&<div style={{background:"var(--surface)",border:"1px solid var(--line-2)",borderRadius:12,overflowX:"auto",marginBottom:12}}><table style={{width:"100%",minWidth:980,borderCollapse:"collapse",fontSize:10}}><thead><tr style={{background:"var(--ink)",color:"var(--bg)"}}>{["Merchant","Submission","Meaningful","Actual","Net revised","Moved later","Comments","Due untouched","Corrections"].map(h=><th key={h} style={{padding:"8px 9px",textAlign:"left",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>{summaries.map(r=><tr key={r.key} onClick={()=>setSelectedUser(r.key)} style={{borderBottom:"1px solid var(--line-3)",cursor:"pointer",background:selectedKey===r.key?"var(--accent-tint)":"transparent"}}><td style={{padding:"8px 9px",fontWeight:950,color:"var(--info)"}}>{r.name} →</td><td style={{padding:"8px 9px",fontWeight:900,color:!r.submission?"var(--danger)":r.changedAfter?"#b45309":"var(--success)"}}>{!r.submission?"Not submitted":r.changedAfter?"Changes after submission":"Submitted"}</td><td style={{padding:"8px 9px"}}>{r.meaningful}</td><td style={{padding:"8px 9px",color:"var(--success)",fontWeight:850}}>{r.actual}</td><td style={{padding:"8px 9px"}}>{r.revised}</td><td style={{padding:"8px 9px",color:r.movedLater?"var(--danger)":"var(--muted-2)",fontWeight:850}}>{r.movedLater}</td><td style={{padding:"8px 9px"}}>{r.comments}</td><td style={{padding:"8px 9px",color:r.untouched?"var(--danger)":"var(--muted-2)",fontWeight:850}}>{todaySelected?r.untouched:"—"}</td><td style={{padding:"8px 9px",color:r.corrections?"#b45309":"var(--muted-2)"}}>{r.corrections}</td></tr>)}</tbody></table></div>}
...
function UsersPanel({ me, onClose }){
  ...
  return (<div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(26,26,26,0.55)", zIndex:400, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
    <div onClick={e=>e.stopPropagation()} style={{ background:"var(--bg)", border:"2px solid var(--ink)", boxShadow:"8px 8px 0 var(--ink)", width:760, maxWidth:"100%", maxHeight:"88vh", overflowY:"auto", padding:22, fontFamily:"'JetBrains Mono',monospace" }}>
      ...
      <p style={{ fontSize:11, color:"var(--muted-3)", lineHeight:1.55, marginBottom:12 }}>Management can set access roles and directly set a new sign-in password. No email reset link is sent, and the password itself is never saved in tracker history.</p>
