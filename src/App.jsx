...
function ReviewTabView({ computed, todoItems, auditRows, auditBusy, loadAuditRows, errorLog, comments, me, colLabelOf, onJump }){
  ...
  const items=useMemo(()=>{
    ...
    Object.entries(comments||{}).forEach(([ck,arr])=>{ const [sid,col]=ck.split(":"); const st=stylesById.get(String(sid)); (arr||[]).slice(-3).forEach(c=> push({ id:`comment-${c.id||sid+col}`, category:reviewCategoryForCol(col), severity:(c.resolved?"Info":"Warning"), status:c.resolved?"Resolved":"Open", styleNo:(st&&st.styleNo)||c.style_no||sid, orderNo:(st&&st.orderNo)||"", buyer:(st&&st.buyer)||"", brand:(st&&st.brand)||"", issue:`Comment: ${String(c.body||"").slice(0,120)}`, action:"Open comment thread", source:"Comment", at:c.created_at, jumpId:Number(sid), jumpCol:col })); });
  ...
  },[computed,todoItems,auditRows,errorLog,comments,stylesById,colLabelOf]);
  const categories=useMemo(()=>["All",...Array.from(new Set(items.map(x=>x.category))).sort((a,b)=>a.localeCompare(b))],[items]);
  ...
  const counts=useMemo(()=>({ total:items.length, critical:items.filter(x=>x.severity==="Critical").length, warning:items.filter(x=>x.severity==="Warning").length, dq:items.filter(x=>x.category==="Data Quality").length }),[items]);
  ...
  return <div style={{ padding:"18px 22px 36px" }}>
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:16, marginBottom:14 }}>
      <div><div style={{ fontFamily:"'Archivo',sans-serif", fontWeight:800, fontSize:24 }}>Review</div><div style={{ fontSize:11.5, color:"var(--muted-3)", marginTop:4, maxWidth:820, lineHeight:1.45 }}>Category-wise review inbox. This is read-only v1: it summarizes pending work, data quality, comments, recent changes and browser errors by business category such as Order, Colour, Fit, Fabric and PP.</div></div>
    ...
    <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center", marginBottom:12, background:"var(--toolbar-bg)", border:"1px solid var(--toolbar-line)", borderRadius:12, padding:10 }}>
      <select value={category} onChange={e=>setCategory(e.target.value)} style={{ fontFamily:"inherit", fontSize:11, padding:"7px 9px", border:"1px solid var(--ink)", background:"var(--surface)" }}>{categories.map(c=><option key={c} value={c}>{c}</option>)}</select>
      <select value={severity} onChange={e=>setSeverity(e.target.value)} style={{ fontFamily:"inherit", fontSize:11, padding:"7px 9px", border:"1px solid var(--ink)", background:"var(--surface)" }}>{["All","Critical","Warning","Info"].map(c=><option key={c} value={c}>{c}</option>)}</select>
      <input value={q} onChange={e=>setQ(e.target.value)} placeholder="search style, order, buyer, issue…" style={{ flex:"1 1 260px", fontFamily:"inherit", fontSize:11, padding:"7px 9px", border:"1px solid var(--ink)", background:"var(--surface)" }} />
      <span style={{ fontSize:10, color:"var(--muted-2)" }}>{filtered.length} shown</span>
    </div>
