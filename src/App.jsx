...
function MerchTracker({ me, onSignOut }){
  const APP_BUILD_LABEL="v31dm Build 9 crisp buyer TNA";
  ...
  const [expOpen,setExpOpen]=useState(false); const [expMode,setExpMode]=useState(()=>{ try{ return localStorage.getItem("mt_exp_mode")||"full"; }catch(e){ return "full"; } }); const [expBuf,setExpBuf]=useState(()=>{ try{ const v=parseInt(localStorage.getItem("mt_exp_buf"),10); return (isFinite(v)&&v>=0&&v<=30)?v:2; }catch(e){ return 2; } }); const [expBuyerPlans,setExpBuyerPlans]=useState(()=>{ try{ return localStorage.getItem("mt_exp_buyer_plans")!=="0"; }catch(e){ return true; } }); const [expRelMode,setExpRelMode]=useState(()=>{ try{ return localStorage.getItem("mt_exp_relmode")||"detailed"; }catch(e){ return "detailed"; } });
  ...
  useEffect(()=>{ try{ localStorage.setItem("mt_exp_mode",expMode); localStorage.setItem("mt_exp_buf",String(expBuf)); localStorage.setItem("mt_exp_buyer_plans",expBuyerPlans?"1":"0"); localStorage.setItem("mt_exp_relmode",expRelMode); localStorage.setItem("mt_fr_scope",frScope); localStorage.setItem("mt_fr_case",frCase?"1":"0"); }catch(e){} },[expMode,expBuf,expBuyerPlans,expRelMode,frScope,frCase]);
  ...
  const runExport=(mode,buf,buyerPlans,relMode)=>{ try{ let data, name, sheet="Tracker"; const B=Math.abs(buf||0);
    ...
    const buyerDateCell=(s,c,k,includePlans)=>{ const r=stageRow(c,k); if(!r) return ""; const a=activeActualDate(s,c,k); if(a) return fmtY(a); if(!includePlans||r.skipped||r.autoClosed) return ""; const due=r.rev||r.plan; return due?fmtY(due):""; };
    ...
    const buyerCurrentActivity=(s,c)=>{ if(c.released) return "Released"; let pending=(c.frontier?[...c.frontier]:[]).map(k=>stageRow(c,k)).filter(r=>r&&!r.done&&!r.autoClosed&&buyerPredReady(s,c,r.key)); if(!pending.length&&c.nextPending&&!c.nextPending.done&&!c.nextPending.autoClosed) pending=[c.nextPending]; pending.sort((a,b)=>stageOrderOf(a.key)-stageOrderOf(b.key)); const labels=pending.map(r=>{ const label=buyerStageLabel(r.key); if(r.rework) return `${label} — redo & resend`; if(r.rejected) return `${label} — rejected / re-approval`; const due=r.rev||r.plan; if(due&&TODAY>due) return `${label} — overdue`; return `${label} — pending`; }); return labels.join(" + ")||"Pending update"; };
    ...
    if(mode==="buyer"){ const buyerStages=STAGES.filter(st=>rows.some(({c})=>!!stageRow(c,st.key))); data=rows.map(({s,c})=>{ const buyerBrand=[s.buyer,s.brand].filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i).join(" / "); const o={ "Order No":s.orderNo||"", "Style No":s.styleNo||"", "Colour":s.colour||"", "Buyer / Brand":buyerBrand, "Qty":s.qty||0, "Delivery":fmtIso(s.delivery), "Current Activity / Stuck At":buyerCurrentActivity(s,c), "Delivery Risk":buyerRisk(s,c), "Last Completed Activity":buyerLastCompleted(c) }; buyerStages.forEach(st=>{ o[buyerStageLabel(st.key)]=buyerDateCell(s,c,st.key,buyerPlans); }); o["Remarks"]=s.remarks||""; return o; }); name=buyerPlans?"buyer_tna_progress":"buyer_tna_actuals_only"; sheet="Main Data"; }
    ...
    const ws=XLSX.utils.json_to_sheet(data); if(mode==="buyer"&&data&&data.length){ const headers=Object.keys(data[0]); ws["!autofilter"]={ ref:XLSX.utils.encode_range({ s:{r:0,c:0}, e:{r:data.length,c:Math.max(0,headers.length-1)} }) }; const widthOf=(h)=>h==="Current Activity / Stuck At"?38:h==="Last Completed Activity"?30:h==="Remarks"?42:h==="Colour"||h==="Buyer / Brand"?22:h==="Style No"?20:h==="Order No"?16:h==="Delivery Risk"?15:h==="Qty"?10:16; ws["!cols"]=headers.map(h=>({wch:widthOf(h)})); ws["!rows"]=[{hpt:22}]; ws["!margins"]={ left:0.25, right:0.25, top:0.5, bottom:0.5, header:0.2, footer:0.2 }; ws["!pageSetup"]={ orientation:"landscape", fitToWidth:1, fitToHeight:0 }; }
