...
function MerchTracker({ me, onSignOut }){
  ...
  const runExport=(mode,buf,incBuf)=>{ try{ let data, name, sheet="Tracker"; const B=Math.abs(buf||0);
    ...
    else if(mode==="release"){ data=rows.map(({s,c})=>{ const stg=k=>{ const r=(c.stages||[]).find(x=>x.key===k); if(!r) return ""; const a=(s.actuals&&s.actuals[k])?parse(s.actuals[k]):null; return a?fmt(a):((r.rev||r.plan)?fmt(r.rev||r.plan):(r.skipped?"WAIVED":"")); }; const open=(c.frontier?[...c.frontier]:[]).map(k=>{ const r=(c.stages||[]).find(x=>x.key===k); if(!r||r.done) return null; const lbl=r.label; if(r.rework) return lbl+": redo & resend"; if(r.rejected) return lbl+": rejected"; if(REJECTABLE.includes(k)) return lbl+": not approved"; return lbl+": pending"; }).filter(Boolean); return { "Order No":s.orderNo, "Style No":s.styleNo, "Colour":s.colour, "Qty":s.qty, "Fabric IH":stg("fabricIH"), "PP Appr":stg("ppAppr"), "Prod File":stg("prodFile"), "Proj. Release":c.projRelease?fmt(c.projRelease):"", "Delivery":s.delivery, "Released":c.released?"YES":"", "Pending / blockers":c.released?"\u2014 released":(open.join("; ")||"\u2014"), "Remarks":s.remarks||"", "Status":c.status }; }); name="release_plan"; sheet="Release Plan"; }
