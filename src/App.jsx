            ...
            {rows.map(({s,c},rowIdx)=>{ const t=TONE_STYLE[c.tone]; const sk=cellKey(s.id,"__style"); const styBg=bgFor(s.id,"__style","#fff"); return (
              <tr key={s.id} role="row">
                <td id={`cell-${s.id}-__style`} onClick={(e)=>onCellClick(e,s.id,"__style")} onDoubleClick={(e)=>{ e.stopPropagation(); startEdit(s.id,"__style"); }} style={{ position:"sticky", left:0, zIndex:5, background:styBg, border:"1px solid #ddd", padding:"6px 9px", overflow:"hidden", boxShadow:ringFor(s.id,"__style"), cursor:"cell" }}>
                  ...
                  <NoteTri k={sk}/>
                  {role!=="Viewer" && <button title="delete row" onClick={(e)=>{ e.stopPropagation(); deleteStyle(s.id); }} style={{ position:"absolute", top:2, right:2, zIndex:6, border:"none", background:"transparent", cursor:"pointer", padding:0, lineHeight:0, color:"#cbb4ac" }}><Trash2 size={11}/></button>}
                </td>
                ...
                {visStages.map(st=>{
                  ...
                  return (
                    <td key={st.key} id={`cell-${s.id}-${st.key}`} onClick={(e)=>onCellClick(e,s.id,st.key)} onDoubleClick={(e)=>{ e.stopPropagation(); if(editable) beginDate(s.id,st.key,"actual"); }}
                      ...
                      {editable && (!cs.actual || cs.rework) && (<button title="set revised plan date" onClick={(e)=>{ e.stopPropagation(); beginDate(s.id,st.key,"rev"); }} style={{ position:"absolute", top:3, right:3, border:"none", background:"transparent", cursor:"pointer", padding:0, lineHeight:1, display:"flex" }}><RotateCcw size={11} color="#6d4aab"/></button>)}
                      {editable && !cs.actual && REJECTABLE.includes(st.key) && (<button title={cs.rejected?"clear rejection (remove rework)":"mark REJECTED (log rejection date)"} onClick={(e)=>{ e.stopPropagation(); if(cs.rejected) setReject(s.id,st.key,null); else beginDate(s.id,st.key,"reject"); }} style={{ position:"absolute", top:3, right:20, border:"none", background:cs.rejected?"#b03020":"transparent", borderRadius:2, cursor:"pointer", padding:cs.rejected?2:0, lineHeight:1, display:"flex" }}><X size={cs.rejected?9:11} color={cs.rejected?"#fff":"#b03020"}/></button>)}
                      {cs.rework && ROLES[role].canEdit(st.owner) && (<button title="clear rework (un-reject the approval)" onClick={(e)=>{ e.stopPropagation(); setReject(s.id, APPR_OF_SEND[st.key], null); }} style={{ position:"absolute", top:3, right:20, border:"none", background:"#b03020", borderRadius:2, cursor:"pointer", padding:2, lineHeight:1, display:"flex" }}><X size={9} color="#fff"/></button>)}
...
function FillPanel({ count, onApply, onClose }){
  ...
  return (<div onClick={e=>e.stopPropagation()} style={{ position:"absolute", top:"100%", left:0, marginTop:4, zIndex:70, background:"#fff", border:"1px solid #1a1a1a", boxShadow:"4px 4px 0 #1a1a1a", padding:12, width:280 }}><div style={{ fontSize:11, fontWeight:700, marginBottom:8 }}>Set one date across {count} filtered styles</div><label style={{ fontSize:10, color:"#888" }}>Stage</label><select value={key} onChange={e=>setKey(e.target.value)} style={{ width:"100%", fontFamily:"inherit", fontSize:11, padding:5, marginBottom:8, border:"1px solid #1a1a1a" }}><option value="ordRec">Order Date</option><option value="delivery">Delivery Date</option>{STAGES.map(s=><option key={s.key} value={s.key}>{s.label}{s.cutoff?" (Fabric IH)":""}</option>)}</select><label style={{ fontSize:10, color:"#888" }}>Date</label><input type="date" value={val} onChange={e=>setVal(e.target.value)} style={{ width:"100%", fontFamily:"inherit", fontSize:11, padding:5, marginBottom:10, border:"1px solid #1a1a1a" }}/><div style={{ display:"flex", gap:8 }}><button onClick={()=>onApply(key,val)} style={{ flex:1, fontFamily:"inherit", fontSize:11, fontWeight:700, padding:6, cursor:"pointer", border:"1px solid #1a1a1a", background:"#d97706" }}>Apply to all filtered</button><button onClick={onClose} style={{ fontFamily:"inherit", fontSize:11, padding:"6px 10px", cursor:"pointer", border:"1px solid #1a1a1a", background:"#f4f0e8" }}><X size={12}/></button></div></div>);
}
