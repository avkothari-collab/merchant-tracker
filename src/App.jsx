...
function MerchTracker({ me, onSignOut }){
  ...
  const runExport=(mode,buf,incBuf)=>{ try{ let data, name, sheet="Tracker"; const B=Math.abs(buf||0);
    ...
    else if(mode==="release"){ data=rows.map(({s,c})=>{ const sd=k=>{ const r=(c.stages||[]).find(x=>x.key===k); if(!r) return "n/a"; const a=(s.actuals&&s.actuals[k])?parse(s.actuals[k]):null; if(a) return fmt(a); if(r.skipped) return "WAIVED"; if(r.rework) return "REDO"+(r.plan?" "+fmt(r.plan):""); if(r.rejected) return "REJECTED"; const t=r.rev||r.plan; if(REJECTABLE.includes(k)) return "await appr"+(t?" "+fmt(t):""); return "pending"+(t?" "+fmt(t):""); }; const open=(c.frontier?[...c.frontier]:[]).map(k=>{ const r=(c.stages||[]).find(x=>x.key===k); if(!r||r.done) return null; const lbl=r.label; if(r.rework) return lbl+": redo & resend"; if(r.rejected) return lbl+": rejected"; if(REJECTABLE.includes(k)) return lbl+": not approved"; return lbl+": pending"; }).filter(Boolean); const o={ "Order No":s.orderNo, "Style No":s.styleNo, "Colour":s.colour, "Qty":s.qty, "FIT":s.fitReq?"Y":"-", "PRT":s.printReq?"Y":"-", "S-O":s.soReq?"Y":"-", "LAB":s.labDipReq?"Y":"-", "BYP":s.ppBypass?"Y":"-", "PP":s.ppNeeded?"Y":"-" }; STAGES.forEach(st=>{ o[st.label]=sd(st.key); }); o["Proj. Release"]=c.projRelease?fmt(c.projRelease):""; o["Delivery"]=s.delivery; o["Released"]=c.released?"YES":""; o["Pending / blockers"]=c.released?"released":(open.join("; ")||"none"); o["Remarks"]=s.remarks||""; o["Status"]=c.status; return o; }); name="release_plan"; sheet="Release Plan"; }
