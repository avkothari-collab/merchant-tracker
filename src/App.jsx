...
function MerchTracker({ me, onSignOut }){
  ...
  const parseUpload=async(file)=>{ try{ const buf=await file.arrayBuffer(); const wb=XLSX.read(buf,{cellDates:true}); const sn=wb.SheetNames.includes("Tracker")?"Tracker":wb.SheetNames[0]; const aoa=XLSX.utils.sheet_to_json(wb.Sheets[sn],{header:1,raw:false,cellDates:true,defval:""}); let hr=-1; for(let i=0;i<Math.min(aoa.length,6);i++){ if((aoa[i]||[]).some(c=>{ const n=NORMH(c); return n==="styleno"||n==="style"; })){ hr=i; break; } } if(hr<0) hr=0; const headers=(aoa[hr]||[]).map(NORMH); const fi={}; headers.forEach((h,i)=>{ const f=HEADER_MAP[h]; if(f&&fi[f]==null) fi[f]=i; }); if(fi.styleNo==null){ setBulkResult({ error:"Couldn't find a 'Style No' column. Upload rejected — Style No + Order No are mandatory." }); return; } if(fi.orderNo==null){ setBulkResult({ error:"Couldn't find an 'Order No' column. Upload rejected — Style No + Order No are mandatory so we don't mix repeated styles across orders." }); return; }
    const recs=[], missing=[]; for(let i=hr+1;i<aoa.length;i++){ const row=aoa[i]||[]; const styleNo=String(row[fi.styleNo]??"").trim(); const orderNo=String(row[fi.orderNo]??"").trim(); if(!styleNo&&!orderNo) continue; if(!styleNo||!orderNo){ missing.push(i+1); continue; } const rec={ styleNo, orderNo }; ["sampleFit","family","colour","brand","fabricType","owner","setId","setRole","age"].forEach(f=>{ if(fi[f]!=null){ const v=String(row[fi[f]]??"").trim(); if(v) rec[f]=v; } }); if(fi.qty!=null){ const q=String(row[fi.qty]??"").replace(/[^0-9.]/g,""); if(q) rec.qty=Number(q)||0; } if(fi.ordRec!=null){ const d=toISO(row[fi.ordRec]); if(d) rec.ordRec=d; } if(fi.delivery!=null){ const d=toISO(row[fi.delivery]); if(d) rec.delivery=d; } ["fitReq","printReq","soReq","ppBypass","labDipReq","ppNeeded"].forEach(f=>{ if(fi[f]!=null && String(row[fi[f]]??"").trim()!=="") rec[f]=toBool(row[fi[f]]); }); recs.push(rec); }
    if(missing.length){ setBulkResult({ error:`Upload rejected: ${missing.length} row(s) are missing Style No or Order No. First bad row(s): ${missing.slice(0,8).join(", ")}.` }); return; }
    const ckey=(o,st)=> String(o||"").trim().toLowerCase()+"|"+String(st||"").trim().toLowerCase(); const byKey={}; styles.forEach(s=> byKey[ckey(s.orderNo,s.styleNo)]=s); const inserts=[], updates=[], dupes=[]; let unchanged=0; const seen=new Set();
    recs.forEach(rec=>{ const k=ckey(rec.orderNo,rec.styleNo); if(seen.has(k)){ dupes.push(rec); return; } seen.add(k); const ex=byKey[k]; if(ex){ const chg={}, diffs=[]; Object.keys(rec).forEach(f=>{ if(f==="styleNo"||f==="orderNo") return; const nv=rec[f]; if(nv===undefined||nv==="") return; if(String(ex[f]??"")!==String(nv)){ chg[f]=nv; diffs.push({ field:f, from:ex[f]??"", to:nv }); } }); if(Object.keys(chg).length) updates.push({ id:ex.id, styleNo:rec.styleNo, orderNo:rec.orderNo, chg, diffs }); else unchanged++; } else inserts.push(rec); });
    setBulkResult({ inserts, updates, unchanged, dupes, noOrder:false, sheetName:sn, total:recs.length });
  }catch(e){ logAppError("upload parse failed",e,file&&file.name); setBulkResult({ error:"Couldn't read that file: "+(e.message||e) }); } };
  const applyBulk=async()=>{ if(!bulkResult||bulkResult.error) return; const total=(bulkResult.inserts.length+bulkResult.updates.length); if(!window.confirm(`Apply ${total} upload change(s)? Existing discrepancies will be overwritten with the uploaded values.`)) return; pushHistory(); const { inserts, updates }=bulkResult;
  ...
  const freezeStyle=(col,bg)=> isFrozen(col)?{ position:"sticky", left:leftOf(col), zIndex: col==="__style"?12:11, background:bg, backgroundClip:"padding-box", ...(col===lastFrozen?{ borderRight:"2px solid var(--muted-4)", boxShadow:"2px 0 0 var(--muted-4)" }:{ boxShadow:"1px 0 0 var(--line-2)" }) }:{};
      ...
      {bulkOpen && (<div onClick={()=>{ setBulkOpen(false); setBulkResult(null); }} style={{ position:"fixed", inset:0, background:"rgba(26,26,26,0.55)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
        <div onClick={e=>e.stopPropagation()} style={{ background:"var(--bg)", border:"2px solid var(--ink)", boxShadow:"8px 8px 0 var(--ink)", width:560, maxWidth:"100%", maxHeight:"86vh", overflowY:"auto", padding:22, fontFamily:"'JetBrains Mono',monospace" }}>
          ...
          {bulkResult && !bulkResult.error && (<div style={{ marginBottom:12 }}>
            ...
            {bulkResult.updates.length>0 && <div style={{ maxHeight:180, overflowY:"auto", border:"1px solid var(--line-1)", background:"var(--surface)", padding:8, fontSize:10 }}><div style={{ fontWeight:700, marginBottom:4 }}>Review discrepancies before applying:</div>{bulkResult.updates.slice(0,40).map(u=>(<div key={u.id} style={{ padding:"4px 0", borderBottom:"1px solid #f0ece3" }}><b>{u.orderNo} · {u.styleNo}</b>{(u.diffs||[]).slice(0,5).map(d=><div key={d.field} style={{ color:"var(--muted-3)" }}>{d.field}: <span style={{ color:"var(--danger)" }}>{String(d.from||"—")}</span> → <span style={{ color:"var(--success)" }}>{String(d.to||"—")}</span></div>)}</div>))}{bulkResult.updates.length>40 && <div style={{ color:"var(--muted-1)" }}>…and {bulkResult.updates.length-40} more</div>}</div>}{bulkResult.dupes&&bulkResult.dupes.length>0 && <div style={{ fontSize:10, color:"var(--danger)", fontWeight:700, marginTop:8, background:"#fbeaea", border:"1px solid #e8b4ae", padding:"6px 8px" }}>⚠ {bulkResult.dupes.length} row(s) repeat the same Order + Style in this file — skipped, no duplicates created.</div>}
          </div>)}
