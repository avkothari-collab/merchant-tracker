...
[data-theme="clean"] {
  ...
  --line-1:#e2e5ea; --line-2:#d7dbe1; --line-3:#eef0f3;
}
`;
...
function MerchTracker({ me, onSignOut }){
  ...
  const [theme,setTheme]=useState(()=>{ try{ return localStorage.getItem("mt_theme")||"paper"; }catch(e){ return "paper"; } }); const [dispOpen,setDispOpen]=useState(false);
  useEffect(()=>{ try{ document.documentElement.setAttribute("data-theme",theme); localStorage.setItem("mt_theme",theme); }catch(e){} },[theme]);
  ...
  const runExport=(mode,buf,incBuf,relMode)=>{ try{ let data, name, sheet="Tracker"; const B=Math.abs(buf||0);
    ...
    else if(mode==="release"){ data=rows.map(({s,c})=>{ const sd=k=>{ const r=(c.stages||[]).find(x=>x.key===k); if(!r) return "n/a"; const a=(s.actuals&&s.actuals[k])?parse(s.actuals[k]):null; if(a) return fmt(a); if(r.skipped) return "WAIVED"; if(r.rework) return "REDO"+(r.plan?" "+fmt(r.plan):""); if(r.rejected) return "REJECTED"; const t=r.rev||r.plan; if(REJECTABLE.includes(k)) return "await appr"+(t?" "+fmt(t):""); return "pending"+(t?" "+fmt(t):""); }; const open=(c.frontier?[...c.frontier]:[]).map(k=>{ const r=(c.stages||[]).find(x=>x.key===k); if(!r||r.done) return null; const lbl=r.label; if(r.rework) return lbl+": redo & resend"; if(r.rejected) return lbl+": rejected"; if(REJECTABLE.includes(k)) return lbl+": not approved"; return lbl+": pending"; }).filter(Boolean); const o={ "Order No":s.orderNo, "Style No":s.styleNo, "Colour":s.colour, "Qty":s.qty }; if(relMode!=="summary"){ o["FIT"]=s.fitReq?"Y":"-"; o["PRT"]=s.printReq?"Y":"-"; o["S-O"]=s.soReq?"Y":"-"; o["LAB"]=s.labDipReq?"Y":"-"; o["BYP"]=s.ppBypass?"Y":"-"; o["PP"]=s.ppNeeded?"Y":"-"; STAGES.forEach(st=>{ o[st.label]=sd(st.key); }); } else { o["Fabric IH"]=sd("fabricIH"); o["PP Appr"]=sd("ppAppr"); o["Prod File"]=sd("prodFile"); } o["Proj. Release"]=c.projRelease?fmt(c.projRelease):""; o["Delivery"]=s.delivery; o["Released"]=c.released?"YES":""; o["Pending / blockers"]=c.released?"released":(open.join("; ")||"none"); o["Remarks"]=s.remarks||""; o["Status"]=c.status; return o; }); name=relMode==="summary"?"release_plan_summary":"release_plan_detailed"; sheet="Release Plan"; }
    else if(mode==="detail"){ data=rows.map(({s,c})=>{ const sd=k=>{ const r=(c.stages||[]).find(x=>x.key===k); if(!r) return "n/a"; const a=(s.actuals&&s.actuals[k])?parse(s.actuals[k]):null; if(a) return fmt(a); if(r.skipped) return "WAIVED"; if(r.rework) return "REDO"+(r.plan?" "+fmt(r.plan):""); if(r.rejected) return "REJECTED"; const t=r.rev||r.plan; if(REJECTABLE.includes(k)) return "await appr"+(t?" "+fmt(t):""); return "pending"+(t?" "+fmt(t):""); }; const open=(c.frontier?[...c.frontier]:[]).map(k=>{ const r=(c.stages||[]).find(x=>x.key===k); if(!r||r.done) return null; const lbl=r.label; if(r.rework) return lbl+": redo & resend"; if(r.rejected) return lbl+": rejected"; if(REJECTABLE.includes(k)) return lbl+": not approved"; return lbl+": pending"; }).filter(Boolean); const o={ "Order No":s.orderNo, "Style No":s.styleNo, "Sample Fit":s.sampleFit, "Family":s.family, "Colour":s.colour, "Brand":s.brand, "Fabric Type":s.fabricType, "Owner":s.owner, "Qty":s.qty, "Order Date":s.ordRec, "Delivery":s.delivery, "FIT":s.fitReq?"Y":"-", "PRT":s.printReq?"Y":"-", "S-O":s.soReq?"Y":"-", "LAB":s.labDipReq?"Y":"-", "BYP":s.ppBypass?"Y":"-", "PP":s.ppNeeded?"Y":"-" }; STAGES.forEach(st=>{ o[st.label]=sd(st.key); }); o["% Done"]=c.pct+"%"; o["Float"]=(c.float!=null?c.float+"d":""); o["Idle"]=(c.idle!=null?c.idle+"d":""); o["Proj. Release"]=c.projRelease?fmt(c.projRelease):""; o["Released"]=c.released?"YES":""; o["Pending / blockers"]=c.released?"released":(open.join("; ")||"none"); o["Remarks"]=s.remarks||""; o["Status"]=c.status; return o; }); name="detailed_summary"; sheet="Detailed"; }
  ...
  useEffect(()=>{ try{ localStorage.setItem("mt_trackfilters", JSON.stringify({ search, statusFilter, ownerFilter, archiveView, activityFilter, colFilters, tab, todoFilter, followFilter })); }catch(e){} },[search,statusFilter,ownerFilter,archiveView,activityFilter,colFilters,tab,todoFilter,followFilter]);
  ...
  const resolveThread=async(cell)=>{ const ck=cellKey(cell.id,cell.col); const anyOpen=(comments[ck]||[]).some(x=>!x.resolved); const val=anyOpen; setComments(p=>({...p,[ck]:(p[ck]||[]).map(x=>({...x,resolved:val}))})); try{ await supabase.from("comments").update({ resolved:val }).eq("style_id",cell.id).eq("col",cell.col); }catch(e){} };
  ...
  const openNotif=(n)=>{ markRead(n); setBellOpen(false); if(n.style_id&&n.col) setThreadCell({ id:n.style_id, col:n.col }); };
...
{history && (<div onClick={()=>setHistory(false)} style={{ position:"fixed", inset:0, background:"rgba(26,26,26,0.55)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}><div onClick={e=>e.stopPropagation()} style={{ background:"var(--surface)", border:"2px solid var(--ink)", boxShadow:"8px 8px 0 var(--ink)", width:640, maxWidth:"100%", maxHeight:"80vh", display:"flex", flexDirection:"column", fontFamily:"'JetBrains Mono',monospace" }}><div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 14px", borderBottom:"1px solid var(--line-3)" }}><div style={{ fontFamily:"'Archivo',sans-serif", fontWeight:800, fontSize:17 }}>Change history</div><button onClick={()=>setHistory(false)} style={{ border:"1px solid var(--ink)", background:"var(--bg)", cursor:"pointer", fontFamily:"inherit", fontSize:11, padding:"4px 9px" }}>Close</button></div><div style={{ padding:"8px 14px", borderBottom:"1px solid #f2f2f2" }}><input value={histFilter} onChange={e=>setHistFilter(e.target.value)} placeholder="filter by style no, field, stage, or person…" style={{ width:"100%", boxSizing:"border-box", fontFamily:"inherit", fontSize:11, padding:6, border:"1px solid var(--ink)" }}/></div><div style={{ overflowY:"auto", padding:"4px 0" }}>{auditBusy ? <div style={{ fontSize:11, color:"var(--muted-1)", padding:"24px", textAlign:"center" }}>Loading…</div> : (()=>{ const q=histFilter.trim().toLowerCase(); const fl=auditRows.filter(a=> !q || (a.style_no||"").toLowerCase().includes(q) || (a.field||"").toLowerCase().includes(q) || (a.actor_name||"").toLowerCase().includes(q) || (colLabelOf(a.col)||"").toLowerCase().includes(q)); if(fl.length===0) return <div style={{ fontSize:11, color:"var(--muted-1)", padding:"24px", textAlign:"center" }}>{auditRows.length===0?"No changes recorded yet.":"No matches."}</div>; return fl.map(a=>(<div key={a.id} style={{ padding:"7px 14px", borderBottom:"1px solid #f5f5f5", fontSize:11, display:"flex", gap:10, alignItems:"baseline" }}><span style={{ color:"var(--muted-1)", fontSize:9, whiteSpace:"nowrap", minWidth:80 }}>{tsShort(a.created_at)}</span><span style={{ flex:1 }}><b>{a.style_no||a.style_id}</b> · {colLabelOf(a.col)} <span style={{ color:"var(--muted-2)" }}>({a.field})</span><br/><span style={{ color:"var(--danger)" }}>{a.old_val||"—"}</span> <span style={{ color:"var(--muted-2)" }}>→</span> <span style={{ color:"var(--success)" }}>{a.new_val||"—"}</span> <span style={{ color:"var(--muted-1)" }}>· {a.actor_name}</span></span></div>)); })()}</div></div></div>)}
      ...
      <div style={{ display:"flex", gap:10, alignItems:"center", padding:"12px 22px 6px", flexWrap:"wrap" }}>
        ...
        <span style={{ position:"relative" }}><button onClick={(e)=>{ e.stopPropagation(); setFrOpen(o=>!o); }} title="Find & replace text in cells" style={{ fontFamily:"inherit", fontSize:11, padding:"6px 10px", cursor:"pointer", border:"1px solid var(--ink)", background:frOpen?"var(--ink)":"var(--surface)", color:frOpen?"var(--bg)":"var(--ink)", fontWeight:700 }}>⌕ Find/Replace</button>{frOpen && (<div onClick={e=>e.stopPropagation()} style={{ position:"absolute", top:"100%", left:0, marginTop:4, zIndex:75, background:"var(--surface)", border:"1px solid var(--ink)", boxShadow:"4px 4px 0 var(--ink)", padding:12, width:260 }}><div style={{ fontSize:11, fontWeight:700, marginBottom:8 }}>Find &amp; replace</div><label style={{ fontSize:10, color:"var(--muted-2)" }}>Find</label><input autoFocus value={frFind} onChange={e=>setFrFind(e.target.value)} placeholder="text to find" style={{ width:"100%", boxSizing:"border-box", fontFamily:"inherit", fontSize:11, padding:5, marginBottom:6, border:"1px solid var(--ink)" }}/><label style={{ fontSize:10, color:"var(--muted-2)" }}>Replace with</label><input value={frRepl} onChange={e=>setFrRepl(e.target.value)} placeholder="(leave blank to delete)" style={{ width:"100%", boxSizing:"border-box", fontFamily:"inherit", fontSize:11, padding:5, marginBottom:8, border:"1px solid var(--ink)" }}/><div style={{ display:"flex", gap:12, fontSize:10, marginBottom:6 }}><label style={{ display:"flex", gap:4, cursor:"pointer" }}><input type="radio" checked={frScope==="filtered"} onChange={()=>setFrScope("filtered")}/>All filtered</label><label style={{ display:"flex", gap:4, cursor:"pointer" }}><input type="radio" checked={frScope==="selected"} onChange={()=>setFrScope("selected")}/>Selected cells</label></div><label style={{ fontSize:10, display:"flex", gap:5, marginBottom:8, cursor:"pointer" }}><input type="checkbox" checked={frCase} onChange={e=>setFrCase(e.target.checked)}/>Match case</label><div style={{ fontSize:10, color:"var(--muted-3)", marginBottom:8, minHeight:13 }}>{frScope==="selected" && !rect() ? "Select cells in the grid first." : (frFind ? findReplace(true)+" cell(s) will change" : "Applies to text fields (style, colour, brand, owner, remarks…)")}</div><div style={{ display:"flex", gap:8 }}><button disabled={!frFind} onClick={(e)=>{ e.stopPropagation(); const n=findReplace(false); flash(); }} style={{ flex:1, fontFamily:"inherit", fontSize:11, fontWeight:700, padding:6, cursor:frFind?"pointer":"not-allowed", border:"1px solid var(--ink)", background:frFind?"var(--accent)":"var(--line-2)", opacity:frFind?1:0.6 }}>Replace all</button><button onClick={()=>setFrOpen(false)} style={{ fontFamily:"inherit", fontSize:11, padding:"6px 10px", cursor:"pointer", border:"1px solid var(--ink)", background:"var(--bg)" }}><X size={12}/></button></div></div>)}</span>
        ...
        <div style={{ display:"flex", border:"1px solid var(--ink)" }} title="data text size"><button onClick={(e)=>{ e.stopPropagation(); bumpScale(-0.1); }} style={{ fontFamily:"inherit", fontSize:11, padding:"6px 9px", cursor:"pointer", border:"none", borderRight:"1px solid var(--ink)", background:"var(--surface)", fontWeight:700 }}>A−</button><button onClick={(e)=>{ e.stopPropagation(); bumpScale(0.1); }} style={{ fontFamily:"inherit", fontSize:13, padding:"6px 9px", cursor:"pointer", border:"none", background:"var(--surface)", fontWeight:700 }}>A+</button></div>
          ...
          <tbody>
            {rows.map(({s,c},rowIdx)=>{ const t=TONE_STYLE[c.tone]; const sk=cellKey(s.id,"__style"); const styBg=bgFor(s.id,"__style","var(--surface)"); return (
              <tr key={s.id} role="row">
                <td id={`cell-${s.id}-__style`} onClick={(e)=>onCellClick(e,s.id,"__style")} onDoubleClick={(e)=>{ e.stopPropagation(); startEdit(s.id,"__style"); }} style={{ position:"sticky", left:0, zIndex:5, background:styBg, border:"1px solid var(--line-1)", padding:"6px 9px", overflow:"hidden", boxShadow:ringFor(s.id,"__style"), cursor:"cell" }}>
                  ...
                  <div style={{ fontSize:9, color:"var(--muted-1)", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", maxWidth:188, marginTop:1 }}>{s.colour}</div>
                ...
                {visStages.map(st=>{
                  ...
                  return (
                    <td key={st.key} id={`cell-${s.id}-${st.key}`} onClick={(e)=>onCellClick(e,s.id,st.key)} onDoubleClick={(e)=>{ e.stopPropagation(); if(editable) beginDate(s.id,st.key,"actual"); }}
                      ...
                      <div style={{ minHeight:38, padding:"4px 8px", fontSize:11, color:cs.actual?"var(--ink)":"var(--muted-6)" }}>
            ...
            {canMaster(role) && (
              <tr style={{ background:"#fbf7ee" }}>
                <td style={{ position:"sticky", left:0, zIndex:5, background:"#fbf7ee", border:"1px dashed var(--accent)", padding:"6px 9px" }}><div style={{ display:"flex", alignItems:"center", gap:5 }}><Plus size={13} color="var(--accent)"/><input value={newRow.styleNo} onClick={e=>e.stopPropagation()} onChange={e=>setNewRow(n=>({...n,styleNo:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addNewStyle()} placeholder="new style no… *" style={{ border:"none", outline:"none", background:"transparent", fontFamily:"inherit", fontSize:11, fontWeight:700, width:120 }}/></div></td>
...
function LoginScreen(){
  ...
  return (<AuthShell>
    <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:10, letterSpacing:1.5, color:"var(--accent)", fontWeight:700, marginBottom:6 }}>KOTHARI SPORTS AND APPARELS</div>
