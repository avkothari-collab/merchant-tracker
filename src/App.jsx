...
const INFO_COLS=[
  ...
  { key:"fabricIH",  label:"Fabric IH",  kind:"calc", w:74 },
...
export default function App(){
  ...
  useEffect(()=>{ if(firstRender.current){ firstRender.current=false; return; } if(!loadedRef.current) return; const t=setTimeout(async()=>{ try{
    await supabase.from("styles").upsert(styles.map(styleToRow));
    ...
    if(stageRows.length) await supabase.from("stage_dates").upsert(stageRows,{ onConflict:"style_id,stage" });
    ...
    if(metaRows.length) await supabase.from("cell_meta").upsert(metaRows,{ onConflict:"style_id,col" });
    flash();
  }catch(e){ console.error("save failed",e); } },700); return ()=>clearTimeout(t); },[styles,fills,notes]);
  ...
  const sortVal=(col,{s,c})=>{ switch(col){ case "__style": return s.styleNo.toLowerCase(); case "orderNo": return (s.orderNo||"~").toLowerCase(); case "sampleFit": return fitNum(s); case "family": return s.family.toLowerCase(); case "colour": return s.colour.toLowerCase(); case "owner": return (s.owner||"").toLowerCase(); case "setId": return (s.setId||"~").toLowerCase(); case "setRole": return (s.setRole||"").toLowerCase(); case "qty": return s.qty; case "ordRec": return s.ordRec?new Date(s.ordRec).getTime():Infinity; case "delivery": return s.delivery?new Date(s.delivery).getTime():Infinity; case "overall": return toneRank[c.tone]; case "fit": return toneRank[c.fitBranch.tone]; case "print": return toneRank[c.printBranch.tone]; case "fabric": return toneRank[c.fabricBranch.tone]; case "pp": return toneRank[c.ppBranch.tone]; case "fabricIH": return c.fabricCountdown.n==null?Infinity:c.fabricCountdown.n; case "proj": return c.projRelease?c.projRelease.getTime():Infinity; case "pct": return c.pct; case "chase": return (c.chaseOwners||[]).length; case "float": return c.float==null?Infinity:c.float; case "idle": return c.idle==null?-1:c.idle; case "remarks": return (s.remarks||"~").toLowerCase(); default: { const a=s.actuals[col]; return a?new Date(a).getTime():Infinity; } } };
  ...
  const clickHeader=(col)=> setSort(p=> p.col===col?{col,dir:-p.dir}:{col,dir:1});
  ...
  const onCellClick=(e,id,col)=>{ e.stopPropagation(); if(gridRef.current) gridRef.current.focus({preventScroll:true}); if(e.shiftKey&&sel){ setFocus({id,col}); scrollToCell(id,col); return; } if(sel&&sel.id===id&&sel.col===col&&!editing&&isEditableCol(col)&&ROLES[role].canEdit(ownerOfCol(col))){ startEdit(id,col); return; } setSel({id,col}); setFocus(null); };
  ...
  const coerce=(col,raw)=>{ if(raw==null) return ""; const v=String(raw).trim(); if(isDateCol(col)){ const d=new Date(v); return isNaN(d)?"":iso(d); } return v; };
  ...
  const onKeyDown=(e)=>{
    ...
    if(!sel) return;
    if(editing){ if(e.key==="Enter"){ commitText(); moveAnchor(1,0); e.preventDefault(); } else if(e.key==="Escape"){ setEditing(null); e.preventDefault(); } else if(e.key==="Tab"){ commitText(); moveAnchor(0,e.shiftKey?-1:1); e.preventDefault(); } return; }
    ...
    else if(e.key.length===1&&!e.metaKey&&!e.ctrlKey){ if(sel.col==="qty"&&/[0-9]/.test(e.key)){ startEdit(sel.id,"qty",e.key); e.preventDefault(); } else if(sel.col==="__style"||TEXT_COLS.includes(sel.col)){ startEdit(sel.id,sel.col,e.key); e.preventDefault(); } }
  };
  const jumpToEnter=(id,stageKey)=>{ const st=STAGES.find(x=>x.key===stageKey); setSel({id,col:stageKey}); setFocus(null); requestAnimationFrame(()=>{ const el=document.getElementById(`cell-${id}-${stageKey}`); if(el) el.scrollIntoView({ behavior:"smooth", inline:"center", block:"nearest" }); }); if(st&&ROLES[role].canEdit(st.owner)) setTimeout(()=>setEditing({id,col:stageKey,mode:"actual"}),260); };

  const handleCopy=(e)=>{ const R=rect(); if(!R) return; const lines=[]; let any=false; for(let r=R.r1;r<=R.r2;r++){ const cells=[]; for(let cc=R.c1;cc<=R.c2;cc++){ let v=rows[r]?getVal(rows[r].s,navCols[cc]):""; if(isDateCol(navCols[cc])&&v) v=fmt(parse(v)); if(v) any=true; cells.push(v??""); } lines.push(cells.join("\t")); } if(any){ const tsv=lines.join("\n"); try{ e.clipboardData.setData("text/plain",tsv); e.preventDefault(); }catch(err){} setClip({ values:lines.map(l=>l.split("\t")), h:lines.length, w:lines[0].split("\t").length }); flash(); } };
  ...
  const valueFor=(s,cc,col)=>{
    ...
    if(col==="fabricIH") return cc.fabricCountdown.txt;
  ...
  return (
    <div ref={gridRef} tabIndex={0} onKeyDown={onKeyDown} onCopy={handleCopy} onPaste={handlePaste} onMouseUp={()=>{ if(filling){ applyFillHandle(); setFilling(false); setFillFrom(null); setFillTo(null); } }} onMouseOver={(e)=>{ if(!filling) return; const td=e.target.closest && e.target.closest('td[id^="cell-"]'); if(td){ const m=td.id.match(/^cell-(\d+)-(.+)$/); if(m) setFillTo({ id:Number(m[1]), col:m[2] }); } }} onClick={()=>{ setEditing(null); setFillOpen(false); setColsOpen(false); setFilterCol(null); }}
      ...
      <div style={{ background:"#1a1a1a", color:"#f4f0e8", padding:"14px 22px", display:"flex", alignItems:"center", justifyContent:"space-between", borderBottom:"3px solid #d97706" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:14 }}><span style={{ fontSize:11, color: saved?"#7fd1a8":"#6a665e" }}>{saved?"● saved":"○ auto-save (simulated)"}</span><div style={{ display:"flex", border:"1px solid #4a463e" }}>{Object.keys(ROLES).map(r=>(<button key={r} onClick={(e)=>{ e.stopPropagation(); setRole(r); }} style={{ fontFamily:"inherit", fontSize:10, padding:"5px 9px", cursor:"pointer", border:"none", background:role===r?"#d97706":"transparent", color:role===r?"#1a1a1a":"#cfc9bf", fontWeight:role===r?700:400 }}>{ROLES[r].label}</button>))}</div></div>
      </div>
      ...
      <div style={{ display:"flex", gap:10, alignItems:"center", padding:"12px 22px 6px", flexWrap:"wrap" }}>
        ...
        <div style={{ position:"relative" }}><button onClick={(e)=>{ e.stopPropagation(); setFillOpen(o=>!o); setColsOpen(false); }} disabled={role==="Viewer"} style={{ fontFamily:"inherit", fontSize:11, padding:"6px 11px", cursor:role==="Viewer"?"not-allowed":"pointer", border:"1px solid #1a1a1a", background:"#d97706", color:"#1a1a1a", fontWeight:700, display:"flex", alignItems:"center", gap:6, opacity:role==="Viewer"?0.4:1 }}><Copy size={13}/> Fill date → {rows.length}</button>{fillOpen && role!=="Viewer" && (<FillPanel count={rows.length} onClose={()=>setFillOpen(false)} onApply={(key,val)=>{ setStyles(prev=>prev.map(s=>rows.some(r=>r.s.id===s.id)?{...s,actuals:{...s.actuals,[key]:val||undefined}}:s)); flash(); setFillOpen(false); }}/>)}</div>
        <div style={{ position:"relative" }}><button onClick={(e)=>{ e.stopPropagation(); setColsOpen(o=>!o); setFillOpen(false); }} style={{ fontFamily:"inherit", fontSize:11, padding:"6px 11px", cursor:"pointer", border:"1px solid #1a1a1a", background:"#fff", display:"flex", alignItems:"center", gap:6 }}><Columns3 size={13}/> Columns {hidden.size>0?`(${hidden.size} hidden)`:""}</button>
      ...
      <div style={{ overflowX:"auto", padding:"0 22px" }}>
        <table role="grid" aria-label="Pre-production tracker grid. Arrow keys to move, Escape to exit, Tab to leave the grid." style={{ borderCollapse:"separate", borderSpacing:0, fontSize:11, tableLayout:"fixed" }}>
          <colgroup>
            ...
            {rows.map(({s,c},rowIdx)=>{ const t=TONE_STYLE[c.tone]; const sk=cellKey(s.id,"__style"); const styBg=bgFor(s.id,"__style","#fff"); return (
              <tr key={s.id} role="row">
                ...
                {visInfo.map(col=>{
                  ...
                  const k=cellKey(s.id,col.key);
                  if(col.kind==="date"){ const bg=bgFor(s.id,col.key,"#fff"); return (<td key={col.key} id={`cell-${s.id}-${col.key}`} onClick={(e)=>onCellClick(e,s.id,col.key)} onDoubleClick={(e)=>{ e.stopPropagation(); if(role!=="Viewer") setEditing({id:s.id,col:col.key,mode:"actual"}); }} style={{ border:"1px solid #ddd", padding:"6px 9px", whiteSpace:"nowrap", boxShadow:ringFor(s.id,col.key), cursor:"cell", position:"relative", overflow:"hidden", background:bg, ...freezeStyle(col.key,bg) }}>{fmt(parse(s[col.key]))||<span style={{color:"#ccc"}}>—</span>}{editing&&editing.id===s.id&&editing.col===col.key && <CalPopup value={s[col.key]} onClose={()=>setEditing(null)} onPick={(d)=>setField(s.id,col.key,d)}/>}<NoteTri k={k}/><FillHandle id={s.id} col={col.key}/></td>); }
                  let content=null;
                  ...
                  else if(col.key==="fabricIH") content=<span style={{ fontWeight:700, color:BR_TONE[c.fabricCountdown.tone].fg }}>{c.fabricCountdown.txt}</span>;
                ...
                {visStages.map(st=>{
                  ...
                  return (
                    <td key={st.key} id={`cell-${s.id}-${st.key}`} onClick={(e)=>onCellClick(e,s.id,st.key)} onDoubleClick={(e)=>{ e.stopPropagation(); if(editable) setEditing({id:s.id,col:st.key,mode:"actual"}); }}
                      style={{ border:"1px solid #ddd", padding:0, position:"relative", overflow:"hidden", background:bg, boxShadow:ringFor(s.id,st.key)||(isNext?"inset 0 0 0 2px #d97706":null), cursor:editable?"cell":"default" }}>
                      ...
                      {editable && !cs.actual && (<button title="set revised plan date" onClick={(e)=>{ e.stopPropagation(); setSel({id:s.id,col:st.key}); setFocus(null); setEditing({id:s.id,col:st.key,mode:"rev"}); }} style={{ position:"absolute", top:1, right:1, border:"none", background:"transparent", cursor:"pointer", padding:1, lineHeight:0 }}><RotateCcw size={10} color="#6d4aab"/></button>)}
                      {editing&&editing.id===s.id&&editing.col===st.key&&editable && (<CalPopup label={editing.mode==="rev"?"set REVISED plan":"set actual"} value={editing.mode==="rev"?(s.revs?.[st.key]):s.actuals[st.key]} onClose={()=>setEditing(null)} onPick={(d)=> editing.mode==="rev"?setRev(s.id,st.key,d):setField(s.id,st.key,d)}/>)}
