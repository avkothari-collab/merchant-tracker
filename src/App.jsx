...
function MerchTracker({ me, onSignOut }){
  ...
  const runExport=(mode,buf,incBuf,relMode)=>{ try{ let data, name, sheet="Tracker"; const B=Math.abs(buf||0);
    ...
    const actCell=(s,c,k)=>{ const r=(c.stages||[]).find(x=>x.key===k); if(!r) return ""; if(s.actuals&&s.actuals[k]) return fmtY(parse(s.actuals[k])); if(r.skipped) return "WAIVED"; if(r.rejected) return "REJECTED"; if(r.rework) return "REDO"; return ""; };
    const planCell=(s,c,k,withBuf)=>{ const r=(c.stages||[]).find(x=>x.key===k); if(!r) return ""; const t=r.rev||r.plan; if(!t) return ""; const internal=!BUYER_STAGES.includes(k); const d=(withBuf&&B&&internal)?addWorkdays(t,B):t; return fmtY(d); };
    const pairs=(o,s,c,withBuf,list)=>{ (list||STAGES).forEach(st=>{ o[st.label]=actCell(s,c,st.key); o[st.label+" (plan)"]=planCell(s,c,st.key,withBuf); }); };
    ...
    else if(mode==="release"){ data=rows.map(({s,c})=>{ const open=blockers(c); const o={ "Order No":s.orderNo, "Style No":s.styleNo, "Colour":s.colour, "Qty":s.qty }; if(relMode!=="summary"){ o["FIT"]=s.fitReq?"Y":"-"; o["PRT"]=s.printReq?"Y":"-"; o["S-O"]=s.soReq?"Y":"-"; o["LAB"]=s.labDipReq?"Y":"-"; o["BYP"]=s.ppBypass?"Y":"-"; o["PP"]=s.ppNeeded?"Y":"-"; pairs(o,s,c,false); } else { ["fabricIH","ppAppr","prodFile"].forEach(k=>{ const st=STAGES.find(x=>x.key===k); o[st.label]=actCell(s,c,k); o[st.label+" (plan)"]=planCell(s,c,k,false); }); } o["Proj. Release"]=c.projRelease?fmtY(c.projRelease):""; o["Delivery"]=fmtIso(s.delivery); o["Released"]=c.released?"YES":""; o["Pending / blockers"]=c.released?"released":(open.join("; ")||"none"); o["Remarks"]=s.remarks||""; o["Status"]=c.status; return o; }); name=relMode==="summary"?"release_plan_summary":"release_plan_detailed"; sheet="Release Plan"; }
