...
export default function App(){
  ...
  const jumpToEnter=(id,stageKey)=>{ const st=STAGES.find(x=>x.key===stageKey); setSel({id,col:stageKey}); setFocus(null); requestAnimationFrame(()=>{ const el=document.getElementById(`cell-${id}-${stageKey}`); if(el) el.scrollIntoView({ behavior:"smooth", inline:"center", block:"nearest" }); }); if(st&&canEdit(role,stageKey,"actual")) setTimeout(()=>beginDate(id,stageKey,"actual"),260); };
...
function FillPanel({ count, role, onApply, onClose }){
  ...
  return (<div onClick={e=>e.stopPropagation()} style={{ position:"absolute", top:"100%", left:0, marginTop:4, zIndex:70, background:"#fff", border:"1px solid #1a1a1a", boxShadow:"4px 4px 0 #1a1a1a", padding:12, width:280 }}><div style={{ fontSize:11, fontWeight:700, marginBottom:8 }}>Set one date across {count} filtered styles</div><label style={{ fontSize:10, color:"#888" }}>Stage</label><select value={key} onChange={e=>setKey(e.target.value)} style={{ width:"100%", fontFamily:"inherit", fontSize:11, padding:5, marginBottom:8, border:"1px solid #1a1a1a" }}>{opts.map(o=><option key={o.key} value={o.key}>{o.label}</option>)}</select><label style={{ fontSize:10, color:"#888" }}>Date</label><input type="date" value={val} onChange={e=>setVal(e.target.value)} style={{ width:"100%", fontFamily:"inherit", fontSize:11, padding:5, marginBottom:10, border:"1px solid #1a1a1a" }}/><div style={{ display:"flex", gap:8 }}><button onClick={()=>onApply(key,val)} style={{ flex:1, fontFamily:"inherit", fontSize:11, fontWeight:700, padding:6, cursor:"pointer", border:"1px solid #1a1a1a", background:"#d97706" }}>Apply to all filtered</button><button onClick={onClose} style={{ fontFamily:"inherit", fontSize:11, padding:"6px 10px", cursor:"pointer", border:"1px solid #1a1a1a", background:"#f4f0e8" }}><X size={12}/></button></div></div>);
}
