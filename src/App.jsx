...
function MerchTracker({ me, onSignOut }){
  const APP_BUILD_LABEL="v31dq Build 19 User–Merchant Link";
...
function DailyUpdatesView({ computed, todoItems, role, me, team, userMerchantLinks={}, onJump, onOpenTodo }){
  ...
  const merchantProfiles=useMemo(()=>{
    const list=(team||[]).filter(p=>["junior","senior"].includes(String(p.role||"").toLowerCase()));
    if(me&&["junior","senior"].includes(role)&&!list.some(p=>profileKey(p)===myKey)) list.push(me);
  ...
  const dayRange=()=>{ const start=new Date(`${date}T00:00:00+05:30`); return { start:start.toISOString(), end:new Date(start.getTime()+ONE_DAY).toISOString() }; };
  const loadDay=async()=>{
    if(!date) return;
    setBusy(true);
    try{
      ...
      const [a,c,sr]=await Promise.all([fetchPaged("audit_log"),fetchPaged("comments"),supabase.from("app_settings").select("data").eq("id",SUBMISSION_SETTING_ID).maybeSingle()]);
      setAudit(a||[]); setDayComments(c||[]); const sd=sr&&sr.data&&sr.data.data; setSubmissions(sd&&typeof sd==="object"?(sd.items||sd):{});
    }catch(e){ setAudit([]); setDayComments([]); }
    setBusy(false);
  };
  ...
  const correctionRows=netRows.filter(r=>r.correction);
  ...
  const userKeys=useMemo(()=>{ const s=new Set(merchantProfiles.map(profileKey)); meaningfulRows.forEach(r=>s.add(r.actorKey)); commentRows.forEach(r=>s.add(r.actorKey)); return [...s]; },[merchantProfiles,audit,dayComments]);
  ...
  const currentlyDueTasks=useMemo(()=>todaySelected?(todoItems||[]).filter(t=>{ const d=taskDue(t); return !!(t.overdue||(d&&d<=TODAY)); }):[],[todoItems,date]);
  const updatedActivitySet=useMemo(()=>new Set(atomicMeaningfulRows.map(r=>`${r.actorKey}::${r.styleId}::${r.stageKey}`)),[netRows]);
  const untouchedFor=(key)=>currentlyDueTasks.filter(t=>{ if(taskMerchantKey(t)!==key) return false; const stage=stageKeyFromAnyGlobal(t._stageKey||t.activityKey||t.key); const ids=(Array.isArray(t.styleIds)&&t.styleIds.length?t.styleIds:[t.id]).filter(Boolean); return !ids.some(id=>updatedActivitySet.has(`${key}::${id}::${stage}`)); });
  const handledRequiredFor=(key)=>{ const reportDay=parse(date); const rows=meaningfulRows.filter(r=>{ if(r.actorKey!==key) return false; const due=parse(r.expectedValue||r.dueBefore); return !!(reportDay&&due&&due<=reportDay); }); const seen=new Set(); return rows.filter(r=>{ const ids=(Array.isArray(r.styleIds)&&r.styleIds.length?r.styleIds:[r.styleId]).map(String).sort().join(","); const k=[ids,r.rawCol||r.stageKey].join("::"); if(seen.has(k)) return false; seen.add(k); return true; }); };
  const submissionFor=(key)=>submissions[`${date}::${key}`]||null;
  const lastWorkAt=(key)=>{ const times=[...meaningfulRows.filter(r=>r.actorKey===key).map(r=>new Date(r.at).getTime()),...commentRows.filter(r=>r.actorKey===key).map(r=>new Date(r.created_at).getTime())].filter(Number.isFinite); return times.length?Math.max(...times):0; };
  const summaryFor=(key)=>{ const rows=meaningfulRows.filter(r=>r.actorKey===key), comments=commentRows.filter(r=>r.actorKey===key), corrections=correctionRows.filter(r=>r.actorKey===key), sub=submissionFor(key), last=lastWorkAt(key), handled=handledRequiredFor(key).length, untouched=untouchedFor(key).length; return { key,name:displayName(key,rows[0]&&rows[0].actorName),trackerMerchant:linkedMerchantForKey(key),required:todaySelected?handled+untouched:null,handled,actual:rows.filter(r=>r.kind==="actual").length,revised:rows.filter(r=>r.kind==="revised").length,movedLater:rows.filter(r=>r.kind==="revised"&&r.movedLater).length,rejected:rows.filter(r=>r.kind==="rejected").length,waived:rows.filter(r=>r.kind==="waived").length,meaningful:rows.length,comments:comments.length,corrections:corrections.length,untouched,submission:sub,changedAfter:!!(sub&&last>new Date(sub.submittedAt||0).getTime())}; };
  const summaries=userKeys.map(summaryFor).sort((a,b)=>Number(!b.submission)-Number(!a.submission)||b.untouched-a.untouched||b.movedLater-a.movedLater||String(a.name).localeCompare(String(b.name)));
  const visibleRows=selectedKey==="All"?meaningfulRows:meaningfulRows.filter(r=>r.actorKey===selectedKey);
  const visibleComments=selectedKey==="All"?commentRows:commentRows.filter(r=>r.actorKey===selectedKey);
  const visibleCorrections=selectedKey==="All"?correctionRows:correctionRows.filter(r=>r.actorKey===selectedKey);
  const visibleUntouched=selectedKey==="All"?currentlyDueTasks.filter(t=>untouchedFor(taskMerchantKey(t)).includes(t)):untouchedFor(selectedKey);
  const visibleSummary=selectedKey==="All"?summaries.reduce((m,r)=>{ m.required+=(r.required||0);m.handled+=r.handled;m.actual+=r.actual;m.revised+=r.revised;m.movedLater+=r.movedLater;m.meaningful+=r.meaningful;m.comments+=r.comments;m.corrections+=r.corrections;m.untouched+=r.untouched;return m; },{required:0,handled:0,actual:0,revised:0,movedLater:0,meaningful:0,comments:0,corrections:0,untouched:0}):(summaries.find(r=>r.key===selectedKey)||{required:0,handled:0,actual:0,revised:0,movedLater:0,meaningful:0,comments:0,corrections:0,untouched:0});
  const mySubmission=submissionFor(myKey);
  useEffect(()=>{ const s=submissionFor(myKey); setBlockers((s&&s.blockers)||""); setHelpNeeded((s&&s.helpNeeded)||""); setTomorrow((s&&s.tomorrow)||""); },[date,submissions,myKey]);
  const submitMyDay=async()=>{
    const myRows=meaningfulRows.filter(r=>r.actorKey===myKey); const lateMoves=myRows.filter(r=>r.kind==="revised"&&r.movedLater).length;
    if(lateMoves&&!blockers.trim()){ alert(`You moved ${lateMoves} commitment${lateMoves===1?"":"s"} later. Add the reason/blocker before submitting.`); return; }
    setSubmitBusy(true);
    try{
      ...
      const myRequiredSummary=summaryFor(myKey); const entry={ date,userId:me&&me.id||null,userName:(me&&(me.name||me.email))||"",trackerMerchant:linkedMerchantForKey(myKey),submittedAt:new Date().toISOString(),blockers:blockers.trim(),helpNeeded:helpNeeded.trim(),tomorrow:tomorrow.trim(),snapshot:{required:myRequiredSummary.required,handledRequired:myRequiredSummary.handled,meaningful:myRows.length,actual:myRows.filter(r=>r.kind==="actual").length,revised:myRows.filter(r=>r.kind==="revised").length,movedLater:lateMoves,comments:commentRows.filter(r=>r.actorKey===myKey).length,corrections:correctionRows.filter(r=>r.actorKey===myKey).length,untouched:untouchedFor(myKey).length}};
  ...
  const summaryData=summaries.map(r=>({"Merchant Login":r.name,"Linked Tracker Merchant":r.trackerMerchant||"NOT LINKED","Required Today & Behind":todaySelected?r.required:"Current-day only","Required Work Handled":todaySelected?r.handled:"Current-day only","Submitted":r.submission?(r.changedAfter?"Changes after submission":"Yes"):"No","Meaningful Updates":r.meaningful,"Actual Completed":r.actual,"Net Revised":r.revised,"Commitments Moved Later":r.movedLater,"Chase Comments":r.comments,"Due Work Untouched":r.untouched,"Corrections / Repeated Edits":r.corrections,"Submitted At":r.submission?new Date(r.submission.submittedAt).toLocaleString():"","Blockers / Reasons":r.submission&&r.submission.blockers||"","Help Required":r.submission&&r.submission.helpNeeded||"","Tomorrow Priorities":r.submission&&r.submission.tomorrow||""}));
  const detailData=visibleRows.map(r=>({"Merchant":r.actorName,"Order No":r.orderNo,"Style No":r.styleNo,"Colour":r.colour,"Buyer / Brand":r.buyer,"Activity":r.activity,"Update Type":r.kind==="actual"?"Actual completed":r.kind==="revised"?"Net revised commitment":r.kind,"Expected Date Used":r.kind==="actual"?r.expectedValue:r.previous,"Expected Basis":r.kind==="actual"?r.expectedSource:r.expectedSource,"Final Value":r.finalValue,"Result":r.result,"Edits Collapsed":r.editCount,"Final Update Time":new Date(r.at).toLocaleString()}));
  const dueData=visibleUntouched.map(t=>({"Merchant":displayName(taskMerchantKey(t),t.junior||""),"Order No":t.orderNo||"","Style / Colour":t.isColour?t.colour||"":t.styleNo||"","Activity":t._activityLabel||t.activity||stageLabelFromKeyGlobal(t._stageKey||t.activityKey||t.key),"Due":taskDue(t)?iso(taskDue(t)):"","Status":t.overdue?"Overdue":"Due today","Days":Math.abs(Number(t.du)||0),"Chase":t.owner||""}));
  return <div style={{padding:"18px 22px 36px"}}>
    <div style={{display:"flex",justifyContent:"space-between",gap:14,alignItems:"flex-start",flexWrap:"wrap",marginBottom:12}}><div><div style={{fontFamily:"'Archivo',sans-serif",fontWeight:850,fontSize:23}}>Daily Updates</div><div style={{fontSize:11,color:"var(--muted-2)",marginTop:4,maxWidth:850,lineHeight:1.5}}>End-of-day merchant report generated from final/net tracker changes. Repeated edits are collapsed; corrections stay visible but are not counted as additional work.</div></div><ReportExportMenu title="Merchant Daily Updates" prefix="merchant-daily-updates" sheets={[{label:"Merchant Summary",data:summaryData,modes:["summary","detailed"],detailData},{label:"Meaningful Updates",data:detailData,modes:["detailed"]},{label:"Due Work Untouched",data:dueData,modes:["detailed"]}]} defaultMode="detailed"/></div>
    <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",padding:"10px 12px",background:"var(--toolbar-subtle)",border:"1px solid var(--line-2)",borderRadius:10,marginBottom:10}}><label style={{fontSize:10,fontWeight:900}}>Report date <input type="date" value={date} max={iso(TODAY)} onChange={e=>setDate(e.target.value)} style={{...inputStyle,marginLeft:5}}/></label>{role!=="junior"&&<select value={selectedUser} onChange={e=>setSelectedUser(e.target.value)} style={inputStyle}><option value="All">All merchants</option>{merchantProfiles.map(p=><option key={profileKey(p)} value={profileKey(p)}>{p.name||p.email}{userMerchantLinks[p.id]?` · ${userMerchantLinks[p.id]}`:" · NOT LINKED"}</option>)}</select>}<button onClick={loadDay} disabled={busy} style={{...inputStyle,fontWeight:900,cursor:"pointer"}}>{busy?"Loading…":"Refresh"}</button><span style={{fontSize:9,color:"var(--muted-2)"}}>IST day · complete selected-day audit query, not the 300-row Entry Log display limit</span></div>
    ...
    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>{card("Required today & behind",todaySelected?visibleSummary.required:"—","var(--ink)",todaySelected?"all due + overdue activities":"current-day metric")}{card("Required handled",todaySelected?visibleSummary.handled:"—","var(--success)","updated from required list")}{card("Due untouched",todaySelected?visibleSummary.untouched:"—",visibleSummary.untouched?"var(--danger)":"var(--success)",todaySelected?"still needs an update":"current-day metric")}{card("Meaningful updates",visibleSummary.meaningful,"var(--ink)","all final/net outcomes")}{card("Actual completed",visibleSummary.actual,"var(--success)","Revised/Auto → Actual")}{card("Net revised",visibleSummary.revised,"var(--revised)","one final commitment")}{card("Moved later",visibleSummary.movedLater,visibleSummary.movedLater?"var(--danger)":"var(--success)","reason required")}{card("Chase comments",visibleSummary.comments,"var(--info)","outcomes recorded")}{card("Corrections",visibleSummary.corrections,visibleSummary.corrections?"#b45309":"var(--muted-2)","not counted as work")}</div>
    {role!=="junior"&&<div style={{background:"var(--surface)",border:"1px solid var(--line-2)",borderRadius:12,overflowX:"auto",marginBottom:12}}><table style={{width:"100%",minWidth:1220,borderCollapse:"collapse",fontSize:10}}><thead><tr style={{background:"var(--ink)",color:"var(--bg)"}}>{["Merchant login","Tracker merchant","Required","Handled","Untouched","Submission","Meaningful","Actual","Net revised","Moved later","Comments","Corrections"].map(h=><th key={h} style={{padding:"8px 9px",textAlign:"left",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>{summaries.map(r=><tr key={r.key} onClick={()=>setSelectedUser(r.key)} style={{borderBottom:"1px solid var(--line-3)",cursor:"pointer",background:selectedKey===r.key?"var(--accent-tint)":"transparent"}}><td style={{padding:"8px 9px",fontWeight:950,color:"var(--info)"}}>{r.name} →</td><td style={{padding:"8px 9px",fontWeight:850,color:r.trackerMerchant?"var(--ink)":"var(--danger)"}}>{r.trackerMerchant||"NOT LINKED"}</td><td style={{padding:"8px 9px",fontWeight:900}}>{todaySelected?r.required:"—"}</td><td style={{padding:"8px 9px",color:"var(--success)",fontWeight:900}}>{todaySelected?r.handled:"—"}</td><td style={{padding:"8px 9px",color:r.untouched?"var(--danger)":"var(--success)",fontWeight:900}}>{todaySelected?r.untouched:"—"}</td><td style={{padding:"8px 9px",fontWeight:900,color:!r.submission?"var(--danger)":r.changedAfter?"#b45309":"var(--success)"}}>{!r.submission?"Not submitted":r.changedAfter?"Changes after submission":"Submitted"}</td><td style={{padding:"8px 9px"}}>{r.meaningful}</td><td style={{padding:"8px 9px",color:"var(--success)",fontWeight:850}}>{r.actual}</td><td style={{padding:"8px 9px"}}>{r.revised}</td><td style={{padding:"8px 9px",color:r.movedLater?"var(--danger)":"var(--muted-2)",fontWeight:850}}>{r.movedLater}</td><td style={{padding:"8px 9px"}}>{r.comments}</td><td style={{padding:"8px 9px",color:r.corrections?"#b45309":"var(--muted-2)"}}>{r.corrections}</td></tr>)}</tbody></table></div>}
    <div style={{background:"var(--surface)",border:"1px solid var(--line-2)",borderRadius:12,overflowX:"auto",marginBottom:12}}><div style={{padding:"10px 12px",fontFamily:"'Archivo',sans-serif",fontWeight:850,fontSize:14}}>Meaningful updates · {visibleRows.length}</div><table style={{width:"100%",minWidth:1200,borderCollapse:"collapse",fontSize:10}}><thead><tr style={{background:"var(--toolbar-subtle)"}}>{["Merchant","Order / Style","Activity","Update","Expected date used","Final value","Result","Edits"].map(h=><th key={h} style={{padding:"8px 9px",textAlign:"left",borderBottom:"1px solid var(--line-2)",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>{visibleRows.map(r=><tr key={r.key} onClick={()=>onJump&&onJump(r.styleId,r.stageKey)} style={{borderBottom:"1px solid var(--line-3)",cursor:"pointer",background:r.movedLater?"#fff6f3":"transparent"}}><td style={{padding:"8px 9px"}}>{r.actorName}</td><td style={{padding:"8px 9px"}}><b>{r.styleNo}</b><div style={{fontSize:8.5,color:"var(--muted-2)"}}>{r.orderNo}{r.colour?` · ${r.colour}`:""}</div></td><td style={{padding:"8px 9px",fontWeight:850}}>{r.activity}</td><td style={{padding:"8px 9px",fontWeight:900,color:r.kind==="actual"?"var(--success)":r.kind==="revised"?"var(--revised)":"var(--danger)"}}>{r.kind==="actual"?"Actual completed":r.kind==="revised"?"Net revised":r.kind}</td><td style={{padding:"8px 9px",whiteSpace:"nowrap"}}>{r.kind==="actual"?(r.expectedValue?<><b>{r.expectedValue}</b><div style={{fontSize:8.5,color:"var(--muted-2)"}}>{r.expectedSource}</div></>:"—"):(r.expectedValue||"—")}</td><td style={{padding:"8px 9px",fontWeight:900}}>{r.finalValue||"—"}</td><td style={{padding:"8px 9px",color:r.movedLater||Number(r.variance)>0&&r.kind==="actual"?"var(--danger)":"var(--muted-3)",fontWeight:850}}>{r.result}</td><td style={{padding:"8px 9px",color:r.editCount>1?"#b45309":"var(--muted-2)"}}>{r.editCount}{r.editCount>1?" collapsed":""}</td></tr>)}{!visibleRows.length&&<tr><td colSpan={8} style={{padding:18,textAlign:"center",color:"var(--muted-2)"}}>No meaningful Actual, Revised, Rejected or Waived updates for this selection.</td></tr>}</tbody></table></div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(360px,1fr))",gap:12,marginBottom:12}}><div style={{background:"var(--surface)",border:"1px solid var(--line-2)",borderRadius:12,padding:12}}><div style={{fontFamily:"'Archivo',sans-serif",fontWeight:850,fontSize:14}}>Chase outcomes/comments · {visibleComments.length}</div><div style={{maxHeight:240,overflowY:"auto",marginTop:7}}>{visibleComments.slice(0,100).map(c=><button key={c.id} onClick={()=>onJump&&onJump(c.style_id,c.col)} style={{display:"block",width:"100%",textAlign:"left",border:"none",borderBottom:"1px solid var(--line-3)",background:"transparent",padding:"7px 0",fontFamily:"inherit",cursor:"pointer"}}><div style={{fontSize:9,color:"var(--muted-2)"}}>{c.author_name} · {c.orderNo} · {c.styleNo} · {c.activity}</div><div style={{fontSize:10.5,marginTop:2}}>{c.body}</div></button>)}{!visibleComments.length&&<div style={{padding:"12px 0",fontSize:10,color:"var(--muted-2)"}}>No chase outcome/comments entered.</div>}</div></div><div style={{background:"var(--surface)",border:"1px solid var(--line-2)",borderRadius:12,padding:12}}><div style={{display:"flex",justifyContent:"space-between",gap:8,alignItems:"baseline"}}><div style={{fontFamily:"'Archivo',sans-serif",fontWeight:850,fontSize:14}}>Due work untouched · {todaySelected?visibleUntouched.length:"—"}</div>{todaySelected&&selectedKey!=="All"&&linkedMerchantForKey(selectedKey)&&<button onClick={()=>onOpenTodo&&onOpenTodo(linkedMerchantForKey(selectedKey))} style={{...inputStyle,cursor:"pointer",fontWeight:900}}>Open in To-Do →</button>}</div>{!todaySelected?<div style={{padding:"12px 0",fontSize:10,color:"var(--muted-2)"}}>Historical reports show work performed. Untouched pending work is a live/current metric and stays in Today & Behind and To-Do.</div>:<div style={{maxHeight:240,overflowY:"auto",marginTop:7}}>{visibleUntouched.slice(0,100).map((t,i)=><div key={`${t.id}:${t.key}:${i}`} style={{padding:"7px 0",borderBottom:"1px solid var(--line-3)",fontSize:10}}><b>{t.styleNo||t.colour||"—"}</b> · {t.orderNo||""}<div style={{fontSize:9,color:t.overdue?"var(--danger)":"#7a560f",marginTop:2}}>{t._activityLabel||t.activity||stageLabelFromKeyGlobal(t._stageKey||t.activityKey||t.key)} · {t.overdue?"Overdue":"Due today"}</div></div>)}{!visibleUntouched.length&&<div style={{padding:"12px 0",fontSize:10,color:"var(--success)",fontWeight:850}}>No currently due work is untouched for this selection.</div>}</div>}</div></div>
    ...
    <div style={{background:"var(--surface)",border:"2px solid var(--ink)",borderRadius:12,padding:14}}><div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"flex-start",flexWrap:"wrap"}}><div><div style={{fontFamily:"'Archivo',sans-serif",fontWeight:900,fontSize:16}}>Submit my end-of-day update</div><div style={{fontSize:9.5,color:"var(--muted-2)",marginTop:3}}>The activity list is automatic. Only add exceptions, help required and tomorrow’s priorities.</div></div><div style={{fontSize:10,fontWeight:900,color:!mySubmission?"var(--danger)":lastWorkAt(myKey)>new Date(mySubmission.submittedAt||0).getTime()?"#b45309":"var(--success)"}}>{!mySubmission?"Not submitted":lastWorkAt(myKey)>new Date(mySubmission.submittedAt||0).getTime()?"Changes made after submission · resubmit":"Submitted · "+new Date(mySubmission.submittedAt).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</div></div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:9,marginTop:10}}><label style={{fontSize:9,fontWeight:900}}>BLOCKERS / REASONS FOR LATER COMMITMENTS<textarea value={blockers} onChange={e=>setBlockers(e.target.value)} placeholder="Required when a revised commitment moved later" rows={3} style={{...inputStyle,width:"100%",resize:"vertical",display:"block",marginTop:4}}/></label><label style={{fontSize:9,fontWeight:900}}>HELP / SENIOR DECISION REQUIRED<textarea value={helpNeeded} onChange={e=>setHelpNeeded(e.target.value)} placeholder="What intervention is needed, by whom and by when?" rows={3} style={{...inputStyle,width:"100%",resize:"vertical",display:"block",marginTop:4}}/></label><label style={{fontSize:9,fontWeight:900}}>TOMORROW’S TOP PRIORITIES<textarea value={tomorrow} onChange={e=>setTomorrow(e.target.value)} placeholder="Top 3–5 expected results tomorrow" rows={3} style={{...inputStyle,width:"100%",resize:"vertical",display:"block",marginTop:4}}/></label></div><div style={{display:"flex",justifyContent:"flex-end",marginTop:10}}><button onClick={submitMyDay} disabled={submitBusy||busy} style={{fontFamily:"inherit",fontSize:11,fontWeight:950,padding:"9px 15px",border:"1px solid var(--ink)",borderRadius:9,background:"var(--accent)",color:"var(--ink)",cursor:submitBusy?"wait":"pointer"}}>{submitBusy?"Submitting…":mySubmission?"Resubmit Today’s Update":"Submit Today’s Update"}</button></div></div>
  </div>;
}
