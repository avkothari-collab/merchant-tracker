...
import { Check, Plus, Lock, Filter, X, Copy, ChevronUp, ChevronDown, CornerDownRight, Columns3, MessageSquare, RotateCcw, Droplet, Snowflake, Trash2, SkipForward, Bell, Clock } from "lucide-react";
...
function MerchTracker({ me, onSignOut }){
  ...
  const loadShared=async()=>{ try{
    ...
    setFills(f); setNotes(n); try{ const [cmR,prR]=await Promise.all([ supabase.from("comments").select("*").order("created_at"), supabase.from("profiles").select("id,name,role,email") ]); const cg={}; (cmR.data||[]).forEach(r=>{ const ck=r.style_id+":"+r.col; (cg[ck]=cg[ck]||[]).push(r); }); setComments(cg); setTeam(prR.data||[]); const nR=await supabase.from("notifications").select("*").eq("user_id",me.id).order("created_at",{ascending:false}).limit(100); setInbox(nR.data||[]); }catch(e){} loadedRef.current=true; flash();
  ...
  useEffect(()=>{ if(firstRender.current){ firstRender.current=false; return; } if(!loadedRef.current) return; const t=setTimeout(async()=>{ try{ setSaveState("saving");
    ...
    if(stgChanged.length){ const up2=await supabase.from("stage_dates").upsert(stgChanged.map(x=>x.row),{ onConflict:"style_id,stage" }); if(up2.error) throw up2.error; logStageAudit(stgChanged,SR); }
