...
function MerchTracker({ me, onSignOut }){
  const APP_BUILD_LABEL="v31dm Build 10 buyer TNA clarity";
  ...
  const [expOpen,setExpOpen]=useState(false); const [expMode,setExpMode]=useState(()=>{ try{ return localStorage.getItem("mt_exp_mode")||"full"; }catch(e){ return "full"; } }); const [expBuf,setExpBuf]=useState(()=>{ try{ const v=parseInt(localStorage.getItem("mt_exp_buf"),10); return (isFinite(v)&&v>=0&&v<=30)?v:2; }catch(e){ return 2; } }); const [expBuyerPlans,setExpBuyerPlans]=useState(()=>{ try{ return localStorage.getItem("mt_exp_buyer_plans")!=="0"; }catch(e){ return true; } }); const [expBuyerBuffer,setExpBuyerBuffer]=useState(()=>{ try{ const v=localStorage.getItem("mt_exp_buyer_buffer"); return v==null?localStorage.getItem("mt_exp_incbuf")==="1":v==="1"; }catch(e){ return false; } }); const [expRelMode,setExpRelMode]=useState(()=>{ try{ return localStorage.getItem("mt_exp_relmode")||"detailed"; }catch(e){ return "detailed"; } });
  ...
  useEffect(()=>{ try{ localStorage.setItem("mt_exp_mode",expMode); localStorage.setItem("mt_exp_buf",String(expBuf)); localStorage.setItem("mt_exp_buyer_plans",expBuyerPlans?"1":"0"); localStorage.setItem("mt_exp_buyer_buffer",expBuyerBuffer?"1":"0"); localStorage.setItem("mt_exp_relmode",expRelMode); localStorage.setItem("mt_fr_scope",frScope); localStorage.setItem("mt_fr_case",frCase?"1":"0"); }catch(e){} },[expMode,expBuf,expBuyerPlans,expBuyerBuffer,expRelMode,frScope,frCase]);
  ...
  const runExport=(mode,buf,buyerPlans,relMode,buyerBuffer=false)=>{ try{ let data, name, sheet="Tracker"; const B=Math.abs(buf||0);
    ...
    const buyerRisk=(s,c)=>!s.delivery?"Delivery date missing":(c.releaseOnTrack===false?"At Risk":"On Track");
    if(mode==="buyer"){ const useBuyerBuffer=!!(buyerPlans&&buyerBuffer&&B); const buyerStages=STAGES.filter(st=>rows.some(({c})=>!!stageRow(c,st.key))); data=rows.map(({s,c})=>{ const buyerBrand=[s.buyer,s.brand].filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i).join(" / "); const o={ "Order No":s.orderNo||"", "Style No":s.styleNo||"", "Colour":s.colour||"", "Buyer / Brand":buyerBrand, "Qty":s.qty||0, "Delivery":fmtIso(s.delivery), "Current Pending Action":buyerCurrentActivity(s,c,useBuyerBuffer), "Delivery Risk":buyerRisk(s,c), "Last Completed Activity":buyerLastCompleted(c) }; buyerStages.forEach(st=>{ o[buyerStageLabel(st.key)]=buyerDateCell(s,c,st.key,buyerPlans,useBuyerBuffer); }); o["Remarks"]=s.remarks||""; return o; }); name=buyerPlans?(useBuyerBuffer?`buyer_tna_progress_buf${B}d`:"buyer_tna_progress"):"buyer_tna_actuals_only"; sheet="Main Data"; }
