        ...
        <span style={{ marginLeft:10, position:"relative" }}>
          ...
          {threadCell && (()=>{ const ck=cellKey(threadCell.id,threadCell.col); const list=(comments[ck]||[]).slice().sort((a,b)=>new Date(a.created_at)-new Date(b.created_at)); const anyOpen=list.some(x=>!x.resolved); const mq=(cmText.match(/@(\S*)$/)||[])[1]; const sugg = mq!=null ? team.filter(t=>String(t.name||t.email||"").toLowerCase().includes(mq.toLowerCase())).slice(0,5) : [];
                return (<span onClick={e=>e.stopPropagation()} style={{ position:"absolute", top:"100%", left:0, marginTop:4, zIndex:85, background:"#fff", border:"1px solid #1a1a1a", boxShadow:"4px 4px 0 #1a1a1a", padding:10, width:290, display:"block" }}>
                  ...
                  <div style={{ maxHeight:170, overflowY:"auto", marginBottom:6 }}>{list.length===0 && <div style={{ fontSize:10, color:"#999" }}>No comments yet. Use @ to mention a teammate.</div>}{list.map(cm=>(<div key={cm.id} style={{ marginBottom:7, opacity:cm.resolved?0.5:1 }}><div style={{ fontSize:9, color:"#888" }}><b style={{ color:colorFor(cm.author_id||cm.author_name) }}>{cm.author_name||"?"}</b> · {tsShort(cm.created_at)}</div><div style={{ fontSize:11, whiteSpace:"pre-wrap", wordBreak:"break-word" }}>{renderMentions(cm.body)}</div></div>))}</div>
