...
Cancel = cancel edit`); if(!ok) return; recordResendActual(curStyle,field,val,"single"); } pushHistory(); if(STAGE_KEYS.includes(field)){ const ck=id+":"+field+":actual"; if(val) clearedRef.current.delete(ck); else clearedRef.current.add(ck); } setStyles(prev=>prev.map(s=>{ if(s.id!==id) return s; if(STAGE_KEYS.includes(field)) return { ...s, actuals:{ ...s.actuals, [field]: val||undefined } }; if(field==="qty") return { ...s, qty:Number(val)||0 }; return { ...s, [field]:val }; })); flash(); };
  ...
  const handleCopy=(e)=>{ const selectedTxt=(typeof window!=="undefined"&&window.getSelection)?String(window.getSelection().toString()||""):""; if(selectedTxt && selectedTxt.trim()){ return; } const tag=(e.target&&e.target.tagName?String(e.target.tagName).toLowerCase():""); if(tag==="input"||tag==="textarea"||(e.target&&e.target.isContentEditable)) return; const R=rect(); if(!R) return; const lines=[]; let any=false; for(let r=R.r1;r<=R.r2;r++){ const cells=[]; for(let cc=R.c1;cc<=R.c2;cc++){ let v=rows[r]?getVal(rows[r].s,navCols[cc]):""; if(isDateCol(navCols[cc])&&v) v=fmtTyped(v); if(v) any=true; cells.push(v??""); } lines.push(cells.join("\t")); } if(any){ const tsv=lines.join("\n"); try{ e.clipboardData.setData("text/plain",tsv); e.preventDefault(); }catch(err){} setClip({ values:lines.map(l=>l.split("\t")), h:lines.length, w:lines[0].split("\t").length }); flash(); } };
