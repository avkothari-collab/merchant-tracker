...
:root, [data-theme="paper"] {
  --ink:#1a1a1a; --bg:#f4f0e8; --surface:#fff; --accent:#d97706; --accent-tint:#fff7ec;
  --danger:#c0392b; --success:#1f6f54; --info:#1d4ed8;
...
const OWNER_COLOR = { Merchant:"var(--success)", CAD:"#2563a6", Buyer:"#b4531a", Designer:"#6d4aab", Mill:"#7a5a1e", Tamal:"var(--success)", Rina:"#6d4aab" };
...
function MerchTracker({ me, onSignOut }){
  ...
  const [layoutMode,setLayoutMode]=useState(()=>{ try{ const v=localStorage.getItem("mt_layout_mode"); return (v==="workspace"||v==="compact")?v:"compact"; }catch(e){ return "compact"; } });
  useEffect(()=>{ try{ localStorage.setItem("mt_layout_mode", layoutMode); }catch(e){} },[layoutMode]);
  ...
  const ribbonLabel=(label)=> layoutMode==="workspace" ? <span style={{ fontSize:9, fontWeight:700, letterSpacing:0.45, textTransform:"uppercase", color:"var(--accent)", marginRight:2 }}>{label}</span> : null;
  ...
  const [inbox,setInbox]=useState([]); const [bellOpen,setBellOpen]=useState(false);
  ...
  const parseUpload=async(file)=>{ try{ const buf=await file.arrayBuffer(); const wb=XLSX.read(buf,{cellDates:true}); const sn=wb.SheetNames.includes("Tracker")?"Tracker":wb.SheetNames[0]; const aoa=XLSX.utils.sheet_to_json(wb.Sheets[sn],{header:1,raw:false,cellDates:true,defval:""}); let hr=-1; for(let i=0;i<Math.min(aoa.length,6);i++){ if((aoa[i]||[]).some(c=>{ const n=NORMH(c); return n==="styleno"||n==="style"; })){ hr=i; break; } } if(hr<0) hr=0; const headers=(aoa[hr]||[]).map(NORMH); const fi={}; headers.forEach((h,i)=>{ const f=HEADER_MAP[h]; if(f&&fi[f]==null) fi[f]=i; }); if(fi.styleNo==null){ setBulkResult({ error:"Couldn't find a 'Style No' column. Use the template headers." }); return; }
    ...
    const hasOrder = fi.orderNo!=null; const ckey=(o,st)=> hasOrder ? (String(o||"").trim().toLowerCase()+"|"+String(st||"").trim().toLowerCase()) : String(st||"").trim().toLowerCase(); const byKey={}; styles.forEach(s=> byKey[ckey(s.orderNo,s.styleNo)]=s); const inserts=[], updates=[], dupes=[]; let unchanged=0; const seen=new Set();
    recs.forEach(rec=>{ const k=ckey(rec.orderNo,rec.styleNo); if(seen.has(k)){ dupes.push(rec); return; } seen.add(k); const ex=byKey[k]; if(ex){ const chg={}; Object.keys(rec).forEach(f=>{ if(f==="styleNo"||f==="orderNo") return; const nv=rec[f]; if(nv===undefined||nv==="") return; if(String(ex[f]??"")!==String(nv)) chg[f]=nv; }); if(Object.keys(chg).length) updates.push({ id:ex.id, styleNo:rec.styleNo, orderNo:rec.orderNo, chg }); else unchanged++; } else inserts.push(rec); });
    setBulkResult({ inserts, updates, unchanged, dupes, noOrder:!hasOrder, sheetName:sn, total:recs.length });
  ...
  const runExport=(mode,buf,incBuf,relMode)=>{ try{ let data, name, sheet="Tracker"; const B=Math.abs(buf||0);
    ...
    const planCell=(s,c,k,withBuf)=>{ const r=(c.stages||[]).find(x=>x.key===k); if(!r) return "n/a"; if(actCell(s,c,k)) return ""; const t=r.rev||r.plan; if(!t) return ""; const internal=!BUYER_STAGES.includes(k); const d=(withBuf&&B&&internal)?addWorkdays(t,B):t; return fmtY(d); };
  ...
  useEffect(()=>{ try{ localStorage.setItem("mt_trackfilters", JSON.stringify({ search, searchCol, statusFilter, ownerFilter, archiveView, activityFilter, colFilters, tab, todoFilter, followFilter })); }catch(e){} },[search,searchCol,statusFilter,ownerFilter,archiveView,activityFilter,colFilters,tab,todoFilter,followFilter]);
  ...
  const anyFilter = statusFilter!=="All"||ownerFilter!=="All"||!!search||Object.keys(colFilters).length>0||!!activityFilter||followFilter;
  ...
  const filtered=computed.filter(({s,c})=>{ const q=search.toLowerCase(); const ownerMatch=(c.chaseOwners||[]).some(o=>o.owner.toLowerCase().includes(q)); const matchQ = !q ? true : (searchCol==="auto" ? (s.styleNo.toLowerCase().includes(q)||s.colour.toLowerCase().includes(q)||s.family.toLowerCase().includes(q)||s.sampleFit.toLowerCase().includes(q)||s.orderNo.toLowerCase().includes(q)||ownerMatch) : String(s[searchCol]==null?"":s[searchCol]).toLowerCase().includes(q)); const matchS=statusFilter==="All"||(statusFilter==="At Risk"&&(c.tone==="late"||c.tone==="warn"))||(statusFilter==="On Track"&&c.tone==="ok")||(statusFilter==="Released"&&c.released); const matchF=Object.entries(colFilters).every(([col,allowed])=> passCol(s,c,col,allowed)); const matchO=ownerFilter==="All"||(c.chaseOwners||[]).some(o=>o.owner===ownerFilter); const matchA=!activityFilter||(c.frontier&&c.frontier.has(activityFilter)); const matchArch=archiveView==="all"?true:(archiveView==="archived"?!!s.archived:!s.archived); const matchFollow=!followFilter||follows.has(s.id); return matchQ&&matchS&&matchF&&matchO&&matchA&&matchArch&&matchFollow; });
  ...
  const freezeStyle=(col,bg)=> isFrozen(col)?{ position:"sticky", left:leftOf(col), zIndex: col==="__style"?12:11, background:bg, ...(col===lastFrozen?{ borderRight:"2px solid var(--muted-4)" }:{}) }:{};
  ...
  const scrollToCell=(id,col)=>{ requestAnimationFrame(()=>{ const el=document.getElementById(`cell-${id}-${col}`); if(!el) return; el.scrollIntoView({ block:"center", behavior:"smooth" }); const wrap=scrollWrapRef.current; if(wrap){ const cr=el.getBoundingClientRect(), wr=wrap.getBoundingClientRect(); const frozen=STYLE_W+6; if(cr.left < wr.left+frozen){ wrap.scrollLeft -= (wr.left+frozen-cr.left)+8; } else if(cr.right > wr.right){ wrap.scrollLeft += (cr.right-wr.right)+8; } } }); };
  ...
  const FR_COLS=["__style","orderNo","sampleFit","family","colour","brand","fabricType","owner","remarks"];
  const frGet=(s,col)=> col==="__style"?(s.styleNo||""):String(s[col]==null?"":s[col]);
  const escapeRegExp=(v)=> String(v||"").replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const getFindMatches=()=>{ const fRaw=frFind||""; const needle=frCase?fRaw:fRaw.toLowerCase(); if(!needle) return []; let cells=[];
    if(frScope==="selected"){ const R=rect(); if(!R) return []; for(let r=R.r1;r<=R.r2;r++) for(let cc=R.c1;cc<=R.c2;cc++){ const row=rows[r], col=navCols[cc]; if(row&&col&&FR_COLS.includes(col)) cells.push([row.s,col]); } }
    else { for(const row of rows) for(const col of FR_COLS) cells.push([row.s,col]); }
    const out=[]; const seen=new Set();
    for(const [s,col] of cells){ const cur=frGet(s,col); if(!cur) continue; const hay=frCase?cur:cur.toLowerCase(); const idx=hay.indexOf(needle); if(idx===-1) continue; const field=(col==="__style"?"styleNo":col); const key=s.id+":"+field; if(seen.has(key)) continue; seen.add(key); out.push({ id:s.id, col, value:cur, styleNo:s.styleNo||"", orderNo:s.orderNo||"", cell:cellRef({id:s.id,col}), preview:cur.slice(Math.max(0,idx-18), Math.min(cur.length, idx+needle.length+28)), idx }); }
    return out;
  };
  const findNext=()=>{ const matches=getFindMatches(); if(!matches.length){ window.alert('No cells contain "'+frFind+'".'); return; } const ni=(findIdx+1)%matches.length; setFindIdx(ni); const t=matches[ni]; setSel({ id:t.id, col:t.col }); setFocus(null); scrollToCell(t.id, t.col); };
  const findReplace=(preview)=>{ const matches=getFindMatches(); if(preview) return matches.length; if(!matches.length) return 0; const esc=escapeRegExp(frFind||""); const re=new RegExp(esc, frCase?"g":"gi"); const repStr=(frRepl||"").replace(/\$/g,"$$$$"); const ch={};
    matches.forEach(m=>{ const s=styles.find(x=>x.id===m.id); if(!s) return; const col=m.col; const nv=frGet(s,col).replace(re,repStr); if(nv===frGet(s,col)) return; if(canPasteCell(s,col)) (ch[s.id]=ch[s.id]||{})[col]=nv; });
    if(Object.keys(ch).length){ pushHistory(); writeChanges(ch); }
  ...
  const allCommentsFlat = useMemo(()=> Object.entries(comments||{}).flatMap(([ck,list])=>{
    const [sidStr,col] = String(ck).split(":");
    const sid = Number(sidStr);
    const st = styles.find(x=>x.id===sid) || {};
    return (list||[]).map(cm=>({ ...cm, style_id:sid, col, styleNo:st.styleNo||"", orderNo:st.orderNo||"", cellRef: cellRef({id:sid,col}) }));
  }).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)), [comments,styles]);
  const mentionTokens = useMemo(()=>{
    const rawName = String((me&&me.name)||"").trim();
    const rawEmail = String((me&&me.email)||"").trim();
    const tokens = [];
    if(rawName) tokens.push("@"+rawName.replace(/\s+/g,""));
    if(rawEmail) tokens.push("@"+rawEmail.split("@")[0]);
    return tokens.filter(Boolean);
  },[me]);
  const commentsForMe = useMemo(()=> allCommentsFlat.filter(cm=>{
    const body = String(cm.body||"");
    return mentionTokens.some(tok=>body.toLowerCase().includes(tok.toLowerCase())) || cm.author_id===me?.id;
  }), [allCommentsFlat, mentionTokens, me]);
  const openReview=(which="changes")=>{ setReviewTab(which); setReviewOpen(true); if(which==="changes" && auditRows.length===0) openHistory(); };
  const openGuide=(which="user")=>{ setGuideTab(which); setGuideOpen(true); };
  ...
  const openNotif=(n)=>{ markRead(n); setBellOpen(false); if(n.style_id&&n.col){ setSel({ id:n.style_id, col:n.col }); setFocus(null); scrollToCell(n.style_id, n.col); setThreadCell({ id:n.style_id, col:n.col }); } };
  ...
  const logStyleAudit=(changedRows,SR)=>{ try{ const L={ style_no:"Style No", order_no:"Order No", sample_fit:"Sample Fit", family:"Family", colour:"Colour", brand:"Brand", fabric_type:"Fabric Type", owner:"Owner", set_id:"Set ID", set_role:"Set Role", qty:"Qty", order_date:"Order Date", delivery_date:"Delivery", remarks:"Remarks" }; const entries=[]; for(const r of changedRows){ if(!SR.sty[r.id]) continue; let oldRow={}; try{ oldRow=JSON.parse(SR.sty[r.id]); }catch(e){ continue; } const sNo=r.style_no||oldRow.style_no||""; for(const f in L){ const ov=(oldRow[f]==null)?"":String(oldRow[f]); const nv=(r[f]==null)?"":String(r[f]); if(ov!==nv) entries.push({ style_id:r.id, style_no:sNo, col:L[f], field:"value", old_val:ov||null, new_val:nv||null, actor_id:me.id, actor_name:me.name||me.email }); } } if(entries.length) supabase.from("audit_log").insert(entries).then(()=>{}).catch(()=>{}); }catch(e){} };
  const openHistory=async(styleId,col)=>{ setHistory(true); setHistFilter(col?(colLabelOf(col)||""):""); setAuditBusy(true); try{ let q=supabase.from("audit_log").select("*").order("created_at",{ascending:false}).limit(styleId?500:300); if(styleId) q=q.eq("style_id",styleId); const { data }=await q; setAuditRows(data||[]); }catch(e){ setAuditRows([]); } setAuditBusy(false); };
  ...
  const ndFieldFor=(key)=>{ const ni={ width:"100%", boxSizing:"border-box", border:"1px solid #e7dcc2", outline:"none", background:"var(--surface)", fontFamily:"inherit", fontSize:11, padding:"3px 4px" }; const stop=e=>e.stopPropagation(); const ent=e=>{ if(e.key==="Enter") addNewStyle(); };
    ...
    const ph={ orderNo:"order# *", sampleFit:"fit#", family:"family", colour:"colour", brand:"brand", fabricType:"fabric", owner:"owner", age:"age", setId:"set id", setRole:"set role" }[key]||"";
  ...
  return (
    <div ref={gridRef} tabIndex={0} onKeyDown={onKeyDown} onCopy={handleCopy} onPaste={handlePaste} onMouseDown={(e)=>{ if(editing) return; if(e.target.closest && (e.target.closest("input")||e.target.closest("button")||e.target.closest("th"))) return; const td=e.target.closest && e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m||e.shiftKey) return; e.preventDefault(); gridRef.current&&gridRef.current.focus(); setSel({ id:Number(m[1]), col:m[2] }); setFocus(null); selectingRef.current=true; setDragSel(true); }} onMouseUp={()=>{ if(filling){ applyFillHandle(); setFilling(false); setFillFrom(null); setFillTo(null); } if(selectingRef.current){ selectingRef.current=false; setDragSel(false); } }} onMouseOver={(e)=>{ const td=e.target.closest && e.target.closest('td[id^="cell-"]'); if(!td) return; const m=td.id.match(/^cell-(\d+)-(.+)$/); if(!m) return; if(filling){ setFillTo({ id:Number(m[1]), col:m[2] }); return; } if(selectingRef.current){ setFocus({ id:Number(m[1]), col:m[2] }); } }} onClick={()=>{ finishEditing(); setFillOpen(false); setColsOpen(false); setFilterCol(null); setFrOpen(false); setExpOpen(false); setBellOpen(false); }}
      ...
      <div style={{ background:"var(--ink)", color:"var(--bg)", padding:"14px 22px", display:"flex", alignItems:"center", justifyContent:"space-between", borderBottom:"3px solid var(--accent)" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12 }}><span style={{ fontSize:11, color: saveState==="error"?"#e8746b":saveState==="saving"?"#d9b46a":saveState==="saved"?"#7fd1a8":"#6a665e" }}>{saveState==="error"?"⚠ save failed":saveState==="saving"?"… saving":saveState==="saved"?"● saved to cloud":"○ connected"}</span><span style={{ fontSize:11, color:"var(--on-dark)", whiteSpace:"nowrap" }}>{(me&&(me.name||me.email))||""} · <b style={{ color:"var(--accent)" }}>{(ROLES[role]||{}).label||role}</b></span>{peers.length>0 && (<span style={{ display:"flex", alignItems:"center", gap:3 }} title={peers.map(p=>p.name).join(", ")}>{peers.slice(0,6).map(p=>(<span key={p.id} title={(p.name||"")+" · "+((ROLES[p.role]||{}).label||p.role)} style={{ width:18, height:18, borderRadius:9, background:colorFor(p.id), color:"var(--surface)", fontSize:8, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center" }}>{initials(p.name)}</span>))}<span style={{ fontSize:9, color:"#9b958a", marginLeft:2 }}>{peers.length} here</span></span>)}<button onClick={(e)=>{ e.stopPropagation(); setSidebarOpen(v=>!v); }} title={sidebarOpen?"Collapse sidebar":"Expand sidebar"} style={{ fontFamily:"inherit", fontSize:11, padding:"5px 8px", cursor:"pointer", border:"1px solid var(--on-dark-line)", background:"transparent", color:"var(--on-dark)", fontWeight:700 }}>{sidebarOpen?"⇤":"⇥"}</button><span title="switch between the current compact grid controls and a sticky workspace toolbar" style={{ display:"inline-flex", border:"1px solid var(--on-dark-line)", overflow:"hidden" }}><button onClick={(e)=>{ e.stopPropagation(); setLayoutMode("compact"); }} style={{ fontFamily:"inherit", fontSize:10, padding:"5px 8px", cursor:"pointer", border:"none", borderRight:"1px solid var(--on-dark-line)", background:layoutMode==="compact"?"var(--accent)":"transparent", color:layoutMode==="compact"?"var(--ink)":"var(--on-dark)", fontWeight:700 }}>Compact</button><button onClick={(e)=>{ e.stopPropagation(); setLayoutMode("workspace"); }} style={{ fontFamily:"inherit", fontSize:10, padding:"5px 8px", cursor:"pointer", border:"none", background:layoutMode==="workspace"?"var(--accent)":"transparent", color:layoutMode==="workspace"?"var(--ink)":"var(--on-dark)", fontWeight:700 }}>Workspace</button></span><button onClick={(e)=>{ e.stopPropagation(); openReview("changes"); }} title="Review changes, comments, and mentions" style={{ fontFamily:"inherit", fontSize:10, padding:"5px 8px", cursor:"pointer", border:"1px solid var(--on-dark-line)", background:"transparent", color:"var(--on-dark)", display:"inline-flex", alignItems:"center", gap:5 }}><Clock size={13}/>Review</button><button onClick={(e)=>{ e.stopPropagation(); openGuide("user"); }} title="User guide and logic guide" style={{ fontFamily:"inherit", fontSize:10, padding:"5px 8px", cursor:"pointer", border:"1px solid var(--on-dark-line)", background:"transparent", color:"var(--on-dark)", fontWeight:700 }}>Guide</button><span style={{ position:"relative" }}><button onClick={(e)=>{ e.stopPropagation(); setBellOpen(o=>!o); }} title="Notifications" style={{ fontFamily:"inherit", fontSize:10, padding:"5px 8px", cursor:"pointer", border:"1px solid var(--on-dark-line)", background:bellOpen?"var(--on-dark-line)":"transparent", color:"var(--on-dark)", position:"relative", display:"inline-flex", alignItems:"center" }}><Bell size={13}/>{unreadCount>0 && <span style={{ position:"absolute", top:-5, right:-5, background:"#e8746b", color:"var(--surface)", fontSize:8, fontWeight:700, minWidth:14, height:14, borderRadius:7, display:"flex", alignItems:"center", justifyContent:"center", padding:"0 3px" }}>{unreadCount>9?"9+":unreadCount}</span>}</button>{bellOpen && (<div onClick={e=>e.stopPropagation()} style={{ position:"absolute", top:"100%", right:0, marginTop:6, zIndex:90, background:"var(--surface)", color:"var(--ink)", border:"1px solid var(--ink)", boxShadow:"4px 4px 0 var(--ink)", width:320, maxHeight:380, display:"flex", flexDirection:"column" }}><div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 10px", borderBottom:"1px solid var(--line-3)" }}><span style={{ fontSize:11, fontWeight:700 }}>Notifications{unreadCount>0?" ("+unreadCount+" new)":""}</span><span style={{ display:"flex", gap:8 }}>{unreadCount>0 && <button onClick={markAllRead} style={{ fontSize:9, border:"none", background:"transparent", color:"var(--info)", cursor:"pointer", fontFamily:"inherit", fontWeight:700 }}>Mark all read</button>}{inbox.length>0 && <button onClick={clearInbox} style={{ fontSize:9, border:"none", background:"transparent", color:"var(--danger)", cursor:"pointer", fontFamily:"inherit", fontWeight:700 }}>Clear</button>}</span></div><div style={{ overflowY:"auto" }}>{inbox.length===0 ? <div style={{ fontSize:10, color:"var(--muted-1)", padding:"14px 10px", textAlign:"center" }}>No notifications yet.</div> : inbox.map(n=>(<div key={n.id} onClick={()=>openNotif(n)} style={{ padding:"8px 10px", borderBottom:"1px solid #f2f2f2", cursor:"pointer", background:n.read?"var(--surface)":"var(--accent-tint)", display:"flex", gap:7, alignItems:"flex-start" }}><span style={{ width:6, height:6, borderRadius:6, marginTop:4, flex:"0 0 auto", background:n.read?"transparent":"var(--accent)" }}/><span style={{ flex:1 }}><span style={{ fontSize:11, lineHeight:1.35, display:"block" }}>{n.body}</span><span style={{ fontSize:9, color:"var(--muted-1)" }}>{tsShort(n.created_at)}</span></span></div>))}</div></div>)}</span>{canManageUsers(role) && <button onClick={(e)=>{ e.stopPropagation(); setUsersOpen(true); }} style={{ fontFamily:"inherit", fontSize:10, padding:"5px 9px", cursor:"pointer", border:"1px solid var(--on-dark-line)", background:"transparent", color:"var(--on-dark)" }}>Users</button>}<button onClick={(e)=>{ e.stopPropagation(); onSignOut&&onSignOut(); }} style={{ fontFamily:"inherit", fontSize:10, padding:"5px 9px", cursor:"pointer", border:"1px solid var(--on-dark-line)", background:"transparent", color:"var(--on-dark)" }}>Sign out</button></div>
      </div>

      <div style={{ display:"flex", alignItems:"stretch", minHeight:"calc(100vh - 84px)" }}>
        <div style={{ width:sidebarW, flex:"0 0 auto", background:"var(--ink)", color:"var(--bg)", borderRight:"2px solid #3a362e", position:"sticky", top:0, height:"calc(100vh - 0px)", alignSelf:"flex-start", zIndex:57 }}>
          <div style={{ padding:"12px 8px", borderBottom:"1px solid #3a362e", display:"flex", justifyContent:sidebarOpen?"space-between":"center", alignItems:"center" }}>
            {sidebarOpen ? <span style={{ fontSize:9, color:"#9a958c", letterSpacing:0.6, textTransform:"uppercase" }}>Navigation</span> : null}
            <button onClick={(e)=>{ e.stopPropagation(); setSidebarOpen(v=>!v); }} title={sidebarOpen?"Collapse sidebar":"Expand sidebar"} style={{ border:"1px solid var(--on-dark-line)", background:"transparent", color:"var(--on-dark)", cursor:"pointer", fontFamily:"inherit", fontSize:10, padding:"4px 7px" }}>{sidebarOpen?"⇤":"⇥"}</button>
          </div>
          <div style={{ padding:"8px 6px", display:"flex", flexDirection:"column", gap:6 }}>
            {[["tracker","Tracker"],["dashboard","Dashboard"],["todo","To-Do"],["settings","Settings"]].map(([k,lab])=>(<button key={k} onClick={(e)=>{ e.stopPropagation(); setTab(k); }} title={lab+" view"} style={{ fontFamily:"'Archivo',sans-serif", fontWeight:700, fontSize:12, letterSpacing:0.3, padding:sidebarOpen?"10px 12px":"10px 8px", cursor:"pointer", border:"1px solid "+(tab===k?"var(--accent)":"#3a362e"), background:tab===k?"rgba(217,119,6,0.18)":"transparent", color:tab===k?"var(--bg)":"#c7c1b7", textAlign:sidebarOpen?"left":"center" }}>{sidebarOpen?lab:lab[0]}{k==="todo"&&todoItems.length&&sidebarOpen?` · ${todoItems.length}`:""}</button>))}
            <button onClick={(e)=>{ e.stopPropagation(); openReview("changes"); }} title="Review changes, all comments, and comments involving me" style={{ fontFamily:"'Archivo',sans-serif", fontWeight:700, fontSize:12, letterSpacing:0.3, padding:sidebarOpen?"10px 12px":"10px 8px", cursor:"pointer", border:"1px solid #3a362e", background:"transparent", color:"#c7c1b7", textAlign:sidebarOpen?"left":"center" }}>{sidebarOpen?"Review":"R"}</button>
            <button onClick={(e)=>{ e.stopPropagation(); openGuide("user"); }} title="User guide and logic guide" style={{ fontFamily:"'Archivo',sans-serif", fontWeight:700, fontSize:12, letterSpacing:0.3, padding:sidebarOpen?"10px 12px":"10px 8px", cursor:"pointer", border:"1px solid #3a362e", background:"transparent", color:"#c7c1b7", textAlign:sidebarOpen?"left":"center" }}>{sidebarOpen?"Guides":"G"}</button>
          </div>
          <div style={{ marginTop:"auto", padding:"10px 8px", fontSize:9, color:"#9a958c", letterSpacing:0.5, textTransform:"uppercase" }}>{sidebarOpen?(layoutMode==="workspace"?"workspace ribbon":"compact controls"):(layoutMode==="workspace"?"W":"C")}</div>
        </div>
...
{history && (<div onClick={()=>setHistory(false)} style={{ position:"fixed", inset:0, background:"rgba(26,26,26,0.55)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}><div onClick={e=>e.stopPropagation()} style={{ background:"var(--surface)", border:"2px solid var(--ink)", boxShadow:"8px 8px 0 var(--ink)", width:640, maxWidth:"100%", maxHeight:"80vh", display:"flex", flexDirection:"column", fontFamily:"'JetBrains Mono',monospace" }}><div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 14px", borderBottom:"1px solid var(--line-3)" }}><div style={{ fontFamily:"'Archivo',sans-serif", fontWeight:800, fontSize:17 }}>Change history</div><button onClick={()=>setHistory(false)} style={{ border:"1px solid var(--ink)", background:"var(--bg)", cursor:"pointer", fontFamily:"inherit", fontSize:11, padding:"4px 9px" }}>Close</button></div><div style={{ padding:"8px 14px", borderBottom:"1px solid #f2f2f2" }}><input value={histFilter} onChange={e=>setHistFilter(e.target.value)} placeholder="filter by style no, field, stage, or person…" style={{ width:"100%", boxSizing:"border-box", fontFamily:"inherit", fontSize:11, padding:6, border:"1px solid var(--ink)" }}/></div><div style={{ overflowY:"auto", padding:"4px 0" }}>{auditBusy ? <div style={{ fontSize:11, color:"var(--muted-1)", padding:"24px", textAlign:"center" }}>Loading…</div> : (()=>{ const q=histFilter.trim().toLowerCase(); const fl=auditRows.filter(a=> !q || (a.style_no||"").toLowerCase().includes(q) || (a.field||"").toLowerCase().includes(q) || (a.actor_name||"").toLowerCase().includes(q) || (colLabelOf(a.col)||"").toLowerCase().includes(q)); if(fl.length===0) return <div style={{ fontSize:11, color:"var(--muted-1)", padding:"24px", textAlign:"center" }}>{auditRows.length===0?"No changes recorded yet.":"No matches."}</div>; return fl.map(a=>(<div key={a.id} style={{ padding:"7px 14px", borderBottom:"1px solid #f5f5f5", fontSize:11, display:"flex", gap:10, alignItems:"baseline" }}><span style={{ color:"var(--muted-1)", fontSize:9, whiteSpace:"nowrap", minWidth:80 }}>{tsShort(a.created_at)}</span><span style={{ flex:1 }}><b>{a.style_no||a.style_id}</b> · {colLabelOf(a.col)} <span style={{ color:"var(--muted-2)" }}>({a.field})</span><br/><span style={{ color:"var(--danger)" }}>{a.old_val||"—"}</span> <span style={{ color:"var(--muted-2)" }}>→</span> <span style={{ color:"var(--success)" }}>{a.new_val||"—"}</span> <span style={{ color:"var(--muted-1)" }}>· {a.actor_name}</span></span></div>)); })()}</div></div></div>)}
      ...
      {reviewOpen && (<div onClick={()=>setReviewOpen(false)} style={{ position:"fixed", inset:0, background:"rgba(26,26,26,0.55)", zIndex:201, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
        <div onClick={e=>e.stopPropagation()} style={{ background:"var(--surface)", border:"2px solid var(--ink)", boxShadow:"8px 8px 0 var(--ink)", width:900, maxWidth:"100%", maxHeight:"84vh", display:"flex", flexDirection:"column", overflow:"hidden", fontFamily:"'JetBrains Mono',monospace" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 14px", borderBottom:"1px solid var(--line-3)" }}>
            <div style={{ fontFamily:"'Archivo',sans-serif", fontWeight:800, fontSize:18 }}>Review centre</div>
            <button onClick={()=>setReviewOpen(false)} style={{ border:"1px solid var(--ink)", background:"var(--bg)", cursor:"pointer", fontFamily:"inherit", fontSize:11, padding:"4px 9px" }}>Close</button>
          </div>
          <div style={{ display:"flex", gap:8, padding:"10px 14px", borderBottom:"1px solid var(--line-3)", background:"#faf7f0" }}>
            {[["changes","Changes to sheet"],["comments","All comments"],["mentions","Comments involving me"]].map(([k,lab])=><button key={k} onClick={()=>{ setReviewTab(k); if(k==="changes"&&auditRows.length===0) openHistory(); }} style={{ fontFamily:"inherit", fontSize:10, fontWeight:700, padding:"5px 9px", cursor:"pointer", border:"1px solid var(--ink)", background:reviewTab===k?"var(--accent)":"var(--surface)" }}>{lab}</button>)}
          </div>
          <div style={{ overflowY:"auto", padding:"8px 0", flex:1 }}>
            {reviewTab==="changes" && (auditBusy ? <div style={{ fontSize:11, color:"var(--muted-1)", padding:"24px", textAlign:"center" }}>Loading…</div> : auditRows.length===0 ? <div style={{ fontSize:11, color:"var(--muted-1)", padding:"24px", textAlign:"center" }}>No changes recorded yet.</div> : auditRows.map(a=>(<div key={a.id} style={{ padding:"8px 14px", borderBottom:"1px solid #f5f5f5", fontSize:11, display:"flex", gap:10, alignItems:"baseline" }}><span style={{ color:"var(--muted-1)", fontSize:9, whiteSpace:"nowrap", minWidth:90 }}>{tsShort(a.created_at)}</span><span style={{ flex:1 }}><b>{a.style_no||a.style_id}</b> · {colLabelOf(a.col)} <span style={{ color:"var(--muted-2)" }}>({a.field})</span><br/><span style={{ color:"var(--danger)" }}>{a.old_val||"—"}</span> <span style={{ color:"var(--muted-2)" }}>→</span> <span style={{ color:"var(--success)" }}>{a.new_val||"—"}</span> <span style={{ color:"var(--muted-1)" }}>· {a.actor_name}</span></span></div>)))}
            {reviewTab==="comments" && (allCommentsFlat.length===0 ? <div style={{ fontSize:11, color:"var(--muted-1)", padding:"24px", textAlign:"center" }}>No comments in sheet yet.</div> : allCommentsFlat.map(cm=>(<div key={cm.id} style={{ padding:"8px 14px", borderBottom:"1px solid #f5f5f5", fontSize:11 }}><div style={{ display:"flex", gap:8, alignItems:"baseline" }}><span style={{ color:"var(--muted-1)", fontSize:9, minWidth:90 }}>{tsShort(cm.created_at)}</span><span><b>{cm.styleNo||cm.style_id}</b> · {cm.cellRef} <span style={{ color:"var(--muted-1)" }}>· {cm.author_name||"?"}</span>{cm.resolved?<span style={{ color:"var(--success)" }}> · resolved</span>:""}</span></div><div style={{ marginLeft:98, whiteSpace:"pre-wrap" }}>{renderMentions(cm.body)}</div></div>)))}
            {reviewTab==="mentions" && (commentsForMe.length===0 ? <div style={{ fontSize:11, color:"var(--muted-1)", padding:"24px", textAlign:"center" }}>No comments involving you yet.</div> : commentsForMe.map(cm=>(<div key={cm.id} style={{ padding:"8px 14px", borderBottom:"1px solid #f5f5f5", fontSize:11 }}><div style={{ display:"flex", gap:8, alignItems:"baseline" }}><span style={{ color:"var(--muted-1)", fontSize:9, minWidth:90 }}>{tsShort(cm.created_at)}</span><span><b>{cm.styleNo||cm.style_id}</b> · {cm.cellRef} <span style={{ color:"var(--muted-1)" }}>· {cm.author_name||"?"}</span>{cm.resolved?<span style={{ color:"var(--success)" }}> · resolved</span>:""}</span></div><div style={{ marginLeft:98, whiteSpace:"pre-wrap" }}>{renderMentions(cm.body)}</div></div>)))}
          </div>
        </div>
      </div>)}
      {guideOpen && (<div onClick={()=>setGuideOpen(false)} style={{ position:"fixed", inset:0, background:"rgba(26,26,26,0.55)", zIndex:201, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
        <div onClick={e=>e.stopPropagation()} style={{ background:"var(--surface)", border:"2px solid var(--ink)", boxShadow:"8px 8px 0 var(--ink)", width:920, maxWidth:"100%", maxHeight:"84vh", display:"flex", flexDirection:"column", overflow:"hidden", fontFamily:"'JetBrains Mono',monospace" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 14px", borderBottom:"1px solid var(--line-3)" }}>
            <div style={{ fontFamily:"'Archivo',sans-serif", fontWeight:800, fontSize:18 }}>Guides</div>
            <button onClick={()=>setGuideOpen(false)} style={{ border:"1px solid var(--ink)", background:"var(--bg)", cursor:"pointer", fontFamily:"inherit", fontSize:11, padding:"4px 9px" }}>Close</button>
          </div>
          <div style={{ display:"flex", gap:8, padding:"10px 14px", borderBottom:"1px solid var(--line-3)", background:"#faf7f0" }}>
            {[["user","User guide"],["logic","Logic guide"]].map(([k,lab])=><button key={k} onClick={()=>setGuideTab(k)} style={{ fontFamily:"inherit", fontSize:10, fontWeight:700, padding:"5px 9px", cursor:"pointer", border:"1px solid var(--ink)", background:guideTab===k?"var(--accent)":"var(--surface)" }}>{lab}</button>)}
          </div>
          <div style={{ overflowY:"auto", padding:"16px 18px", fontSize:11, lineHeight:1.65 }}>
            {guideTab==="user" ? <>
              <div style={{ fontFamily:"'Archivo',sans-serif", fontWeight:800, fontSize:18, marginBottom:8 }}>Button-wise user guide</div>
              <p><b>Compact / Workspace</b> switches between the dense working strip and the ribbon workspace. Compact is for fast daily use. Workspace is for guided commands.</p>
              <p><b>Sidebar</b> switches views: Tracker, Dashboard, To-Do, Settings, Review, Guides. Collapse it for more sheet width.</p>
              <p><b>Review</b> opens one place to see recent sheet changes, all comments, and comments involving you.</p>
              <p><b>Guide</b> opens this help panel. Keep it as the built-in operating manual for new users.</p>
              <p><b>Search</b> filters the visible list by Auto or a chosen column. <b>Status / Owner / Activity / Following / Archive</b> narrow the visible sheet.</p>
              <p><b>Find / Replace</b> shows match results with cell reference and matching value. Click a result to jump there. Replace all works on the current result set and asks for confirmation first.</p>
              <p><b>Columns</b> shows or hides info columns. <b>Freeze</b> controls how many leading columns stay sticky while you scroll.</p>
              <p><b>Export</b> creates Excel outputs for buyer, release, internal, detailed, or full views.</p>
              <p><b>Sync</b> reloads shared data from Supabase. Use it if another user changed data and you want the latest view immediately.</p>
              <p><b>Comments</b> work per cell. Use <b>@Name</b> to mention a teammate. <b>Follow</b> gives notifications for a style. <b>History</b> shows the audit trail for the selected cell.</p>
              <p><b>Auto+Rev</b> shows the auto plan and revised date in each stage cell. <b>Go to</b> jumps directly to a cell like A9.</p>
            </> : <>
              <div style={{ fontFamily:"'Archivo',sans-serif", fontWeight:800, fontSize:18, marginBottom:8 }}>Logic guide</div>
              <p><b>Core flow</b>: Techpack → Fit → Artwork / Strike-off → Lab Dip → Fabric In-House → PP → Prod File, with stage applicability controlled by flags such as Fit Required, Print Required, S/O Required, Lab Dip Required, PP Needed, and PP Bypass.</p>
              <p><b>Actual vs Revised vs Auto</b>: actual date wins, then revised, then auto-calculated plan. Skipped activities count as resolved, not done. Rejections can trigger rework and resend logic.</p>
              <p><b>PP bypass</b>: if PP is bypassed or not needed, Prod File follows Fabric IH instead of PP Approval.</p>
              <p><b>Projected release</b> is aligned to the displayed Prod File date so what the user sees in the grid matches the headline release projection.</p>
              <p><b>To-Do frontier</b> only shows actions that are actually actionable now, based on predecessor completion and applicability.</p>
              <p><b>Autosave</b> is debounced. The app compares current rows against the last saved snapshot and only upserts changed rows, stages, and cell meta.</p>
              <p><b>Realtime</b> merges changes from other users through Supabase channels for styles, stage dates, comments, notifications, and presence.</p>
              <p><b>Review centre</b> combines the audit log and the full comment stream so managers can review what changed and who is involved without opening individual cells one by one.</p>
            </>}
          </div>
        </div>
      </div>)}
      {bulkOpen && (<div onClick={()=>{ setBulkOpen(false); setBulkResult(null); }} style={{ position:"fixed", inset:0, background:"rgba(26,26,26,0.55)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
        <div onClick={e=>e.stopPropagation()} style={{ background:"var(--bg)", border:"2px solid var(--ink)", boxShadow:"8px 8px 0 var(--ink)", width:560, maxWidth:"100%", maxHeight:"86vh", overflowY:"auto", padding:22, fontFamily:"'JetBrains Mono',monospace" }}>
          ...
          <p style={{ fontSize:11, color:"var(--muted-3)", lineHeight:1.5 }}>Upload an Excel (.xlsx) or CSV. Rows are matched by <b>Style No</b> — existing styles are updated, new ones are added. Nothing is saved until you confirm.</p>
          ...
          {bulkResult && !bulkResult.error && (<div style={{ marginBottom:12 }}>
            ...
            <div style={{ display:"flex", gap:10, marginBottom:10 }}>
              ...
              <div style={{ flex:1, border:"1px solid var(--ink)", background:"var(--surface)", padding:"8px 10px" }}><div style={{ fontSize:22, fontWeight:800, color:"var(--accent)", fontFamily:"'Archivo',sans-serif" }}>{bulkResult.updates.length}</div><div style={{ fontSize:9, color:"var(--muted-2)", textTransform:"uppercase" }}>updated</div></div>
              <div style={{ flex:1, border:"1px solid var(--ink)", background:"var(--surface)", padding:"8px 10px" }}><div style={{ fontSize:22, fontWeight:800, color:"var(--muted-1)", fontFamily:"'Archivo',sans-serif" }}>{bulkResult.unchanged}</div><div style={{ fontSize:9, color:"var(--muted-2)", textTransform:"uppercase" }}>unchanged</div></div>
            </div>
            {bulkResult.updates.length>0 && <div style={{ maxHeight:140, overflowY:"auto", border:"1px solid var(--line-1)", background:"var(--surface)", padding:8, fontSize:10 }}><div style={{ fontWeight:700, marginBottom:4 }}>Changes to existing styles:</div>{bulkResult.updates.slice(0,40).map(u=>(<div key={u.id} style={{ padding:"2px 0", borderBottom:"1px solid #f0ece3" }}><b>{u.styleNo}</b> — {Object.keys(u.chg).join(", ")}</div>))}{bulkResult.updates.length>40 && <div style={{ color:"var(--muted-1)" }}>…and {bulkResult.updates.length-40} more</div>}</div>}{bulkResult.dupes&&bulkResult.dupes.length>0 && <div style={{ fontSize:10, color:"var(--danger)", fontWeight:700, marginTop:8, background:"#fbeaea", border:"1px solid #e8b4ae", padding:"6px 8px" }}>⚠ {bulkResult.dupes.length} row(s) repeat the same Order + Style in this file — skipped, no duplicates created.</div>}{bulkResult.noOrder && <div style={{ fontSize:9, color:"#a06000", marginTop:6 }}>No "Order No" column found — matched by Style No only. Add an Order No column to match on Order + Style.</div>}
          </div>)}
          {bulkResult && !bulkResult.error && (bulkResult.inserts.length+bulkResult.updates.length>0) && <button onClick={applyBulk} style={{ fontFamily:"inherit", fontSize:12, padding:"9px 16px", cursor:"pointer", border:"1px solid var(--ink)", background:"var(--success)", color:"var(--surface)", fontWeight:700 }}>Confirm — apply {bulkResult.inserts.length+bulkResult.updates.length} change(s)</button>}
        </div>
      </div>)}

      {tab==="tracker" && (<>
      <div style={{ display:"flex", padding:"12px 22px 0", flexWrap:"wrap" }}>
        {Object.entries(funnel).map(([k,v],i,arr)=>(<div key={k} style={{ flex:1, minWidth:90, background:"var(--surface)", border:"1px solid var(--ink)", borderRight:i===arr.length-1?"1px solid var(--ink)":"none", padding:"8px 10px" }}><div style={{ fontSize:22, fontWeight:700, lineHeight:1, fontFamily:"'Archivo',sans-serif", color:k==="Released"?"var(--success)":k==="Fabric IH"?"var(--danger)":"var(--ink)" }}>{v}</div><div style={{ fontSize:9, color:"var(--muted-2)", marginTop:3, letterSpacing:0.5, textTransform:"uppercase" }}>{k}</div></div>))}
      </div>

      <div style={layoutMode==="workspace"?{ position:"sticky", top:0, zIndex:54, background:"var(--bg)", paddingBottom:8, borderBottom:"1px solid #e6dfd2", boxShadow:"0 6px 14px rgba(26,26,26,0.08)" }:null}>
      {layoutMode==="workspace" && <div style={{ display:"flex", alignItems:"flex-end", gap:16, padding:"9px 22px 0", fontSize:9.5, color:"var(--muted-2)", textTransform:"uppercase", letterSpacing:0.55 }}>{["home","clipboard","search","actions","view","collab"].map(k=>ribbonTabBtn(k, k==="collab"?"Collab":(k[0].toUpperCase()+k.slice(1))))}<span style={{ marginLeft:"auto", textTransform:"none", letterSpacing:0, fontSize:9, color:"var(--muted-1)", paddingBottom:5 }}>Workspace ribbon · labels are clickable. Compact keeps the original strip.</span></div>}
{(layoutMode!=="workspace" || ["home","search","actions","view"].includes(activeRibbon)) && <div style={layoutMode==="workspace"?{ display:"flex", gap:14, alignItems:"stretch", padding:"10px 14px 12px", margin:"6px 22px 8px", flexWrap:"wrap", background:"linear-gradient(180deg,#fbfaf7 0%, #f4efe6 100%)", border:"1px solid #d7cfbf", boxShadow:"4px 4px 0 rgba(26,26,26,0.10)" }:{ display:"flex", gap:10, alignItems:"center", padding:"12px 22px 6px", flexWrap:"wrap" }}>{ribbonLabel(activeRibbon==="view"?"View":"Search / filters")}
        <div style={{ display:"flex", alignItems:"center", gap:6, background:"var(--surface)", border:"1px solid var(--ink)", padding:"5px 9px" }}><Filter size={13}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder={searchCol==="auto"?"search style / colour / fit / order…":"search selected column…"} onClick={e=>e.stopPropagation()} style={{ border:"none", outline:"none", fontFamily:"inherit", fontSize:12, width:160, background:"transparent" }}/><span style={{ width:1, height:16, background:"var(--line-2)" }}/><select value={searchCol} onChange={e=>setSearchCol(e.target.value)} onClick={e=>e.stopPropagation()} title="search a specific column (Auto = smart search across the main fields)" style={{ border:"none", outline:"none", fontFamily:"inherit", fontSize:10, background:"transparent", color:searchCol==="auto"?"var(--muted-2)":"var(--accent)", cursor:"pointer", fontWeight:searchCol==="auto"?400:700 }}>{[["auto","Auto"],["styleNo","Style No"],["colour","Colour"],["family","Family"],["sampleFit","Sample Fit"],["orderNo","Order No"],["owner","Owner"],["brand","Brand"],["buyer","Buyer"],["age","Age Group"],["fabricType","Fabric Type"],["setRole","Set Role"],["remarks","Remarks"]].map(([k,lab])=><option key={k} value={k}>{lab}</option>)}</select></div>
        ...
        <div style={{ display:"flex", border:"1px solid var(--ink)" }}>{["All","Merchant","CAD","Buyer","Designer","Mill"].map(f=>(<button key={f} onClick={(e)=>{ e.stopPropagation(); setOwnerFilter(f); }} title="chase owner" style={{ fontFamily:"inherit", fontSize:11, padding:"6px 9px", cursor:"pointer", border:"none", borderRight:f!=="Mill"?"1px solid var(--ink)":"none", background:ownerFilter===f?"#2563a6":"var(--surface)", color:ownerFilter===f?"var(--surface)":"var(--ink)" }}>{f==="All"?"Owner":f}</button>))}</div>
        ...
        <button disabled={!specialClip} onClick={(e)=>{ e.stopPropagation(); pasteSpecial(); }} title="Paste the copied full state into the selected stage cell" style={{ fontFamily:"inherit", fontSize:11, padding:"6px 10px", cursor:specialClip?"pointer":"not-allowed", border:"1px solid var(--ink)", background:specialClip?"var(--success)":"var(--surface)", color:specialClip?"var(--surface)":"var(--muted-6)", fontWeight:700, opacity:specialClip?1:0.6 }}>Paste✦</button>
        <span style={{ position:"relative" }}><button onClick={(e)=>{ e.stopPropagation(); setFrOpen(o=>!o); }} title="Find & replace text in cells" style={{ fontFamily:"inherit", fontSize:11, padding:"6px 10px", cursor:"pointer", border:"1px solid var(--ink)", background:frOpen?"var(--ink)":"var(--surface)", color:frOpen?"var(--bg)":"var(--ink)", fontWeight:700 }}>⌕ Find/Replace</button>{frOpen && (()=>{ const matches=getFindMatches(); return (<div onClick={e=>e.stopPropagation()} style={{ position:"absolute", top:"100%", left:0, marginTop:4, zIndex:75, background:"var(--surface)", border:"1px solid var(--ink)", boxShadow:"4px 4px 0 var(--ink)", padding:12, width:330 }}><div style={{ fontSize:11, fontWeight:700, marginBottom:8 }}>Find &amp; replace</div><label style={{ fontSize:10, color:"var(--muted-2)" }}>Find</label><input autoFocus value={frFind} onChange={e=>{ setFrFind(e.target.value); setFindIdx(-1); }} placeholder="text to find" style={{ width:"100%", boxSizing:"border-box", fontFamily:"inherit", fontSize:11, padding:5, marginBottom:6, border:"1px solid var(--ink)" }}/><label style={{ fontSize:10, color:"var(--muted-2)" }}>Replace with</label><input value={frRepl} onChange={e=>setFrRepl(e.target.value)} placeholder="(leave blank to delete)" style={{ width:"100%", boxSizing:"border-box", fontFamily:"inherit", fontSize:11, padding:5, marginBottom:8, border:"1px solid var(--ink)" }}/><div style={{ display:"flex", gap:12, fontSize:10, marginBottom:6 }}><label style={{ display:"flex", gap:4, cursor:"pointer" }}><input type="radio" checked={frScope==="filtered"} onChange={()=>setFrScope("filtered")}/>All filtered</label><label style={{ display:"flex", gap:4, cursor:"pointer" }}><input type="radio" checked={frScope==="selected"} onChange={()=>setFrScope("selected")}/>Selected cells</label></div><label style={{ fontSize:10, display:"flex", gap:5, marginBottom:8, cursor:"pointer" }}><input type="checkbox" checked={frCase} onChange={e=>setFrCase(e.target.checked)}/>Match case</label><div style={{ fontSize:10, color:"var(--muted-3)", marginBottom:8, minHeight:13 }}>{frScope==="selected" && !rect() ? "Select cells in the grid first." : (frFind ? matches.length+" match(es) found" : "Searches text fields like style, colour, brand, owner and remarks.")}</div>{frFind && <div style={{ maxHeight:164, overflowY:"auto", border:"1px solid #ece6da", marginBottom:8 }}>{matches.length?matches.slice(0,12).map((m,i)=><button key={m.id+":"+m.col} onClick={(e)=>{ e.stopPropagation(); setFindIdx(i); setSel({id:m.id,col:m.col}); setFocus(null); scrollToCell(m.id,m.col); }} style={{ width:"100%", textAlign:"left", border:"none", borderBottom:"1px solid #f1ede5", background:i===findIdx?"var(--accent-tint)":"var(--surface)", cursor:"pointer", padding:"6px 8px", fontFamily:"inherit" }}><div style={{ fontSize:10, fontWeight:700, display:"flex", justifyContent:"space-between", gap:8 }}><span>{m.cell} · {m.styleNo||"(no style)"}</span><span style={{ color:"var(--muted-2)", fontWeight:400 }}>{m.orderNo||""}</span></div><div style={{ fontSize:10, color:"var(--muted-4)", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{m.value}</div></button>):<div style={{ padding:"8px 9px", fontSize:10, color:"var(--muted-2)" }}>No matches.</div>}</div>}<div style={{ display:"flex", gap:8 }}><button disabled={!frFind} onClick={(e)=>{ e.stopPropagation(); findNext(); }} title="jump to the next cell containing this text" style={{ flex:1, fontFamily:"inherit", fontSize:11, fontWeight:700, padding:6, cursor:frFind?"pointer":"not-allowed", border:"1px solid var(--ink)", background:"var(--surface)", opacity:frFind?1:0.6 }}>Find next</button><button disabled={!frFind} onClick={(e)=>{ e.stopPropagation(); const n=getFindMatches().length; if(!n){ window.alert('No cells match "'+frFind+'".'); return; } const bw=frRepl.trim()===""?"\n\n⚠️ This will BLANK those cells.":""; if(!window.confirm('Replace "'+frFind+'" → "'+(frRepl||"(blank)")+'" in '+n+' cell(s)?'+bw)) return; findReplace(false); flash(); }} style={{ flex:1, fontFamily:"inherit", fontSize:11, fontWeight:700, padding:6, cursor:frFind?"pointer":"not-allowed", border:"1px solid var(--ink)", background:frFind?"var(--accent)":"var(--line-2)", opacity:frFind?1:0.6 }}>Replace all</button><button onClick={()=>setFrOpen(false)} style={{ fontFamily:"inherit", fontSize:11, padding:"6px 10px", cursor:"pointer", border:"1px solid var(--ink)", background:"var(--bg)" }}><X size={12}/></button></div></div>); })()}</span>
...
{(layoutMode!=="workspace" || ["home","clipboard","collab"].includes(activeRibbon)) && <div style={layoutMode==="workspace"?{ display:"flex", gap:14, alignItems:"stretch", padding:"10px 14px 12px", margin:"0 22px 8px", flexWrap:"wrap", fontSize:10, color:"#777", background:"linear-gradient(180deg,#fbfaf7 0%, #f4efe6 100%)", border:"1px solid #d7cfbf", boxShadow:"4px 4px 0 rgba(26,26,26,0.10)" }:{ display:"flex", gap:10, alignItems:"center", padding:"0 22px 10px", flexWrap:"wrap", fontSize:10, color:"#777" }}>{ribbonLabel(activeRibbon==="clipboard"?"Clipboard":"Cell tools / notes")}
        ...
        <span style={{ marginLeft:10, position:"relative" }}>
          ...
          {threadCell && (()=>{ const ck=cellKey(threadCell.id,threadCell.col); const list=(comments[ck]||[]).slice().sort((a,b)=>new Date(a.created_at)-new Date(b.created_at)); const anyOpen=list.some(x=>!x.resolved); const mq=(cmText.match(/@(\S*)$/)||[])[1]; const sugg = mq!=null ? team.filter(t=>String(t.name||t.email||"").toLowerCase().includes(mq.toLowerCase())).slice(0,5) : [];
                return (<span onClick={e=>e.stopPropagation()} style={{ position:"absolute", top:"100%", left:0, marginTop:4, zIndex:85, background:"var(--surface)", border:"1px solid var(--ink)", boxShadow:"4px 4px 0 var(--ink)", padding:10, width:290, display:"block" }}>
      ...
      <div ref={scrollWrapRef} style={{ overflow:"auto", padding:"0 22px", maxHeight:"calc(100vh - 210px)" }}>
        <table role="grid" aria-label="Pre-production tracker grid. Arrow keys to move, Escape to exit, Tab to leave the grid." style={{ borderCollapse:"separate", borderSpacing:0, zoom:textScale, fontSize:11, tableLayout:"fixed", userSelect:dragSel?"none":"auto" }}>
          ...
          <tbody>
            {rows.map(({s,c},rowIdx)=>{ const t=TONE_STYLE[c.tone]; const sk=cellKey(s.id,"__style"); const styBg=bgFor(s.id,"__style","var(--surface)"); return (
              <tr key={s.id} role="row">
                <td id={`cell-${s.id}-__style`} onClick={(e)=>onCellClick(e,s.id,"__style")} onDoubleClick={(e)=>{ e.stopPropagation(); startEdit(s.id,"__style"); }} style={{ position:"sticky", left:0, zIndex:12, background:styBg, border:"1px solid var(--line-1)", padding:"6px 9px", overflow:"hidden", boxShadow:ringFor(s.id,"__style"), cursor:"cell" }}>
                ...
                {visInfo.map(col=>{
                  ...
                  else if(col.key==="chase") content=(!c.chaseOwners||c.chaseOwners.length===0)?<span style={{color:"var(--line-2)"}}>—</span>:(<span style={{ display:"flex", gap:3, flexWrap:"wrap" }}>{c.chaseOwners.map(o=>(<span key={o.owner} title={`${o.owner}: ${o.count} open ${o.count>1?"branches":"branch"}`} style={{ fontSize:9, fontWeight:700, padding:"2px 5px", background:(OWNER_COLOR[o.owner]||"var(--muted-2)")+"22", color:OWNER_COLOR[o.owner]||"var(--muted-4)", whiteSpace:"nowrap" }}>{o.owner}{o.count>1?`×${o.count}`:""}</span>))}</span>);
...
function FilterMenu({ values, allowed, onSet, onClose }){
  ...
  const menu=(
    <div onClick={e=>e.stopPropagation()} style={{ position:"fixed", top:pos?pos.top:-9999, left:pos?pos.left:-9999, zIndex:300, background:"var(--surface)", color:"var(--ink)", border:"1px solid var(--ink)", boxShadow:"4px 4px 0 var(--ink)", padding:8, width:210, textTransform:"none", letterSpacing:0, fontWeight:400, maxHeight:"80vh", overflowY:"auto" }}>
...
function DashboardView({ computed, todoItems, applyDrill, drillTodo }){
  ...
  const OWNER_COLOR2={ Merchant:"var(--success)", CAD:"#2563a6", Buyer:"#b4531a", Designer:"#6d4aab", Mill:"#7a5a1e" };
