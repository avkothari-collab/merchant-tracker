...
export default function App(){
  ...
  const beginDate=(id,col,mode)=>{ if(!ROLES[role].canEdit(ownerOfCol(col))) return; setSel({id,col}); setFocus(null); setEditing({id,col,mode}); setCalOpen(false); const s=styles.find(x=>x.id===id); const cur= mode==="rev"?(s&&s.revs&&s.revs[col]):(isStageCol(col)?(s&&s.actuals[col]):(s&&s[col])); setEditVal(cur?fmtTyped(cur):""); };
